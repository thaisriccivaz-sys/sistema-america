...
const menuStart = html.indexOf('<div class="dept-submenu-header" style="color:#7048e8;"><i class="ph ph-handshake"></i> Comercial</div>');
if (menuStart !== -1) {
    const aStart = html.indexOf('<a href="#" class="nav-item" data-target="comercial-em-breve"', menuStart);
    const aEnd = html.indexOf('</a>', aStart) + 4;
    
    if (aStart !== -1 && aEnd !== -1) {
        const menuNovo = `<a href="#" class="nav-item" data-target="comercial-credenciamento" onclick="navigateTo('comercial-credenciamento'); return false;"><i class="ph ph-identification-card"></i> Solicitar Credencial</a>`;
        html = html.substring(0, aStart) + menuNovo + html.substring(aEnd);
        console.log('Menu comercial atualizado');
    }
}
...
if (!html.includes('id="view-comercial-credenciamento"')) {
    const attachPointIdx = html.indexOf('<!-- VIEW: LICENCAS -->');
    if (attachPointIdx !== -1) {
        html = html.substring(0, attachPointIdx) + newView + '\n                ' + html.substring(attachPointIdx);
        console.log('View comercial adicionada');
    }
}
...
if (!html.includes('comercial_credenciamento.js')) {
    ...
    console.log('Script comercial adicionado');
}
