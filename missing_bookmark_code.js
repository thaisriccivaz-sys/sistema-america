...
// Helper global para coletar GPS e Device Info para auditoria jur├¡dica
window.getDeviceSecurityData = async function() {
    ...
    return new Promise((resolve) => {
        ...
        navigator.geolocation.getCurrentPosition(
            ...
            (error) => {
                console.warn('[GPS] Permiss├úo negada ou erro:', error);
...
window.gerarPDFBlob = async function (element, filename = 'documento.pdf') {
    return new Promise((resolve, reject) => {
        if (typeof html2pdf === 'undefined') return reject(new Error('Biblioteca html2pdf n├úo carregada'));
        ...
        // Limpar estilo para n├úo vazar bordas (evita 2a pagina vazia)
        ...
        const opt = {
            margin: [5, 0, 10, 0], // 5mm topo, 0 esq, 10mm inferior, 0 dir para margem de seguran├ºa na emenda das p├íginas
            filename: filename,
...
// --- INICIALIZA├ç├âO E ROTAS B├üSICAS ---

const DOCS_DISPONIVEIS = [
    "Acordo Individual Benef├¡cios", "Autoriza├º├úo Uso de Imagem", "Aux├¡lio Combust├¡vel",
    "Coca Cola Desconto", "Contrato Academia", "Contrato Faculdade", "Descri├º├úo de cargos",
    ...
    "Termo de Responsabilidade Ve├¡culo"
];
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (savedToken && savedUser) {
        ...
        if (appShell) {
            ...
            console.warn('O elemento app-shell n├úo foi encontrado. Interface antiga detectada ou HTML incompleto.');
...
// ÔöÇÔöÇ LEMBRAR USU├üRIO: preencher campos se houver dados salvos ÔöÇÔöÇ
(function () {
...
if (formLogin) {
    formLogin.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            // Carrega permiss├Áes se existir fun├º├úo global
            if (typeof window.carregarPermissoesOnline === 'function') {
...
// ÔöÇÔöÇ Sess├úo autom├ítica: logout ap├│s 5 horas ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
(function () {
    ...
    function mostrarAvisoExpiracao(faltamMs) {
        ...
        if (!div) {
            ...
            // Anima├º├úo CSS para o card aparecer da esquerda
            ...
            div.innerHTML = `
                ...
                <div>
                    <h4 style="margin:0 0 4px; font-size:0.95rem; font-weight:700;">Aten├º├úo! Salve tudo.</h4>
                    <p style="margin:0; font-size:0.85rem; line-height:1.4;">
                        O sistema ser├í relogado em <strong id="aviso-sessao-timer" style="font-size:1.1rem;">--</strong> segundos.
                    </p>
                </div>
            `;
            document.body.appendChild(div);
        ...
        ov.innerHTML = `
            <div style="background:#fff;border-radius:16px;width:92vw;max-width:400px;box-shadow:0 25px 60px rgba(0,0,0,0.4);overflow:hidden;text-align:center;">
                <div style="background:linear-gradient(135deg,#1e293b,#334155);padding:2rem 1.5rem;">
                    ...
                    <h3 style="margin:0;color:#fff;font-size:1.15rem;font-weight:700;">Sess├úo Expirada</h3>
                </div>
                <div style="padding:1.75rem 1.5rem;">
                    <p style="color:#475569;margin:0 0 1.5rem;font-size:0.93rem;line-height:1.6;">
                        Sua sess├úo expirou ap├│s <strong>5 horas</strong> de uso.<br>
                        Por seguran├ºa, fa├ºa login novamente para continuar.
                    </p>
    ...
    function verificarSessao() {
        if (!localStorage.getItem('erp_token')) return; // n├úo logado
    ...
    // Verificar ao carregar (caso o usu├írio reabra ap├│s 5h)
    ...
    // Verificar a cada 1 segundo para o cron├┤metro funcionar corretamente
...
window.navigateInitialPage = function () {
    ...
    if (window.activeUserPerms) {
        ...
        // Pega a primeira aba autorizada caso nenhuma das favoritas esteja dispon├¡vel
...
// ÔöÇÔöÇ BREADCRUMB SYSTEM ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
const BREADCRUMB_MAP = {
    // Telas principais
    'integracao': { path: 'Integra├º├úo', code: 'RHAD06' },
    ...
    'form-colaborador': { path: 'Colaboradores ÔåÆ Cadastro / Edi├º├úo' },
    ...
    'admissao': { path: 'Admiss├úo', code: 'RHAD05' },
    'ficha-epi': { path: 'Ficha EPI', code: 'RHEPI01' },
    'avaliacoes': { path: 'Avalia├º├Áes', code: 'RHAV01' },
    'gerenciar-avaliacoes': { path: 'Diretoria ÔåÆ Gerenciar Avalia├º├Áes', code: 'DIRAVAL' },
    'usuarios-permissoes': { path: 'Diretoria ÔåÆ Usu├írios e Permiss├Áes', code: 'DIR001' },
    'cargos': { path: 'Diretoria ÔåÆ Cargos', code: 'DIR004' },
    'form-usuario': { path: 'Diretoria ÔåÆ Usu├írios e Permiss├Áes ÔåÆ Cadastro', code: 'DIR002' },
    'certificado-digital': { path: 'Diretoria ÔåÆ Certificado Digital', code: 'DIR003' },
    'auditoria': { path: 'Diretoria ÔåÆ Trilha de Auditoria', code: 'DIR006' },
    // Sub-telas (Prontu├írio Digital - abas)
    'tab:00. CheckList': { path: 'Colaboradores ÔåÆ Prontu├írio Digital ÔåÆ 00. CheckList', },
    'tab:01_FICHA_CADASTRAL': { path: 'Colaboradores ÔåÆ Prontu├írio Digital ÔåÆ Ficha Cadastral', },
    'tab:Ficha Cadastral': { path: 'Colaboradores ÔåÆ Prontu├írio Digital ÔåÆ Ficha Cadastral', },
    'tab:Pagamentos': { path: 'Colaboradores ÔåÆ Prontu├írio Digital ÔåÆ Pagamentos', },
    'tab:ASO': { path: 'Colaboradores ÔåÆ Prontu├írio Digital ÔåÆ ASO', },
    'tab:Ficha de EPI': { path: 'Colaboradores ÔåÆ Prontu├írio Digital ÔåÆ Ficha de EPI', },
    'tab:Atestados': { path: 'Colaboradores ÔåÆ Prontu├írio Digital ÔåÆ Atestados', },
    'tab:Faltas': { path: 'Colaboradores ÔåÆ Prontu├írio Digital ÔåÆ Faltas', },
    'tab:Contratos': { path: 'Colaboradores ÔåÆ Prontu├írio Digital ÔåÆ Contratos', },
    'tab:Avalia├º├úo': { path: 'Colaboradores ÔåÆ Prontu├írio Digital ÔåÆ Avalia├º├úo', },
    'tab:Avalia├º├Áes': { path: 'Colaboradores ÔåÆ Prontu├írio Digital ÔåÆ Avalia├º├Áes', },
    'tab:Advert├¬ncias': { path: 'Colaboradores ÔåÆ Prontu├írio Digital ÔåÆ Ocorr├¬ncias', },
    'tab:Faculdade': { path: 'Colaboradores ÔåÆ Prontu├írio Digital ÔåÆ Faculdade', },
    'tab:Boletim de ocorr├¬ncia': { path: 'Colaboradores ÔåÆ Prontu├írio Digital ÔåÆ Boletim de Ocorr├¬ncia', },
    'tab:Certificados': { path: 'Colaboradores ÔåÆ Prontu├írio Digital ÔåÆ Certificados', },
    'tab:Conjuge': { path: 'Colaboradores ÔåÆ Prontu├írio Digital ÔåÆ Conjuge', },
    'tab:Dependentes': { path: 'Colaboradores ÔåÆ Prontu├írio Digital ÔåÆ Dependentes', },
    'tab:Fotos': { path: 'Colaboradores ÔåÆ Prontu├írio Digital ÔåÆ Fotos', },
    'tab:Multas': { path: 'Colaboradores ÔåÆ Prontu├írio Digital ÔåÆ Multas', },
    'tab:NRs': { path: 'Colaboradores ÔåÆ Prontu├írio Digital ÔåÆ NRs', },
    'tab:Terapia': { path: 'Colaboradores ÔåÆ Prontu├írio Digital ÔåÆ Terapia', },
    'tab:Treinamento': { path: 'Colaboradores ÔåÆ Prontu├írio Digital ÔåÆ Treinamento', },
    'tab:Documentos': { path: 'Colaboradores ÔåÆ Prontu├írio Digital ÔåÆ Documentos', },
    'tab:Afastamentos': { path: 'Colaboradores ÔåÆ Prontu├írio Digital ÔåÆ Afastamentos', },
    'tab:Chaves': { path: 'Colaboradores ÔåÆ Prontu├írio Digital ÔåÆ Chaves', },
    'tab:Prontu├írio Digital': { path: 'Colaboradores ÔåÆ Prontu├írio Digital', },
    // M├│dulos adicionais
    'dissidio': { path: 'Diss├¡dio', code: 'RHDIS01' },
    'recibos': { path: 'Recibos de Benef├¡cios', code: 'RHREC01' },
    'ferias': { path: 'Controle de F├®rias', code: 'RHFER01' },
    // Log├¡stica
    'logistica-dashboard': { path: 'Dashboard Log├¡stica', code: 'LOG000' },
    'logistica-sinistros': { path: 'Sinistros Log├¡stica', code: 'LOG010' },
    ...
    'logistica-multas-monaco': { path: 'Multas M├┤naco', code: 'LOG012' },
    ...
    'logistica-agenda': { path: 'Agenda Log├¡stica', code: 'LOG011' },
    ...
    // Administrativo
    'licencas': { path: 'Licen├ºas', code: 'ADM001' },
...
window.carregarPermissoesOnline = async function () {
    ...
    // Define quem ├® super admin e pode ver tudo por padr├úo (Apenas Diretoria!)
    // Verifica: role do banco, departamento OU nome do grupo de permiss├úo
    ...
    // Remove qualquer display-none for├ºado das categorias primeiro
    document.querySelectorAll('.dept-item').forEach(el => el.style.display = '');

    if (isTopAdmin) {
        // Regra fixa solicitada: Ocultar m├│dulos nunca se aplica a lideran├ºa da Diretoria.
        // Eles t├¬m acesso autom├ítico e irrestrito a todas as telas visualmente.
        document.querySelectorAll('.nav-item').forEach(el => el.style.display = '');
        return; // Retorna cedo ignorando qualquer grupo
    }

    if (!currentUser.grupo_permissao_id) {
        // Usu├írio comum sem permiss├Áes. Ocultar tudo.
    ...
    // Avan├ºa para ler as regras reais do grupo (se for usu├írio comum com grupo)

    try {
        const res = await fetch(`${API_URL}/grupos-permissao/${currentUser.grupo_permissao_id}/permissoes`, {
            headers: { Authorization: `Bearer ${currentToken}` }
        });
        if (!res.ok) throw new Error('Falha ao obter permiss├Áes do grupo.');
        const permissoes = await res.json();

        // Cria um mapa r├ípido das permissoes ativas
        const mapPerms = {};
        ...
        // Auto-liberar certificado-digital para quem j├í tem acesso a usuarios-permissoes
        if (mapPerms['usuarios-permissoes']) {
            mapPerms['certificado-digital'] = true;
        }
        window.activeUserPerms = mapPerms;

        // Percorre todos os bot├Áes de navega├º├úo (.nav-item)
        document.querySelectorAll('.nav-item[data-target]').forEach(link => {
            const pathId = link.getAttribute('data-target');

            // Hardcode: m├│dulos "Em breve" nunca devem aparecer para usu├írios comuns, 
            // mesmo que marcados sem querer no banco.
            if (pathId && pathId.includes('em-breve')) {
                link.style.cssText = 'display: none !important;';
                mapPerms[pathId] = false; // For├ºa no mapa para ocultar a bolota tamb├®m
                return;
            }

            // Se existir no mapa de permissoes e for TRUE, mostra. Sen├úo, esconde robustamente.
            if (mapPerms[pathId]) {
                link.style.display = '';
            } else {
                link.style.cssText = 'display: none !important;';
        ...
        // Agora vamos ocultar os "blocos grandes" (Departamentos) inteiros se n├úo sobrar nenhum nav-item ├║til
        ...
        deptSubmenus.forEach(submenu => {
            ...
            const headerObj = submenu.parentElement; // o `.dept-item` ├® o pai

            if (navItems.length > 0) {
                // Checa diretamente no mapa de permiss├Áes se o cara tem algo liberado aqui!
        ...
        // Sempre garantimos que o ├¡cone de SAIR apare├ºa ent├úo n├úo h├í risco.

    } catch (err) {
        console.error("Erro no carregamento de permiss├Áes: ", err);
    }
};
...
function updateBreadcrumb(key) {
    ...
    // Atualiza bot├úo de scroll flutuante
    const scrollerBtn = document.getElementById('global-scroll-top');
    if (scrollerBtn) {
        scrollerBtn.style.backgroundColor = pageColor;
    }

    // Mostra a estrela APENAS se for tela de menu principal ou telas base (sem setas 'ÔåÆ', exceto Diretoria)
    const starBtn = document.getElementById('btn-star-page');
    if (starBtn && entryObj) {
        starBtn.style.color = pageColor;
        const isSimplePage = (!entryObj.path.includes('ÔåÆ') && !key.startsWith('tab:')) || key === 'usuarios-permissoes' || key === 'form-usuario' || key === 'logistica-rota-redonda' || key === 'logistica-multas' || key === 'logistica-multas-monaco' || key === 'logistica-equipes' || key === 'logistica-pipeline' || key === 'logistica-frota' || key === 'logistica-credenciamento' || key === 'logistica-senhas' || key === 'comercial-credenciamento' || key === 'comercial-proposta' || key === 'departamentos' || key === 'logistica-agenda' || key === 'logistica-epi' || key === 'rh-agenda' || key === 'estoque' || key === 'licencas';
        if (isSimplePage) {
    ...
    if (typeof renderBookmarks === 'function') setTimeout(renderBookmarks, 50); // For├ºa render com o novo key
    if (!bar) return;
    const entry = BREADCRUMB_MAP[key] || { path: key };
    const parts = entry.path.split('ÔåÆ').map(p => p.trim());

    // Assegurar que 'Diretoria' ou 'RH' n├úo se duplique se colocarmos o ├¡cone,
    // mas o usu├írio pediu "icones dos setores e o c├│digo" (talvez "nome"?). Vamos por o ├¡cone:
    bar.innerHTML = `${deptIconHTML}<span style="opacity:0.7;margin-right:4px;">Caminho:</span>` +
        parts.map((p, i) =>
            i < parts.length - 1
                ? `<span style="opacity:0.75;">${p}</span><span style="margin:0 5px;opacity:0.5;">ÔåÆ</span>`
                : `<strong>${p}</strong>`
        ).join('');
}
let appOpenTabs = [];

// ÔöÇÔöÇ METADADOS DE ABAS: cor, ├¡cone e m├│dulo por tela ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
const TAB_META = {
    // RH - Rosa
    ...
    'prontuario': { color: '#f503c5', icon: 'ph-folder-open', title: 'Prontu├írio Digital' },
    'admissao': { color: '#f503c5', icon: 'ph-list-checks', title: 'Admiss├úo' },
    'integracao': { color: '#f503c5', icon: 'ph-users-three', title: 'Integra├º├úo' },
    ...
    'gerenciar-avaliacoes': { color: '#d9480f', icon: 'ph-clipboard-text', title: 'Avalia├º├Áes' },
    ...
    'dissidio': { color: '#f503c5', icon: 'ph-trend-up', title: 'Diss├¡dio' },
    'recibos': { color: '#f503c5', icon: 'ph-receipt', title: 'Recibos' },
    'ferias': { color: '#f503c5', icon: 'ph-airplane-tilt', title: 'F├®rias' },
    'experiencia': { color: '#f503c5', icon: 'ph-user-check', title: 'Experi├¬ncia' },
    // Diretoria - Laranja
    'usuarios-permissoes': { color: '#d9480f', icon: 'ph-users-three', title: 'Usu├írios e Permiss├Áes' },
    ...
    'form-usuario': { color: '#d9480f', icon: 'ph-user-gear', title: 'Cadastro de Usu├írio' },
    'config-sigor': { color: '#d9480f', icon: 'ph-key', title: 'Credenciais SIGOR' },
    // Log├¡stica - Verde
    'logistica-em-breve': { color: '#2d9e5f', icon: 'ph-truck', title: 'Log├¡stica' },
    'logistica-dashboard': { color: '#2d9e5f', icon: 'ph-chart-bar', title: 'Dashboard Log├¡stica' },
    'logistica-sinistros': { color: '#059669', icon: 'ph-warning', title: 'Sinistros Log├¡stica' },
    ...
    'logistica-multas-monaco': { color: '#dc2626', icon: 'ph-car', title: 'Multas M├┤naco' },
    ...
    'licencas': { color: '#e67700', icon: 'ph-certificate', title: 'Licen├ºas' },
...
function renderAppTabs() {
    ...
    container.innerHTML = appOpenTabs.map(t => {
        ...
        <div class="app-top-tab ${t.active ? 'active' : ''}" 
            ...
            <span title="${t.title}">${t.title.length > 15 ? t.title.substring(0, 15) + 'ÔÇª' : t.title}</span>
...
// Navegar para uma aba existente pelo seu tabId ├║nico
window.navigateToTab = function (tabId) {
    ...
    // Se a aba tem dados de colaborador (prontu├írio ou form), restaura o viewedColaborador
...
// navigateTo: abre uma aba ├ÜNICA por target (telas de lista/config).
// Para colaborador/prontu├írio, use openColaboradorTab / openProntuarioTab.
function navigateTo(target) {
    ...
    if (currentUser && !isTopAdmin) {
        ...
        if (targetNav && targetNav.style.display === 'none') {
            alert('Voc├¬ n├úo tem permiss├úo para acessar esta tela.');
            return;
        }
    }
...
// Se j├í existir aba para esse colaborador, apenas ativa. Novo colaborador usa tabId 'form-colaborador-novo'.
window._openColaboradorTab = function (colabId, nomeColab) {
    ...
    if (!existingTab) {
        ...
        existingTab.title = label; // Atualiza nome se necess├írio
    }
    appOpenTabs.forEach(t => t.active = (t.tabId === tabId));
    renderAppTabs();
...
// Abre uma aba de PRONTU├üRIO, nomeada com o colaborador.
window._openProntuarioTab = function (colabId, nomeColab, colaboradorData) {
    ...
    const label = `Prontu├írio: ${firstName}`;
...
function setupNavigation() {
    document.querySelectorAll('.sidebar-nav .nav-item[data-target], .dept-submenu .nav-item[data-target]').forEach(item => {
        item.addEventListener('click', (e) => {
            ...
            // Recolhe o submenu automaticamente ap├│s o clique
...
async function apiGet(endpoint) {
    try {
        ...
        if (!res.ok) throw new Error('Falha na requisi├º├úo');
        return res.json();
    } catch (e) {
        console.error(e);
        return null;
...
async function apiPost(endpoint, data, options = {}) {
    ...
    if (contentType && contentType.indexOf("application/json") !== -1) {
        ...
        console.error("Erro na API (N├úo ├® JSON):", text);
...
async function loadCargos() {
    ...
    // Tamb├®m popula o select do formul├írio de colaborador (para quando estiver cadastrando algu├®m)
...
window.toggleCargoView = async function (mode, id = null) {
    ...
    if (mode === 'list') {
        ...
        if (headerActions) headerActions.style.display = 'none'; // Esconde bot├Áes no topo ao ver a lista
        loadCargos();
    } else {
        if (listContainer) listContainer.style.display = 'none';
        if (formContainer) formContainer.style.display = 'block';
        if (headerActions) headerActions.style.display = 'flex'; // Mostra bot├Áes no topo ao editar/criar
...
async function handleCargoFormSubmit() {
    ...
    try {
        ...
        alert('Erro de conex├úo ao salvar cargo.');
    }
}
...
window.handleDeleteCargoUI = async function () {
    ...
    if (nome.toUpperCase() === 'MOTORISTA') {
        alert('O cargo MOTORISTA ├® essencial para o sistema e n├úo pode ser exclu├¡do.');
        return;
    }
...
// Excluir cargo diretamente da listagem (sem precisar abrir o formul├írio de edi├º├úo)
...
// ÔöÇÔöÇÔöÇ CERTIFICADO DIGITAL P├ôS-ASSINATURA ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
// Aplica o certificado A1 da empresa no PDF ap├│s o colaborador assinar no Assinafy
window.assinarComCertificado = async function (assId, event) {
    ...
    try {
        ...
        if (btn) {
            ...
            btn.innerHTML = '<i class="ph ph-seal-check"></i> Ô£à Certificado Aplicado';
            setTimeout(() => {
                // Recarregar a view para refletir o novo status
                if (viewedColaborador) window.viewColaborador(viewedColaborador.id);
            }, 1500);
        }
        if (typeof showToast === 'function') showToast('Ô£à ' + data.mensagem, 'success');
        else alert('Ô£à ' + data.mensagem);

    } catch (e) {
        if (btn) { btn.disabled = false; btn.innerHTML = originalText; }
        if (typeof showToast === 'function') showToast('ÔØî ' + e.message, 'error');
        else alert('ÔØî ' + e.message);
    }
};
...
async function loadDepartamentos() {
    ...
    deptos.forEach(d => {
        const responsavel = d.responsavel_nome ? d.responsavel_nome : '<span style="color:#94a3b8;font-style:italic;">N├úo definido</span>';
...
// --- HELPER PARA ESCALAS NO FORMUL├üRIO ---
window.toggleFormEscalaTipo = function () {
    ...
    // 12x36: sa├¡da calculada automaticamente (n├úo edit├ível)
    if (outSaida) {
...
// Atualiza o t├¡tulo e descri├º├úo do bloco de ciclo conforme o tipo de escala
window.atualizarLabelCiclo = function (modo) {
    ...
    if (!lbl || !small) return;
    if (modo === '12x36') {
        lbl.innerHTML = '<i class="ph ph-calendar-blank"></i> 12x36 ÔÇö Data de in├¡cio do ciclo';
        small.innerHTML = '<i class="ph ph-info"></i> Informe o <strong>primeiro dia de trabalho</strong> desta pessoa. '
            + 'O sistema alterna automaticamente: <strong>1 dia trabalha ÔåÆ 1 dia folga (36h)</strong>.<br>'
            + 'Sem essa data o colaborador aparece sempre como dispon├¡vel na agenda.';
    } else if (modo === 'sab_alternado') {
        lbl.innerHTML = '<i class="ph ph-calendar-blank"></i> S├íbados Alternados ÔÇö Data de refer├¬ncia';
        small.innerHTML = '<i class="ph ph-info"></i> Informe um <strong>s├íbado trabalhado</strong> de refer├¬ncia. '
            + 'O sistema alterna automaticamente: <strong>s├íbado trabalha ÔåÆ s├íbado folga ÔåÆ s├íbado trabalha...</strong><br>'
            + 'Sem essa data o sistema n├úo saber├í qual s├íbado ├® folga.';
    } else {
        lbl.innerHTML = '<i class="ph ph-calendar-blank"></i> Domingo de Lei ÔÇö Data de Refer├¬ncia do Ciclo';
        small.innerHTML = '<i class="ph ph-info"></i> Informe um <strong>domingo</strong> onde o ciclo come├ºa (<em>1┬║ domingo trabalhado</em>). '
            + 'A cada 3 domingos, o sistema automaticamente marca o 3┬║ como <strong>Domingo de Lei</strong> (folga obrigat├│ria).<br>'
            + 'Para Folga 2 dias: na semana do Domingo de Lei, o 1┬║ dia fixo se torna trabalho para manter a carga hor├íria.<br>'
            + 'Para Folga 1 dia: o Domingo de Lei ├® somado ├á folga fixa (semana com menos horas ÔÇö legal).';
    }
};
...
window.addNewChaveRow = async function (selectedChaveId = null, selectedDate = null) {
    try {
        ...
        // Remover Bot├úo de + das linhas anteriores se houver
...
window.removeChaveRow = function (btn) {
    ...
    row.remove();
    // Reexibir o bot├úo + na nova "├║ltima linha"
    ...
    if (rows.length > 0) {
        ...
        // Ou deixar o toggle Sim/N├úo resolver.
    }
};
...
window.toggleTransporteValor = function (val) {
    ...
    if (group) {
        ...
        if (val === 'Vale Transporte (VT)' || val === 'Vale Combust├¡vel (VC)') {
            group.style.display = 'block';
            // Se for VT, definir valor di├írio fixo
            if (val === 'Vale Transporte (VT)') {
                ...
                if (lbl && !lbl.querySelector('.vt-hint')) {
                    ...
                    hint.textContent = '(Valor Di├írio Fixo)';
...
// Recalcula VT em tempo real ao editar o sal├írio (chamado pelo onkeyup do campo sal├írio)
window.atualizarVTSeSelecionado = function () {
    ...
    // For├ºa 6,20 para VT em vez de calcular 6% do sal├írio
    inputT.value = 'R$ 6,20';
};
...
window.calcularHorarioSaida = function () {
    ...
    // ÔöÇÔöÇ Escala 12x36: l├│gica especial ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
    if (tipo === 'escala_12x36') {
        if (entrada) {
            const [he, me] = entrada.split(':').map(Number);
            const entradaMins = he * 60 + me;

            // Sa├¡da = entrada + 12h (n├úo edit├ível)
            ...
            // Pausa = 1h a partir de 6h ap├│s entrada (edit├ível ÔÇö s├│ preenche se estiver vazio)
        ...
        return; // Encerra: 12x36 n├úo tem c├ílculo de s├íbado
    }
    // ÔöÇÔöÇ Demais escalas ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ

    if (tipo && entrada) {
        // Calcula dura├º├úo do intervalo em minutos
        let intervaloMins = 0;
        if (intEntrada && intSaida) {
        ...
        // Tempo l├¡quido de trabalho di├írio
        let workMins = 0;
        ...
        let almo├ºoPadraoMin = 0;
        if (workMins >= (6 * 60)) {
            almo├ºoPadraoMin = 60; // 1 hora de almo├ºo

            // Se os campos de intervalo estiverem em branco, preenchemos com um padr├úo inteligente
            if (!intEntrada && !intSaida) {
                const [he, me] = entrada.split(':').map(Number);
                // Sugere almo├ºo 4 horas ap├│s a entrada
                ...
                let tempSaidaAlmoco = tempEntradaAlmoco + almo├ºoPadraoMin;
                ...
                intervaloMins = almo├ºoPadraoMin;
            }
        }

        if (workMins > 0) {
            const [he, me] = entrada.split(':').map(Number);
            let totalMins = (he * 60 + me) + workMins + intervaloMins; // O total do dia no rel├│gio ├® o trabalho + o intervalo
...
window.updateVacationInfo = function (admissaoStr) {
    ...
    // Tenta ler do campo do formul├írio se admissaoStr vazio
    ...
    try {
        // ÔöÇÔöÇÔöÇ Usa calcularFerias do ferias.js se dispon├¡vel (garante c├ílculo id├¬ntico) ÔöÇÔöÇÔöÇ
        ...
        const parseAdm = (s) => {
            ...
            // Converte DD/MM/YYYY ÔåÆ YYYY-MM-DD
        ...
        // ÔöÇÔöÇÔöÇ Usa calcularFerias do ferias.js (exposta via window._feriasCalc) ÔöÇÔöÇÔöÇ
        if (typeof window._feriasCalc === 'function') {
            ...
            aqField.value = fmt(ult.fim);       // in├¡cio do concessivo
            ...
            if (exibirAlerta) {
                ...
                if (indicator) {
                    ...
                    if (span) span.textContent = diasParaVencimento === 0
                        ? 'Aten├º├úo: F├®rias vencem HOJE!'
                        : `Aten├º├úo: As f├®rias vencem em ${diasParaVencimento} dia(s). Devem ser concedidas antes de ${concField.value}!`;
                }
            } else {
        ...
        // ÔöÇÔöÇÔöÇ Fallback: c├ílculo interno (mesmo algoritmo do ferias.js) ÔöÇÔöÇÔöÇ
        const diasTotal = Math.floor((today - adm) / 86400000);
        const anosCompletos = Math.floor(diasTotal / 365);
        if (anosCompletos < 1) {
            aqField.value = 'Em aquisi├º├úo'; concField.value = '-';
            concField.style.color = '#495057'; concField.style.fontWeight = '600';
            if (indicator) indicator.style.display = 'none';
            return;
        }
        ...
        console.error('Erro ao calcular datas de f├®rias:', e);
    }
}
...
window.calculateVacationDays = function () {
    ...
    if (diffDays < 0) {
        totalField.value = 'Data Inv├ílida';
    } else {
        totalField.value = `${diffDays} ${diffDays === 1 ? 'dia' : 'dias'}`;
    }

    // Re-avaliar alerta de f├®rias ao alterar as datas do formul├írio
...
window.calcularTotalFeriasFracionadas = function () {
    ...
    d2.value = diff >= 0 ? `${diff} ${diff === 1 ? 'dia' : 'dias'}` : 'Data Inv├ílida';
};
    ...
    if (chartsData) {
        ...
        if (tbFerias) {
            ...
            if (!chartsData.feriasVencendo || chartsData.feriasVencendo.length === 0) {
                tbFerias.innerHTML = '<tr><td colspan="3" style="text-align:center;color:#999;font-style:italic;">Nenhuma f├®rias a vencer em 90 dias com agendamento.</td></tr>';
            } else {
                chartsData.feriasVencendo.forEach(f => {
                    ...
                    // L├│gica de cores igual ├á tela de Controle de F├®rias
                    let barColor, labelRestante, labelColor;
                    if (f.ferias_agendadas && f.ferias_inicio_fmt) {
                        // Verde ÔÇö f├®rias agendadas
                        barColor = '#16a34a';
                        labelRestante = `Ô£à Agendada: ${f.ferias_inicio_fmt}`;
                        labelColor = '#15803d';
                    } else if (f.dias_restantes <= 0) {
                        // Vermelho ÔÇö prazo vencido
                        barColor = '#ef4444';
                        labelRestante = 'ÔÜá´©Å Prazo vencido!';
                        labelColor = '#ef4444';
                    } else if (f.dias_restantes <= 30) {
                        // Vermelho ÔÇö urgente (Ôëñ30 dias)
                        barColor = '#ef4444';
                        labelRestante = `Vence ${f.dias_restantes}d`;
                        labelColor = '#ef4444';
                    } else if (f.dias_restantes <= 90) {
                        // Laranja ÔÇö aten├º├úo (Ôëñ90 dias sem agenda)
                        barColor = '#f59e0b';
                        labelRestante = `Vence ${f.dias_restantes}d`;
                        labelColor = '#b45309';
                    } else {
        ...
        if (tbAso) {
            ...
            if (!chartsData.asoVencendo || chartsData.asoVencendo.length === 0) {
                ...
                chartsData.asoVencendo.forEach(a => {
                    let agendadoDisplay = '<span style="color:#999;font-size:0.8rem;">N├úo registrado</span>';
                    if (a.aso_exame_data) {
        ...
        if (tbFerias2) tbFerias2.innerHTML = '<tr><td colspan="3" style="text-align:center;color:#999;font-style:italic;">Sem dados dispon├¡veis.</td></tr>';
        const tbAso2 = document.getElementById('dash-table-aso');
        if (tbAso2) tbAso2.innerHTML = '<tr><td colspan="3" style="text-align:center;color:#999;font-style:italic;">Sem dados dispon├¡veis.</td></tr>';
        const tbDevol2 = document.getElementById('dash-table-devolucoes');
        if (tbDevol2) tbDevol2.innerHTML = '<tr><td colspan="5" style="text-align:center;color:#999;font-style:italic;">Sem dados dispon├¡veis.</td></tr>';
    }

    // --- Quadro de Devolu├º├úo de Equipamentos ---
    ...
    if (tbDevol) {
        try {
            ...
            if (!emprestimos || emprestimos.length === 0) {
                tbDevol.innerHTML = '<tr><td colspan="5" style="text-align:center;color:#999;font-style:italic;">Nenhuma devolu├º├úo pendente.</td></tr>';
            } else {
                ...
                emprestimos.forEach(emp => {
                    ...
                    const dtEntregaFmt = emp.data_entrega || 'ÔÇö';
                    const dtDevolFmt = emp.data_devolucao_prevista || 'ÔÇö';
                    const nomeStr = emp.colaborador_nome || '?';
                    const iniciais = nomeStr.trim().split(/\s+/).filter(Boolean).slice(0,2).map(w => w[0]).join('').toUpperCase();
                    const fotoApiUrl = `/api/colaboradores/foto/${emp.colaborador_id}`;
                    const alertIcon = vencida ? '<span title="Prazo vencido" style="color:#ef4444;font-size:1rem;">ÔÜá´©Å </span>' : '';
                    tbDevol.innerHTML += `
                        <tr style="border-bottom:1px solid #f1f5f9;${rowStyle}">
                            ...
                            <td style="padding:0.6rem 0.65rem;text-align:center;">
                                <button onclick="window.devolverEquipamento(${emp.id})" title="Registrar Devolu├º├úo ao Estoque"
            ...
            if (tbDevol) tbDevol.innerHTML = '<tr><td colspan="5" style="text-align:center;color:#999;font-style:italic;">Erro ao carregar devolu├º├Áes.</td></tr>';
        }
    }

    // --- Quadro de Aniversariantes do M├¬s ---
        ...
        try {
            ...
            if (!cols || cols.length === 0) {
                ...
                if (aniversariantes.length === 0) {
                    tbAniver.innerHTML = '<tr><td colspan="2" style="text-align:center;color:#999;font-style:italic;">Nenhum aniversariante neste m├¬s.</td></tr>';
                    const titAniv = document.getElementById('titulo-aniversariantes');
                    if (titAniv) titAniv.innerHTML = '0';
                } else {
                    const titAniv = document.getElementById('titulo-aniversariantes');
...
function aplicarFiltrosColaboradores() {
    ...
    const lista = _todosColaboradores.filter(c => {
        ...
        // Ocultar Desligados por padr├úo ÔÇö s├│ aparecem se o filtro de status for explicitamente 'Desligado'
        ...
        // Verifica dependentes (pode estar como "Sim", "N├úo", true, false, etc.)
        ...
        if (f.aptoSorteio) {
            ...
            let admDias = 999; // Assume apto se data n├úo informada
            if (c.data_admissao) {
            ...
            const isApto = (faltas <= 3) &&
                (punicoes === 0) &&
                (['Ativo', 'Afastado', 'F├®rias'].includes(statusEf)) &&
                (admDias >= 90) &&
                isCLT;
...
function limparFiltrosColaboradores() {
    ...
    // Atualizar bot├Áes visuais de tipo cadastro
...
window.exportarColaboradoresXLSX = async function () {
    ...
    worksheet.getCell('D1').value = "RELAT├ôRIO DE COLABORADORES";
    ...
    const headerRow = worksheet.addRow([
        "Status", "Nome Completo", "CPF", "RG", "Data Nascimento", "Nome da M├úe", "Nome do Pai",
        "Estado Civil", "Sexo", "E-mail", "Telefone", "Contato Emerg├¬ncia",
        "CEP", "Rua", "N┬║", "Complemento", "Bairro", "Cidade", "Estado",
        "Banco", "Ag├¬ncia", "Conta", "Tipo Conta", "PIX",
        "Departamento", "Cargo", "Data Admiss├úo", "Sal├írio", "Escala Trabalho",
        "PIS", "CTPS", "T├¡tulo Eleitor", "Certificado Militar",
        "CNH", "Cat CNH", "Emiss├úo CNH", "Validade CNH", "CID",
        "F├®rias In├¡cio", "F├®rias Fim", "F├®rias Retorno", "Possui Dependentes",
        "Faculdade", "Academia", "Terapia", "Celular", "Chaves",
        "Tamanho Camiseta", "Tamanho Cal├ºa", "Tamanho Cal├ºado"
    ]);
    ...
    colaboradores.forEach(c => {
        const row = worksheet.addRow([
            ...
            c.tem_dependentes ? 'Sim' : 'N├úo',
            c.faculdade_participa === 'Sim' ? 'Sim' : 'N├úo',
            c.academia_participa === 'Sim' ? 'Sim' : 'N├úo',
            c.terapia_participa === 'Sim' ? 'Sim' : 'N├úo',
            c.celular_participa === 'Sim' ? 'Sim' : 'N├úo',
            c.chaves_participa === 'Sim' ? 'Sim' : 'N├úo',
            c.tamanho_camiseta || '',
            c.tamanho_calca || '',
            c.tamanho_calcado || ''
        ]);
    ...
    // Congela as 5 primeiras linhas (cabe├ºalho) e as 2 primeiras colunas (Status e Nome)
    worksheet.views = [
...
function renderColaboradores(lista) {
    ...
    // Fun├º├úo auxiliar para escalar tipo "padrao_seis_dias" -> "Padrao Seis Dias"
    const formatEscala = (e) => (e || '').replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase());

    // Coletar op├º├Áes ├║nicas para os selects dos filtros
    ...
    wrapper.innerHTML = `
        ...
        <div style="display:flex; align-items:center; flex-wrap:wrap; gap:0.4rem; margin-bottom:1rem; flex-shrink:0;">
            ...
            <button class="btn-tipo-cadastro" data-status="Aguardando in├¡cio" onclick="selecionarTipoCadastro(this, 'Aguardando in├¡cio')" style="padding:0.25rem 0.65rem; border:none; border-radius:999px; font-size:0.78rem; font-weight:600; cursor:pointer; background:#f1f3f5; color:#495057; display:flex; gap:3px; align-items:center; transition:0.2s; opacity:0.5; white-space:nowrap;"><i class="ph ph-hourglass-high"></i> Aguardando</button>
            ...
            <button class="btn-tipo-cadastro" data-status="F├®rias" onclick="selecionarTipoCadastro(this, 'F├®rias')" style="padding:0.25rem 0.65rem; border:none; border-radius:999px; font-size:0.78rem; font-weight:600; cursor:pointer; background:#ffedd5; color:#c2410c; display:flex; gap:3px; align-items:center; transition:0.2s; opacity:0.5; white-space:nowrap;"><i class="ph ph-airplane-tilt"></i> F├®rias</button>
        ...
        <div id="filtro-sidebar" style="position:fixed; top:0; right:-400px; width:400px; max-width:100vw; height:100vh; background:#fff; z-index:9999; box-shadow:-4px 0 15px rgba(0,0,0,0.1); transition:right 0.3s cubic-bezier(0.4, 0, 0.2, 1); overflow-y:auto; display:flex; flex-direction:column;">
            ...
            <div style="padding:1.5rem; border-bottom:1px solid #e2e8f0; display:flex; justify-content:space-between; align-items:center; position:sticky; top:0; background:#fff; z-index:10;">
                <span style="font-weight:700; color:#334155; font-size:1.1rem; display:flex; align-items:center; gap:8px;">
                    <i class="ph ph-funnel"></i> Filtros Avan├ºados
                </span>
            ...
            <div style="padding:1.5rem; display:flex; flex-direction:column; gap:1.25rem;">
                ...
                <div>
                    ...
                    <select id="f-apto-sorteio" onchange="aplicarFiltrosColaboradores()" style="width:100%;padding:0.5rem;border:1px solid #e2e8f0;border-radius:6px;font-size:0.85rem;">
                        <option value="">Todos</option><option value="sim">Sim</option><option value="nao">N├úo</option>
                    </select>
                </div>
                <div>
                ...
                <div style="display:flex; gap:1rem;">
                    ...
                    <div style="flex:1;">
                        <label style="font-size:0.75rem;font-weight:600;color:#64748b;display:block;margin-bottom:3px;">At├®</label>
                ...
                <div>
                    ...
                    <select id="f-estado-civil" onchange="aplicarFiltrosColaboradores()" style="width:100%;padding:0.5rem;border:1px solid #e2e8f0;border-radius:6px;font-size:0.85rem;">
                        <option value="">Todos</option><option>Solteiro(a)</option><option>Casado(a)</option><option>Divorciado(a)</option><option>Vi├║vo(a)</option><option>Uni├úo Est├ível</option>
                    </select>
                </div>
                <div>
                ...
                <div>
                    <label style="font-size:0.75rem;font-weight:600;color:#64748b;display:block;margin-bottom:3px;">Em Experi├¬ncia</label>
                    <select id="f-experiencia" onchange="aplicarFiltrosColaboradores()" style="width:100%;padding:0.5rem;border:1px solid #e2e8f0;border-radius:6px;font-size:0.85rem;">
                        <option value="">Todos</option><option value="sim">Sim (at├® 90 dias)</option>
                    </select>
                </div>
                <div style="display:flex; gap:1rem;">
                    <div style="flex:1;">
                        <label style="font-size:0.75rem;font-weight:600;color:#64748b;display:block;margin-bottom:3px;">Sal├írio De</label>
                    ...
                    <div style="flex:1;">
                        <label style="font-size:0.75rem;font-weight:600;color:#64748b;display:block;margin-bottom:3px;">Sal├írio At├®</label>
                ...
                <div>
                    <label style="font-size:0.75rem;font-weight:600;color:#64748b;display:block;margin-bottom:3px;">Benef├¡cios</label>
                ...
                <div>
                    <label style="font-size:0.75rem;font-weight:600;color:#64748b;display:block;margin-bottom:3px;">Tamanho Cal├ºa</label>
                ...
                <div>
                    <label style="font-size:0.75rem;font-weight:600;color:#64748b;display:block;margin-bottom:3px;">Tamanho Cal├ºado</label>
    ...
    wrapper.innerHTML = `
        <table class="table" style="width:100%; border-collapse:collapse; min-width:800px;">
            <thead style="position:sticky; top:0; z-index:2; background:#f8fafc; outline:1px solid #e2e8f0;"><tr>
                    ...
                    <th style="cursor:pointer;white-space:nowrap;user-select:none;" onclick="colabToggleSort('admissao')">Admiss├úo ${getSortIcon('admissao')}</th>
                    <th>Status</th>
                    <th style="text-align:right;padding-right:1.5rem;">A├º├Áes</th>
                </tr></thead>
                <tbody>
                    ${lista.map(c => {
        ...
        if (c.data_admissao) {
            ...
            probationDatesHtml = `<div style="font-size:7pt;color:#94a3b8;line-height:1.1;margin-top:2px;">1┬║: ${d45.toLocaleDateString('pt-BR')}<br>2┬║: ${d90.toLocaleDateString('pt-BR')}</div>`;
        }
        let statusHtml = '';
        const effectiveStatus = getEffectiveStatus(c);
        if (effectiveStatus === 'Aguardando in├¡cio') statusHtml = `<div style="background:#f1f3f5;color:#495057;border:2px solid #adb5bd;border-radius:20px;font-weight:600;padding:2px 10px;display:inline-flex;align-items:center;gap:4px;font-size:0.75rem;"><i class="ph ph-clock"></i> Aguardando</div>`;
        ...
        else if (effectiveStatus === 'F├®rias') statusHtml = `<div style="background:#ffedd5;color:#c2410c;border:2px solid #c2410c;border-radius:20px;font-weight:600;padding:2px 10px;display:inline-flex;align-items:center;gap:4px;font-size:0.75rem;"><i class="ph ph-airplane-tilt"></i> F├®rias</div>`;
        ...
        if (c.data_admissao) {
            ...
            if (diffDays >= 0 && diffDays <= 90) {
                ...
                let tagHtml = diffDays <= 45
                    ? `<span class="probation-badge" style="font-size:0.6rem;padding:0.15rem 0.4rem;border-radius:4px;white-space:nowrap;">1┬║ 45</span>`
                    : `<span class="probation-badge second" style="font-size:0.6rem;padding:0.15rem 0.4rem;border-radius:4px;white-space:nowrap;">2┬║ 45</span>`;
                let vigenciaHtml = diffDays <= 45
                    ? `<span style="font-size:0.65rem;color:#64748b;white-space:nowrap;font-weight:600;">1┬║: ${d45.toLocaleDateString('pt-BR')}</span>`
                    : `<span style="font-size:0.65rem;color:#64748b;white-space:nowrap;font-weight:600;">2┬║: ${d90.toLocaleDateString('pt-BR')}</span>`;
        ...
        return `<tr>
                            ...
                            <td style="text-align:right;padding-right:1rem;">
                                <div style="display:flex;gap:0.4rem;justify-content:flex-end;">
                                    ...
                                    <button class="btn btn-primary btn-sm" onclick="openProntuario(${c.id},'${(c.nome_completo || '').replace(/'/g, "\\'")}','${(c.cargo || '').replace(/'/g, "\\'")}','${c.cpf || ''}','${c.sexo || ''}','${c.data_admissao || ''}','${c.status || ''}','${c.rg_tipo || 'RG'}')" title="Prontu├írio" style="padding:0.4rem;width:32px;height:32px;justify-content:center;background:#2563eb;"><i class="ph ph-folder-open"></i></button>
...
window.deleteColaborador = async function (id, isStatusIncompleto = false) {
    let msg = '­ƒÜ¿ ATEN├ç├âO: Tem certeza que deseja inativar este colaborador?\n\nO status dele(a) ser├í alterado para "Desligado" mantendo todos os arquivos intactos.';
    if (isStatusIncompleto) {
        msg = '­ƒÜ¿ ATEN├ç├âO: Este colaborador est├í INCOMPLETO. A exclus├úo ir├í DELETAR PERMANENTEMENTE todos os dados e eventuais arquivos j├í enviados. Deseja prosseguir?';
    }
...
// ÔöÇÔöÇ [DEV] Anonimizar colaborador individual ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
window.devAnonimizarColaborador = async function(id, nome) {
    if (!window._IS_DEV_HOMOLOG) return;
    // descobre o ├¡ndice desse colaborador na lista para gerar dados ├║nicos
...
// ÔöÇÔöÇ [DEV] Gera dados fict├¡cios para anonimiza├º├úo ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
function _devDadosFicticios(index) {
    const nomes = ['Jo├úo','Maria','Carlos','Ana','Pedro','Lucia','Rafael','Juliana','Marcos','Fernanda'];
    const sobrenomes = ['Silva','Santos','Oliveira','Souza','Lima','Pereira','Costa','Ferreira','Alves','Rodrigues'];
    const depts = ['Log├¡stica','Administrativo','Comercial','RH','Financeiro'];
    const cargos = ['Assistente','Analista','Auxiliar','Operador','T├®cnico'];
    ...
    return {
        ...
        tipo_contrato: 'CLT',
        endereco: `Rua Fict├¡cia, ${index + 1} - S├úo Paulo/SP`,
        nome_mae: 'Maria de Teste',
        nome_pai: 'Jos├® de Teste',
...
// ÔöÇÔöÇ [DEV] Anonimizar todos (exceto Teste de Sistema da Silva) ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
window.devAnonimizarTodosColaboradores = async function() {
    ...
    overlay.innerHTML = `
        <div style="background:#fff;border-radius:16px;padding:2rem 2.5rem;max-width:420px;width:90%;box-shadow:0 20px 60px rgba(0,0,0,0.3);text-align:center;">
            ...
            <h3 style="margin:0 0 0.4rem;font-size:1.1rem;color:#0f172a;">Substituir dados por fict├¡cios?</h3>
            <p style="margin:0 0 1.25rem;color:#64748b;font-size:0.85rem;">
                Os dados de <strong>${paraAnonimizar.length} colaboradores</strong> ser├úo substitu├¡dos por informa├º├Áes fict├¡cias para testes.<br>
                <span style="color:#059669;font-weight:600;">"Teste de Sistema da Silva" ser├í mantido intacto.</span>
            </p>
...
// ÔöÇÔöÇ fim [DEV] ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
...
window.resetFormColaborador = function () {
    ...
    // Reset F├®rias Fracionadas
    const ffNao = document.querySelector('input[name="ferias_fracionadas_check"][value="N├úo"]');
    if (ffNao) { ffNao.checked = true; }
    if (typeof window.toggleFeriasFracionadas === 'function') window.toggleFeriasFracionadas('N├úo');
    ...
    const radA = document.querySelector('input[name="alergia_check"][value="N├úo"]');
    if (radA) radA.checked = true;
    if (typeof window.toggleAlergias === 'function') window.toggleAlergias('N├úo');

    const radAdt = document.querySelector('input[name="adiantamento_check"][value="N├úo"]');
    if (radAdt) radAdt.checked = true;
    if (typeof window.toggleAdiantamento === 'function') window.toggleAdiantamento('N├úo');
    if (document.getElementById('colab-adiantamento-valor')) document.getElementById('colab-adiantamento-valor').value = '';

    const radioInN = document.querySelector('input[name="insalubridade_check"][value="N├úo"]');
    ...
    if (typeof window.toggleInsalubridade === 'function') window.toggleInsalubridade('N├úo');
    ...
    if (statusContainer) {
        updateStatusChip('Aguardando in├¡cio');
    }
    ...
    const radioFacNao = document.querySelector('input[name="faculdade_participa"][value="N├úo"]');
    if (radioFacNao) { radioFacNao.checked = true; toggleFormacaoFields('N├úo'); }

    const radioAcadNao = document.querySelector('input[name="academia_participa"][value="N├úo"]');
    if (radioAcadNao) { radioAcadNao.checked = true; toggleAcademiaFields('N├úo'); }

    const radioTeraNao = document.querySelector('input[name="terapia_participa"][value="N├úo"]');
    if (radioTeraNao) { radioTeraNao.checked = true; toggleTerapiaFields('N├úo'); }

    const radioCeluNao = document.querySelector('input[name="celular_participa"][value="N├úo"]');
    if (radioCeluNao) { radioCeluNao.checked = true; toggleCelularFields('N├úo'); }

    const radioChavesNao = document.querySelector('input[name="chaves_participa"][value="N├úo"]');
    if (radioChavesNao) { radioChavesNao.checked = true; toggleChavesColabFields('N├úo'); }
};

window.editColaborador = async function (id) {
    // Bot├úo de sincroniza├º├úo manual ocultado (a automa├º├úo j├í faz isso ao salvar)
    ...
    try {
        ...
        // Normalizar c├│digo: se tiver 6 d├¡gitos sem tra├ºo (ex: 342125), formatar como 3421-25
        ...
        if (document.getElementById('colab-valor-transporte')) {
            ...
            const inputT = document.getElementById('colab-valor-transporte');
            inputT.value = valT;
            // Se for VT e n├úo tiver valor salvo, calcular 6% do sal├írio
        ...
        if (document.getElementById('colab-escala-padrao')) {
            ...
            // Carregar data de refer├¬ncia do ciclo de domingos
        ...
        // toggleMotorista ANTES de carregar CNH ÔÇö se chamado depois limpa os campos
        ...
        // F├®rias Fracionadas
        const ffVal = c.ferias_fracionadas || 'N├úo';
        ...
        if (document.getElementById('colab-alergias')) {
            const hasAlergia = c.alergias && c.alergias.trim() !== '' ? 'Sim' : 'N├úo';
        ...
        // Adiantamento ÔÇö formatar valor como moeda ao carregar
        const adiVal = c.adiantamento_salarial || 'N├úo';
        ...
        // Insalubridade ÔÇö formatar valor como moeda ao carregar
        const insVal = c.insalubridade || 'N├úo';
        const radioInS = document.querySelector(`input[name="insalubridade_check"][value="${insVal === 'Sim' ? 'Sim' : 'N├úo'}"]`);
        ...
        // Faculdade fields
        const participa = c.faculdade_participa || 'N├úo';
        ...
        // Academia
        const participaAcad = c.academia_participa || 'N├úo';
        ...
        // Terapia
        const participaTera = c.terapia_participa || 'N├úo';
        ...
        // Celular
        const participaCelu = c.celular_participa || 'N├úo';
        ...
        // Chaves
        const participaChaves = c.chaves_participa || 'N├úo';
        ...
        // Brigadista
        const participaBrig = c.brigadista_participa || 'N├úo';
        ...
        // Popula o cache de f├®rias para checagem de sobreposi├º├úo no alerta CLT
        ...
        // Re-avaliar AP├ôS tudo preenchido ÔÇö l├¬ do campo do formul├írio que j├í foi setado corretamente
        ...
        if (c.estado_civil === 'Casado' || c.estado_civil === 'Uni├úo Est├ível') {
            toggleConjuge();
            // C├┤njuge agora salvo diretamente no colaborador (conjuge_nome / conjuge_cpf)
        ...
        if (admissionBar && admissionText && admissionBtn) {
            if (c.status === 'Aguardando in├¡cio' || c.status === 'Processo iniciado') {
                admissionBar.style.display = 'flex';
                admissionText.textContent = c.status;
                admissionBtn.innerHTML = '<i class="ph ph-arrow-right"></i> P├ígina Admiss├úo';
                ...
                admissionBar.style.display = 'flex';
                admissionText.textContent = 'Admiss├úo Conclu├¡da';
                admissionBtn.innerHTML = '<i class="ph ph-check-square"></i> Conclu├¡da';
        ...
        if (container) {
            container.innerHTML = '';
            // Filtra c├┤njuge para n├úo aparecer na lista de dependentes
            const children = (c.dependentes || []).filter(d => d.grau_parentesco !== 'C├┤njuge');
        ...
        // Failsafe: re-calcula per├¡odo de f├®rias ap├│s render completo
        setTimeout(() => {
        ...
        alert('Ocorreu um erro ao carregar os dados para edi├º├úo: ' + err.message);
    }
};
    ...
    formColab.addEventListener('submit', async (e) => {
        ...
        if (!isPartial) {
            ...
            if (cpfInput && cpfInput.value.replace(/\D/g, '').length < 11) {
                alert("CPF do Colaborador inv├ílido ou incompleto.");
                return;
            }
            // C├┤njuge e CNH: preenchimento opcional
        }
        ...
        const data = {
            ...
            dependentes: (() => {
                const results = [];
                // Incluir Filhos / Dependentes (C├┤njuge n├úo ├® mais dependente)
                ...
                // Incluir apenas filhos como dependentes (c├┤njuge ├® salvo separadamente)
                return results;
            })(),
            // C├┤njuge salvo como colunas diretas no colaborador
            ...
            faculdade_participa: document.querySelector('input[name="faculdade_participa"]:checked')?.value || 'N├úo',
            ...
            academia_participa: document.querySelector('input[name="academia_participa"]:checked')?.value || 'N├úo',
            ...
            terapia_participa: document.querySelector('input[name="terapia_participa"]:checked')?.value || 'N├úo',
            ...
            celular_participa: document.querySelector('input[name="celular_participa"]:checked')?.value || 'N├úo',
            celular_data: document.getElementById('colab-celular-data') ? document.getElementById('colab-celular-data').value : null,
            chaves_participa: document.querySelector('input[name="chaves_participa"]:checked')?.value || 'N├úo',
            ...
            ferias_fracionadas: document.querySelector('input[name="ferias_fracionadas_check"]:checked')?.value || 'N├úo',
            ...
            adiantamento_salarial: document.querySelector('input[name="adiantamento_check"]:checked')?.value || 'N├úo',
            ...
            insalubridade: document.querySelector('input[name="insalubridade_check"]:checked')?.value || 'N├úo',
        ...
        // Converter valores formatados (R$) para n├║meros antes de enviar
        const parseMoeda = (v) => {
        ...
        if (data.escala_tipo === 'escala_uma_folga' && !isPartial) {
            ...
            if (folgas.length !== 1) {
                alert('Aten├º├úo: Para a escala Folga 1 Dia na Semana, voc├¬ deve marcar *exatamente 1 dia* de folga na lista.');
                btnRestorer();
                return;
            }
            ...
            if (folgas.length !== 2) {
                alert('Aten├º├úo: Para o esquema 5x2 (Revezamento), voc├¬ deve marcar *exatamente 2 dias* de folga na lista.');
                btnRestorer();
                return;
            }
            data.escala_folgas = JSON.stringify(folgas);
        } else if (data.escala_tipo && data.escala_tipo !== 'escala_duas_folgas' && data.escala_tipo !== 'escala_uma_folga') {
            data.escala_folgas = JSON.stringify(['Dom']); // Padr├úo para as outras escalas
        }
        ...
        let c_status = statusInput ? statusInput.value : 'Aguardando in├¡cio';
        if (!id) {
            // Todos novos registros iniciam como Aguardando in├¡cio
            c_status = 'Aguardando in├¡cio';
        }
        data.status = c_status;

        // VALIDA├ç├âO FRONT-END (M├ìNIMO)
        if (!data.nome_completo || data.nome_completo.trim() === '') {
            alert('Preenchimento Obrigat├│rio: O campo "Nome Completo" n├úo pode ficar vazio.');
            btnRestorer();
            return;
        }

        if (!data.cpf || data.cpf.trim() === '') {
            alert('Preenchimento Obrigat├│rio: O campo "CPF" n├úo pode ficar vazio.');
            btnRestorer();
            return;
        }

        // Valida├º├úo de Motorista
        if (data.cargo && data.cargo.toUpperCase().includes('MOTORISTA')) {
            if (!isPartial) {
                ...
                if (cnhNumeroEl && cnhCatEl) {
                    // S├│ valida se os campos estiverem no DOM
                    if (!cnhNumeroVal || !cnhCatVal) {
                        alert('Preenchimento Obrigat├│rio: Dados da CNH (N├║mero e Categoria) para Motorista n├úo podem ficar vazios.');
                        btnRestorer();
                        return;
                    }
                    if (cnhNumeroVal.length < 11) {
                        alert('Preenchimento Obrigat├│rio: O n├║mero da CNH deve conter pelo menos 11 d├¡gitos.');
                        btnRestorer();
                        return;
                    }
                }
        ...
        try {
            ...
            if (colabId) {
                ...
                if (fotoInput && fotoInput.files && fotoInput.files[0]) {
                    ...
                    // Limpar input ap├│s upload para n├úo reenviar sem necessidade
                    fotoInput.value = '';
                }
                
                // Navega├º├úo silenciosa ÔÇö sem alertas de confirma├º├úo
            } else {
                // Colaborador salvo sem sync (novo colaborador)
            }

            // Fecha a aba do formul├írio e vai para a lista de colaboradores
...
window.openProntuario = async function (id, nome, cargo, cpf, sexo = '', admissao = '', status = '', rgTipo = 'RG') {
    ...
    // Vincular bot├úo IMEDIATAMENTE (antes de qualquer await)
    ...
    if (rgEl) rgEl.textContent = `${viewedColaborador.rg_tipo || 'RG'}: ${viewedColaborador.rg || 'N├úo informado'}`;
    ...
    if (statusDisplay) {
        ...
        let statusHtml = '';
        if (s === 'Aguardando in├¡cio') statusHtml = `<div style="background:#f1f3f5;color:#495057;border:2px solid #adb5bd;border-radius:20px;font-weight:600;padding:2px 10px;display:inline-flex;align-items:center;gap:4px;font-size:0.75rem;"><i class="ph ph-clock"></i> Aguardando</div>`;
        ...
        else if (s === 'F├®rias') statusHtml = `<div style="background:#ffedd5;color:#c2410c;border:2px solid #c2410c;border-radius:20px;font-weight:600;padding:2px 10px;display:inline-flex;align-items:center;gap:4px;font-size:0.75rem;"><i class="ph ph-airplane-tilt"></i> F├®rias</div>`;
    ...
    // Exibir aba C├┤njuge apenas para Casado ou Uni├úo Est├ível
    const tabConjuge = document.getElementById('tab-conjuge');
    if (tabConjuge) {
        tabConjuge.style.display = 'none'; // Aba de C├┤njuge extinta nativamente (migrado para Passo 4)
    }
...
window.uploadFotoProntuario = async function (input) {
    ...
    try {
        ...
        if (response.ok) {
            // Recarregar prontu├írio para atualizar foto
        ...
        console.error(err);
        alert('Erro na conex├úo ao enviar foto.');
    }
};
...
const FIXED_DOCS = {
    'Contratos': ['Contrato de Trabalho', 'Termo de Confidencialidade', 'Acordo Individual Benef├¡cios', 'Acordo Prorroga├º├úo e Compensa├º├úo', 'Declara├º├úo Vale Transporte', 'Contrato Experi├¬ncia 45 dias', 'Prorroga├º├úo de Contrato', 'Termo de Est├ígio'],
    'ASO': ['ASO Padr├úo'],
    'Ficha de EPI': ['Ficha de EPI Assinada'],
    'Multas': ['Contrato de Responsabilidade com o Ve├¡culo']
};
...
// ============================================================
// GERADOR DE OCORR├èNCIA - Renderiza painel na aba Ocorr├¬ncias (tabId: Advert├¬ncias)
// ============================================================
window.renderAdvertenciasTab = function (listContainer, filteredDocs) {
    ...
    geradorPanel.innerHTML = `
        <div class="card p-4 mb-4" style="background: #f8fafc; border: 1.5px solid #e2e8f0; border-radius: 12px;">
            <div style="display:flex; align-items:center; gap:0.75rem; margin-bottom:1.25rem;">
                ...
                <div>
                    <h4 style="margin:0; font-size:1rem; font-weight:700; color:#1e293b;">Gerar Ocorr├¬ncia</h4>
                    <p style="margin:0; font-size:0.8rem; color:#64748b;">Preencha os campos e gere o documento j├í com os dados do colaborador</p>
                </div>
            </div>

            <div style="display:grid; grid-template-columns:1.5fr 2fr 1fr; gap:1rem; margin-bottom:1rem;">
                <div>
                    <label style="font-size:0.75rem; font-weight:700; color:#475569; display:block; margin-bottom:4px;">Tipo de Advert├¬ncia</label>
                    <select id="adv-tipo" class="form-control" style="padding:0.5rem; border:1px solid #cbd5e1; border-radius:6px; font-size:0.9rem;">
                        ...
                        <option value="verbal">Advert├¬ncia Verbal</option>
                        <option value="escrita">Advert├¬ncia Escrita</option>
                        <option value="suspensao_1">Suspens├úo ÔÇö 1 dia</option>
                        <option value="suspensao_2">Suspens├úo ÔÇö 2 dias</option>
                        <option value="suspensao_3">Suspens├úo ÔÇö 3 dias</option>
                    </select>
                </div>
                <div>
                    <label style="font-size:0.75rem; font-weight:700; color:#475569; display:block; margin-bottom:4px;">T├¡tulo da Advert├¬ncia <span style="color:#94a3b8; font-weight:400;">(opcional)</span></label>
                    <input type="text" id="adv-titulo" class="form-control" placeholder="Ex: Desrespeito ├ás normas internas..." style="padding:0.5rem; border:1px solid #cbd5e1; border-radius:6px; font-size:0.9rem;">
                </div>
                <div>
                    <label style="font-size:0.75rem; font-weight:700; color:#475569; display:block; margin-bottom:4px;">Data Ocorr├¬ncia</label>
            ...
            <div style="margin-bottom:1rem;">
                <label style="font-size:0.75rem; font-weight:700; color:#475569; display:block; margin-bottom:4px;">Motivo / Descri├º├úo da Infra├º├úo <span style="color:#ef4444;">*</span></label>
                <textarea id="adv-motivo" rows="3" class="form-control" placeholder="Descreva o motivo da advert├¬ncia..." style="padding:0.5rem; border:1px solid #cbd5e1; border-radius:6px; font-size:0.9rem; resize:vertical; width:100%; box-sizing:border-box;"></textarea>
            </div>
            ...
            <!-- O painel de preview inline foi removido, pois agora abrir├í no modal -->
        </div>
        ...
        <div class="card p-3 mb-4 bg-light" style="display:flex; gap:1.5rem; align-items:center;">
            ...
            <select id="temporal_year_Advert_ncias" class="form-control" style="padding:0.4rem; max-width:120px;" onchange="renderTemporalAno('Advert├¬ncias')">
                ${optionsHtml}
            </select>
        </div>
    ...
    renderTemporalAno('Advert├¬ncias');
};
...
window.gerarAdvertencia = function () {
    ...
    if (!motivo) { alert('Por favor, descreva o motivo da advert├¬ncia.'); document.getElementById('adv-motivo').focus(); return; }

    const tipoMap = {
        verbal: 'ADVERT├èNCIA VERBAL',
        escrita: 'ADVERT├èNCIA ESCRITA',
        suspensao_1: 'SUSPENS├âO DISCIPLINAR ÔÇö 1 DIA',
        suspensao_2: 'SUSPENS├âO DISCIPLINAR ÔÇö 2 DIAS',
        suspensao_3: 'SUSPENS├âO DISCIPLINAR ÔÇö 3 DIAS',
        ocorrencia: 'OCORR├èNCIA'
    };
    const tipoTexto = tipoMap[tipo] || 'ADVERT├èNCIA';
    ...
    const suspensaoParag = isSuspensao ? `
        <p style="margin-top:1rem; text-align:justify;">
            Em decorr├¬ncia da gravidade da infra├º├úo cometida, o(a) colaborador(a) cumprir├í
            <strong>suspens├úo disciplinar de ${diasSuspensao} (${diasSuspensao === 1 ? 'um' : diasSuspensao === 2 ? 'dois' : 'tr├¬s'}) dia(s)</strong>,
            sem remunera├º├úo, a contar da data da ci├¬ncia deste documento.
        </p>` : '';
    ...
    const htmlDoc = `
        <p style="margin-top:1.5rem; text-align:justify;">
            A empresa <strong>AMERICA RENTAL EQUIPAMENTOS LTDA</strong>, inscrita no CNPJ sob o n┬║ 03.434.448/0001-01,
            situada na Rua Salto da Divisa, n┬║ 97, CEP 07252-300, Parque Alvorada ÔÇô Guarulhos/SP,
            ${isOcorrencia
            ? `vem por meio deste documento <strong>registrar a seguinte ocorr├¬ncia</strong> envolvendo o(a) colaborador(a) <strong>${nomeColab}</strong>.`
            : `vem por meio deste documento aplicar ao(├á) colaborador(a) <strong>${nomeColab}</strong> a presente <strong>${tipoTexto}</strong>.`
        }
        </p>
        ...
        <p style="margin-top:1rem; text-align:justify;">
            <strong>${isOcorrencia ? 'Descri├º├úo da ocorr├¬ncia:' : 'Motivo / Infra├º├úo cometida:'}</strong><br>
            ${motivo.replace(/\n/g, '<br>')}
        </p>
        ${suspensaoParag}
        ${!isOcorrencia ? `
        <p style="margin-top:1rem; text-align:justify;">
            Informamos que esta ├® <strong>uma medida disciplinar</strong> e que reincid├¬ncias poder├úo acarretar
            penalidades mais severas, inclusive a rescis├úo do contrato de trabalho por justa causa,
            nos termos do artigo 482 da Consolida├º├úo das Leis do Trabalho (CLT).
        </p>

        <p style="margin-top:1rem; text-align:justify;">
            O(A) colaborador(a) declara, com sua assinatura, estar ciente do conte├║do desta advert├¬ncia
            e de que a mesma ser├í arquivada em seu prontu├írio.
        </p>` : `
        <p style="margin-top:1rem; text-align:justify; color:#475569; font-size:0.92em;">
            Este registro ├® de car├íter informativo e ser├í arquivado no prontu├írio do colaborador.
        </p>`}
    `;

    // Montar dados do colaborador para o padr├úo do preview
    const tipoSimples = {
        verbal: 'Advert├¬ncia Verbal',
        escrita: 'Advert├¬ncia Escrita',
        suspensao_1: 'Suspens├úo 1 dia',
        suspensao_2: 'Suspens├úo 2 dias',
        suspensao_3: 'Suspens├úo 3 dias',
...
// ÔöÇÔöÇÔöÇ Template ├║nico para Preview e PDF de Advert├¬ncias / Suspens├Áes / Ocorr├¬ncias ÔöÇÔöÇÔöÇ
function buildAdvertenciaTemplate(data, logoSrc) {
    const isOcorrencia = data.isOcorrencia;

    // Se├º├úo de assinaturas (n├úo exibida para ocorr├¬ncias)
    ...
    return `<style>html,body{margin:0!important;padding:0!important;background:#fff!important;}*{box-sizing:border-box;}</style><div style="width:794px; background:#fff; font-family:Arial,Helvetica,sans-serif; font-size:12px; color:#111; line-height:1.5; display:block;">
        ...
        <!-- CONTE├ÜDO COM MARGENS LATERAIS -->
        <div style="padding:20px 40px 40px 40px;">
            ...
            <!-- ESPA├çO RESERVADO PARA ASSINATURAS (desenhadas pelo pdf-lib ap├│s coleta) -->
...
window.abrirPreviewAdvertencia = function (data) {
    ...
    // Voltar para o modal centralizado tradicional onde os bot├Áes ficam no topo
    ...
    if (btnsContainer) {
        btnsContainer.innerHTML = `
            <button onclick="window.anexarAdvertenciaAoProntuario()" id="btn-anexar-adv" class="btn btn-primary" style="background:#2f9e44; border-color:#2b8a3e; align-items:center; gap:5px;">
                <i class="ph ph-paperclip"></i> Anexar ao Prontu├írio
            </button>
...
window.anexarAdvertenciaAoProntuario = async function () {
    ...
    if (typeof html2pdf === 'undefined') {
        alert('A biblioteca de PDF ainda n├úo foi carregada. Tente imprimir como PDF nativo.');
        return;
    }
    ...
    try {
        ...
        // ÔöÇÔöÇ Capturar direto do preview que j├í est├í renderizado corretamente na tela ÔöÇÔöÇ
        const previewEl = document.getElementById('preview-doc-body');
        if (!previewEl) throw new Error('Preview n├úo encontrado. Abra o documento antes de anexar.');

        // Remover temporariamente sombra/borda/min-height para PDF mais limpo e sem p├ígina em branco extra
        ...
        previewEl.style.minHeight = 'auto'; // evita p├ígina em branco extra
        ...
        // Converter canvas ÔåÆ PDF A4 com suporte a m├║ltiplas p├íginas
        ...
        formData.append('tab_name', 'Advert├¬ncias');
        // document_type = 'Titulo###TipoSimples' para exibir badge no prontu├írio
        ...
        // O servidor pode retornar 5xx se o OneDrive falhar, mas o doc J├ü foi salvo no banco.
        // S├│ consideramos falha real se n├úo houver resposta ou status 400.
        const resData = await response.json().catch(() => ({}));
        if (response.status === 400) throw new Error(resData.error || 'Arquivo n├úo recebido pelo servidor.');

        // Ocorr├¬ncia: sem assinatura e sem OneDrive ÔÇö apenas salvo no prontu├írio local
        // (Advert├¬ncia Verbal ÔåÆ OneDrive ocorre ap├│s testemunhas; Escrita/Suspens├úo ÔåÆ ap├│s testemunhas + colaborador)
        ...
        // Recarregar documentos e re-renderizar a aba Advert├¬ncias
        try {
            const docs = await apiGet(`/colaboradores/${viewedColaborador.id}/documentos`);
            if (docs) {
                currentDocs = docs;
                ...
                if (activeTab) {
                    ...
                    renderTabContent('Advert├¬ncias', 'Ocorr├¬ncias', true);
                }
            }
        } catch (refreshErr) {
            console.warn('Aviso: n├úo foi poss├¡vel atualizar a lista automaticamente.', refreshErr);
        }
...
window.renderTabContent = function (tabId, tabTitle, preventScroll = false) {
    ...
    // Focar no final do input se houver texto para n├úo perder o cursor na re-renderiza├º├úo
    ...
    // Filtragem e Ordena├º├úo dos dados
    ...
    if (tabId === 'Multas') {
        // Aba de multas ÔÇö renderiza a UI customizada de gest├úo de multas
        if (typeof window.renderMultasMotoristaTab === 'function') {
            window.renderMultasMotoristaTab(listContainer);
        } else {
            listContainer.innerHTML = '<div class="alert alert-warning"><i class="ph ph-warning"></i> M├│dulo de multas n├úo carregado. Tente recarregar a p├ígina.</div>';
        }
    } else if (tabId === 'Sinistros') {
        if (typeof window.renderSinistrosTab === 'function') {
            window.renderSinistrosTab(listContainer);
        } else {
            listContainer.innerHTML = '<div class="alert alert-warning"><i class="ph ph-warning"></i> M├│dulo de sinistros n├úo carregado. Tente recarregar a p├ígina.</div>';
        }
        ...
        // Usa a MESMA lista ordenada do Passo 3 da Admiss├úo para garantir espelhamento
        ...
        fixed.push('Carteira de vacina├º├úo', 'Curr├¡culo', 'Carteira de Trabalho', 'Contrato e-social');
        if (_isCasado) {
            fixed.push('CPF do C├┤njuge');
        }
        ...
        filteredDocs.filter(d => !fixed.includes(d.document_type) && d.document_type !== 'Pens├úo Aliment├¡cia').forEach(d => {
            listContainer.appendChild(createDocSlot(tabId, d.document_type, d));
        });

        // === PENS├âO ALIMENT├ìCIA (Condicional - Prontu├írio) ===
        const pensaoDocPront = filteredDocs.find(d => d.document_type === 'Pens├úo Aliment├¡cia');
        ...
        pensaoWrapperPront.innerHTML = `
            <div style="display:flex; align-items:center; justify-content:space-between; gap:1rem; flex-wrap:wrap;">
                <div style="display:flex; align-items:center; gap:8px;">
                    ...
                    <span style="font-weight:600; font-size:0.9rem; color:#334155;">Possui documento de Pens├úo Aliment├¡cia?</span>
                </div>
                <div style="display:flex; gap:6px;">
                    ...
                    <button type="button" id="pensao-nao-pront" onclick="window.setPensaoPront('N├úo')" style="padding:0.35rem 1rem; border-radius:6px; border:1.5px solid ${!temPensaoPront ? '#10b981' : '#e2e8f0'}; background:${!temPensaoPront ? '#ecfdf5' : '#fff'}; color:${!temPensaoPront ? '#065f46' : '#334155'}; font-weight:700; cursor:pointer; font-size:0.85rem;">N├úo</button>
                </div>
            </div>
        ...
        if (temPensaoPront) {
            ...
            if (slotPront) slotPront.appendChild(createDocSlot(tabId, 'Pens├úo Aliment├¡cia', pensaoDocPront || null));
        }
        window.setPensaoPront = function (resposta) {
            ...
            if (slot) {
                ...
                if (isSimNow && slot.children.length === 0) {
                    slot.appendChild(createDocSlot(tabId, 'Pens├úo Aliment├¡cia', null));
                }
            }
        ...
        if (!participa) {
            listContainer.innerHTML = '<div class="alert alert-info"><i class="ph ph-info"></i> Esta aba est├í dispon├¡vel apenas para colaboradores que participam da Terapia em Grupo.</div>';
            return;
        }
        ...
        if (tabId === 'Conjuge') {
            ...
            if (!isCasado) {
                listContainer.innerHTML = '<div class="alert alert-info"><i class="ph ph-info"></i> Esta aba est├í dispon├¡vel apenas para colaboradores com estado civil <strong>"Casado(a)"</strong> registrado.</div>';
                return;
            }
        }
        if (tabId === 'Dependentes') {
            const deps = (viewedColaborador.dependentes || []).filter(d => d.grau_parentesco !== 'C├┤njuge');
            if (!deps.length) {
                listContainer.innerHTML = '<div class="alert alert-info"><i class="ph ph-info"></i> Esta aba est├í dispon├¡vel apenas para colaboradores que tenham dependentes cadastrados no sistema.</div>';
                return;
            }
            ...
            deps.forEach((dep, idx) => {
                ...
                // Documentos cond├¡cionais por faixa et├íria
                const docsConfig = [
                    { label: 'CPF ou RG', show: true },
                    { label: 'Caderneta de Vacina├º├úo', show: idade !== null && idade < 7 },
                    { label: 'Atestado de Frequ├¬ncia Escolar', show: idade !== null && idade >= 7 && idade <= 17 },
                    { label: 'Certid├úo de Nascimento', show: true },
                ];
        ...
        if (tabId === 'Faculdade') {
            ...
            if (!participa) {
                listContainer.innerHTML = '<div class="alert alert-info"><i class="ph ph-info"></i> Esta aba est├í dispon├¡vel apenas para colaboradores que participam do programa FormaA├º├úo.</div>';
                return;
            }
            renderFaculdadeTab(listContainer, tabId);
            return;
        }
        const btnLabelMap = { 'Dependentes': 'Documento de Dependente', 'Treinamento': 'Certificado/Curso', 'Conjuge': 'Documento do C├┤njuge', 'NRs': 'Certificado NR' };
        ...
        if (tabId === 'Multas') {
            ...
            if (typeof window.renderMultasMotoristaTab === 'function') {
                ...
                listContainer.innerHTML = '<div class="alert alert-info">Carregando m├│dulo de multas...</div>';
            }
            return;
        }
        ...
        if (tabId === 'Sinistros') {
            if (typeof window.renderSinistrosTab === 'function') {
                ...
                listContainer.innerHTML = '<div class="alert alert-info">Carregando m├│dulo de sinistros...</div>';
            }
            return;
        }
        FIXED_DOCS[tabId].forEach(docType => {
            if (!searchTerm || docType.toLowerCase().includes(searchTerm)) {
                if (tabId === 'Contratos' && docType === 'Acordo de aux├¡lio combust├¡vel') {
                    ...
                    if (meio === 'vale transporte') {
                        const existingDoc = filteredDocs.find(d => d.document_type === docType);
                        const msg = 'N├úo aplic├ível para usu├írios de Vale Transporte.';
                ...
                if (tabId === 'Contratos' && docType === 'Contrato faculdade') {
                    const participa = (viewedColaborador && viewedColaborador.faculdade_participa) ? viewedColaborador.faculdade_participa : 'N├úo';
                    if (participa === 'N├úo') {
                        const existingDoc = filteredDocs.find(d => d.document_type === docType);
                        const msg = 'N├úo aplic├ível para colaboradores que n├úo participam do programa FormaA├º├úo.';
                        listContainer.appendChild(createDocSlot(tabId, docType, existingDoc, null, null, msg));
                        return;
                    }
                }
            ...
            if (nr1Doc) {
                ...
                nr1Wrapper.innerHTML = `
                    <div style="display:flex; align-items:center; gap:0.6rem; flex:1;">
                        ...
                        <div>
                            <span style="font-weight:600; color:#334155; font-size:0.9rem;">NR1 / Ordem de Servi├ºo</span>
                            <div style="font-size:0.75rem; color:#16a34a; margin-top:1px;">Documento exigido no credenciamento ÔÇö fa├ºa o upload do PDF</div>
                        </div>
                    </div>
        ...
        filteredDocs.filter(d => {
            ...
            // J├í mostrou NR1 no slot fixo ÔÇö n├úo duplicar
    ...
    try {
        ...
        if (!cargoAtual) {
            container.innerHTML = `
                <div class="alert alert-warning">
                    <i class="ph ph-warning"></i> Cargo "${viewedColaborador.cargo || 'N├úo Definido'}" n├úo encontrado nas configura├º├Áes de cargos.
                </div>
            `;
            return;
        }
        ...
        if (!docsExigidos || docsExigidos.length === 0) {
            container.innerHTML = `
                <div class="alert alert-info">
                    <i class="ph ph-info"></i> Nenhuma documenta├º├úo espec├¡fica configurada para o cargo <strong>${cargoAtual.nome}</strong>.
                </div>
            `;
            return;
        }
        ...
        container.innerHTML = `
            <div style="margin-bottom: 2rem; padding: 1rem; background: #fffcf0; border: 1px solid #ffeeba; border-radius: 8px;">
                <h4 style="color: #856404; margin-bottom: 0.5rem;"><i class="ph ph-briefcase"></i> Documenta├º├úo Exigida: ${cargoAtual.nome}</h4>
                <p style="font-size: 0.85rem; color: #856404;">Anexe abaixo os documentos que foram selecionados como obrigat├│rios no gerenciamento de cargos.</p>
            </div>
        `;
...
async function renderFaculdadeSummary(container) {
    ...
    const cursoNome = cursoObj ? cursoObj.nome_curso : 'N├úo selecionado';
    ...
    summaryDiv.innerHTML = `
        <h4 style="color: #0369a1; margin-bottom: 0.5rem;"><i class="ph ph-graduation-cap"></i> Detalhes da Gradua├º├úo</h4>
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; font-size: 0.9rem; color: #0c4a6e;">
            ...
            <div><strong>Institui├º├úo:</strong> ${instituicao}</div>
            <div><strong>In├¡cio:</strong> ${inicio}</div>
            <div><strong>Previs├úo T├®rmino:</strong> ${termino}</div>
...
window.renderFaculdadeCompetencia = function () {
    ...
    // Lista de documentos por compet├¬ncia
    ...
    // Outros documentos din├ómicos para este m├¬s
...
function createDocSlot(tabId, docType, existingDoc, year = null, month = null, blockReason = null) {
    ...
    try {
        if (rawFileName.includes('├â')) {
            rawFileName = decodeURIComponent(escape(rawFileName));
        }
    } catch (e) { }
    const displayFileName = rawFileName.length > 40 ? rawFileName.substring(0, 40) + 'ÔÇª' : rawFileName;
    ...
    if (isSaved && existingDoc.assinafy_sent_at && existingDoc.assinafy_status !== 'Erro') {
        ...
        // Link de assinatura em linha pr├│pria abaixo do Enviado
        ...
        if (existingDoc.assinafy_signed_at) {
            ...
            enviadoHtml += `<br><span style="display:inline-flex;align-items:center;gap:4px;background:#dbeafe;color:#1d4ed8;border-radius:8px;padding:2px 8px;font-weight:700;font-size:0.78rem;margin-top:2px;">
              ...
              Assinado: ${sdd}/${smm}/${syyyy} ├ás ${sh}h${smin}m
            </span>`;
        }
    }
    ...
    if (isSaved) {
        if (existingDoc.atestado_tipo) {
            if (existingDoc.atestado_tipo === 'dias') {
                ...
                atestadoInfoHtml = ` <span style="color:${corText}; font-weight:600;"><i class="ph ph-warning" style="font-size:0.9em; color:${corText}; margin-right:2px;"></i> ${ini} at├® ${fim}</span> `;
            } else {
                atestadoInfoHtml = ` <span style="color:#1098ad; font-weight:600;"><i class="ph ph-clock"></i> ${existingDoc.atestado_inicio} ├ás ${existingDoc.atestado_fim}</span> `;
            }
        }
    }
    ...
    // Suporte ao separador ### para Advert├¬ncias: 'T├¡tulo###TipoSimples'
    ...
    const needsVencimentoList = ['ASO', 'CNH', 'Exames Complementares', 'Comprovante de endere├ºo'];
    ...
    if (needsVencimento) {
        ...
        if (!existingVencimento && docType === 'Comprovante de endere├ºo') {
            const d = new Date();
    ...
    // Status Assinafy: apenas bot├úo de baixar quando assinado
    let assStatusIcon = '';
    ...
    if (isSaved) {
        ...
        // Se o existingDoc ├® da tabela documentos mas tem um ass relacionado
        const assId = existingDoc.admissao_ass_id || existingDoc.id;
        if (st === 'Assinado') {
            // Tenta usar o assId da tabela admissao_assinaturas se dispon├¡vel
            // caso contr├írio usa o id do pr├│prio documento
        ...
        <div class="doc-actions" style="display: flex; align-items: flex-end; gap: 0.5rem;">
            ...
            ` : (tabId === 'Advert├¬ncias') ? `
                <div style="display: flex; gap: 0.5rem; align-items: center; flex-wrap: wrap; justify-content: flex-end; flex: 1;">
                    ...
                    ${(isSaved && !(tabId === 'Advert├¬ncias' && ['Assinado', 'Testemunhas'].includes(stMain) && (docType && (docType.toLowerCase().includes('suspens') || docType.toLowerCase().includes('advert'))))) ? `
                        <button type="button" class="btn btn-secondary" onclick="viewDoc(${existingDoc.id})" title="Visualizar" style="height: 42px;"><i class="ph ph-eye"></i></button>
                    ` : ''}

                    ${(tabId === 'Advert├¬ncias' && isSaved) ? (() => {
                ...
                if (_isOcorrDoc) return ''; // Ocorr├¬ncia n├úo tem assinatura
                return `
                    ...
                    ${(tabId === 'Advert├¬ncias' && isSaved && ['Assinado', 'Testemunhas'].includes(stMain) && (docType && (docType.toLowerCase().includes('suspens') || docType.toLowerCase().includes('advert')))) ? `
                ...
                <div style="display: flex; flex-direction: column; gap: 0.5rem;">
                    <div style="display: flex; gap: 0.5rem; align-items: flex-end;">
                        ...
                        <div style="display:flex; align-items:center; gap:8px; font-size:0.82rem; white-space:nowrap;">
                            ...
                            <label style="display:flex;align-items:center;gap:3px;cursor:pointer;margin:0;font-weight:500;">
                                <input type="radio" name="exig-assin-${(docType || '').replace(/[^a-zA-Z0-9]/g, '_')}" value="NAO_EXIGE" ${stMain === 'NAO_EXIGE' ? 'checked' : ''}> N├úo
                            </label>
                        </div>
                        ` : ''}
                        ...
                        <label class="btn ${isSaved ? 'btn-warning' : 'btn-primary'}" title="${isSaved ? 'Substituir' : 'Fazer Upload'}" style="height: 42px; display: flex; align-items: center;">
                            ...
                            <input type="file" accept=".pdf" style="display:none;" onchange="
                                ...
                                if((${needsVencimento}) && !venc) { alert('Data de vencimento ├® obrigat├│ria'); this.value=''; return; } 
                                let assStatus = null;
...
function createDynamicUploadForm(tabId, btnLabel, defaultDocType = '') {
    const showVencimento = tabId !== 'Advert├¬ncias';
    const div = document.createElement('div');
        ...
        <div style="display: flex; gap: 1rem; flex-wrap: wrap; align-items: flex-end;">
            ...
            <div style="flex: 1; min-width: 140px;">
                ...
                <div style="display:flex; gap:0.5rem; height: 38px; align-items:center; font-size: 0.82rem; font-weight: 500;">
                    ...
                    <label style="margin:0; display:flex; align-items:center; gap:4px; cursor:pointer;"><input type="radio" name="dyn-assin-${tabId}" value="NAO_EXIGE"> N├úo</label>
                </div>
            </div>
            ` : ''}
            <div>
...
window.renderTerapiaCompetencia = function () {
    ...
    const form = createDynamicUploadForm('Terapia', 'Adicionar Sess├úo/Relat├│rio', '');
    ...
    const selectorHtml = `
        ...
        <div class="card p-3 mb-4" style="background:#f8fafc; border:1.5px dashed #e2e8f0; border-radius:12px;">
            <h4 style="font-size:0.9rem; color:#64748b; margin-bottom:0.75rem; font-weight:600;">
                <i class="ph ph-envelope-simple"></i> Enviar Solicita├º├úo de Exame ├á IACI
            </h4>
            ${noticeHtml}
            <div style="display:flex; gap:0.75rem; align-items:flex-end; flex-wrap:wrap;">
                ...
                <div class="input-group" style="width:180px; flex-shrink:0; margin-bottom:0;">
                    ...
                    <select id="aso-tipo-exame-tab" class="form-control" style="padding:0.5rem; font-size:0.85rem; height:38px;" onchange="document.getElementById('aso-nova-funcao-container').style.display = this.value === 'Troca de Fun├º├úo' ? 'block' : 'none';">
                        <option value="Admissional">Admissional</option>
                        <option value="Demissional">Demissional</option>
                        <option value="Retorno ao trabalho">Retorno ao trabalho</option>
                        <option value="Peri├│dico">Peri├│dico</option>
                        <option value="Troca de Fun├º├úo">Troca de Fun├º├úo</option>
                    </select>
                </div>
                <div class="input-group" id="aso-nova-funcao-container" style="display:none; width:180px; flex-shrink:0; margin-bottom:0;">
                    <label style="font-size:0.75rem; font-weight:700;">Nova Fun├º├úo</label>
                ...
                <div class="input-group" style="flex:1; min-width:200px; margin-bottom:0;">
                    <label style="font-size:0.75rem; font-weight:700;">Destinat├írio</label>
                ...
                <button class="btn btn-primary" id="btn-enviar-aso-email-tab"
                    ...
                    <i class="ph ph-paper-plane-tilt"></i> Enviar Solicita├º├úo
                </button>
            </div>
        </div>
...
// Fun├º├úo espec├¡fica para envio pela aba ASO (n├úo conflita com a de Admiss├úo)
window.sendASOEmailTab = async function () {
    if (!viewedColaborador) { alert('Colaborador n├úo selecionado.'); return; }
    ...
    if (tipoExame === 'Troca de Fun├º├úo' && !novaFuncao) { alert('Preencha a nova fun├º├úo.'); return; }
    ...
    const tipoComExamesCompl = ['admissional', 'peri├│dico', 'periodico', 'peri├│dico'];
    const examesCompl = isMotorista && tipoComExamesCompl.some(t => tipoExameUpper.includes(t))
        ...
        : '';
    const exames = examesCompl ? `Exame Padr├úo\nExames Complementares: ${examesCompl}` : 'Exame Padr├úo';

    const novaFuncaoText = (tipoExame === 'Troca de Fun├º├úo' && novaFuncao) ? `\nNova Fun├º├úo: ${novaFuncao}` : '';
    const mailBody = `T├¡tulo: Exame M├®dico\n\nSegue abaixo as informa├º├Áes para a realiza├º├úo do exame do colaborador.\n\nData: ${dt}\nNome: ${viewedColaborador.nome_completo || viewedColaborador.nome}\nCPF: ${viewedColaborador.cpf || '-'}\nFun├º├úo Atual: ${viewedColaborador.cargo || '-'}${novaFuncaoText}\nDepartamento: ${viewedColaborador.departamento || '-'}\n\nExames:\n${exames}\n\nÔÜá´©Å IMPORTANTE:\nAp├│s o exame ficar pronto, favor enviar o documento por e-mail para: rh@americarental.com.br`;
    ...
    try {
        ...
        if (res.sucesso) {
            alert('Ô£à E-mail enviado com sucesso para a IACI!');
        ...
        if (confirm(`N├úo foi poss├¡vel enviar automaticamente. Erro do Servidor:\n\n${e.message}\n\nDeseja abrir seu e-mail com o texto preenchido?`)) {
            window.location.href = `mailto:${destinatario}?cc=rh@americarental.com.br,rh2@americarental.com.br&subject=Exame M├®dico - ${viewedColaborador.nome_completo || viewedColaborador.nome}&body=${encodeURIComponent(mailBody)}`;
        }
    } finally {
        if (btn) { btn.disabled = false; btn.innerHTML = '<i class="ph ph-paper-plane-tilt"></i> Enviar Solicita├º├úo'; }
    }
};
...
window.renderASOAno = function () {
    ...
    // Usar os documentos j├í filtrados pela barra global
    ...
    // Outros documentos din├ómicos j├í salvos para este ano (considerando o filtro de busca)
    ...
    // Bot├úo para adicionar outro exame av├╝lso
...
async function renderFaltasTab(container) {
    ...
    const turnoColor = { 'Dia todo': '#e03131', 'Manh├ú': '#f08c00', 'Tarde': '#1971c2' };
            ...
            <tr style="border-bottom:1px solid #f1f5f9;">
                ...
                <td style="padding:0.65rem 0.75rem; text-align:center;">
                    <span style="background:${f.avisado_previamente === 'Sim' ? '#dcfce7' : '#fee2e2'}; color:${f.avisado_previamente === 'Sim' ? '#166534' : '#991b1b'}; padding:2px 10px; border-radius:10px; font-size:0.75rem; font-weight:700;">
                        ${f.avisado_previamente === 'Sim' ? 'Ô£ô Sim' : 'Ô£ù N├úo'}
                    </span>
                </td>
                <td style="padding:0.65rem 0.75rem; color:#475569; font-size:0.88rem;">${f.observacao || 'ÔÇö'}</td>
            ...
            <div style="display:flex; gap:0.75rem; flex-wrap:wrap; align-items:flex-end;">
                ...
                <div style="display:flex; flex-direction:column; gap:0.25rem;">
                    ...
                    <div style="display:flex; gap:0.75rem; height:38px; align-items:center; font-size:0.88rem;">
                        ...
                        <label style="display:flex; align-items:center; gap:4px; cursor:pointer; margin:0; font-weight:500;">
                            <input type="radio" name="falta-avisado" value="N├úo"> N├úo
                        </label>
                    </div>
                </div>
                <div style="display:flex; flex-direction:column; gap:0.25rem; flex:1; min-width:180px;">
                    <label style="font-size:0.8rem; font-weight:600; color:#475569;">Observa├º├úo (opcional)</label>
                    <input type="text" id="falta-obs" placeholder="Ex: n├úo comunicou, sem justificativa..." style="height:38px; padding:0 0.6rem; border:1px solid #cbd5e1; border-radius:6px; font-size:0.9rem;">
                </div>
            ...
            <table style="width:100%; border-collapse:collapse; font-size:0.9rem;">
                <thead>
                    <tr style="background:#f8fafc; text-align:left;">
                        ...
                        <th style="padding:0.6rem 0.75rem; color:#64748b; font-size:0.78rem; text-transform:uppercase;">Observa├º├úo</th>
...
window.registrarFalta = async function () {
    ...
    const avisado = document.querySelector('input[name="falta-avisado"]:checked')?.value || 'N├úo';
    ...
    overlay.innerHTML = `
        <div style="background:#fff;border-radius:12px;width:90%;max-width:550px;box-shadow:0 10px 25px rgba(0,0,0,0.1);display:flex;flex-direction:column;overflow:visible;">
            ...
            <div style="padding:1.5rem;display:flex;flex-direction:column;gap:1.25rem;overflow:visible;">
                ...
                <div style="display:flex;gap:1rem;">
                    <div style="flex:1;">
                        <label style="font-size:0.8rem;font-weight:600;color:#334155;margin-bottom:0.25rem;display:block;">Data In├¡cio</label>
                    ...
                    <div style="flex:1;">
                        <label style="font-size:0.8rem;font-weight:600;color:#334155;margin-bottom:0.25rem;display:block;">T├®rmino</label>
    ...
    try {
        ...
        dd.innerHTML = data.map((c, i) =>
            `<div style="padding:0.6rem 0.8rem;cursor:pointer;font-size:0.85rem;border-bottom:1px solid #f1f5f9;" onclick="window.selectModalCID('${c.code}', this.dataset.desc)" data-desc="${c.desc.replace(/"/g, '&quot;')}">
                <strong style="color:#1a73e8;">${c.code}</strong> ÔÇö ${c.desc}
             </div>`
        ).join('');
...
window.selectModalCID = function (code, desc) {
    ...
    document.getElementById('modal_cid_search').value = `${code} ÔÇö ${desc}`;
};
...
window.uploadModalAtestado = async function () {
    ...
    try {
        ...
        if (res.ok) {
            ...
            // Atualiza status se o atestado for v├ílido para hoje
...
window.renderAtestadosTab = function (container, filteredDocs) {
    ...
    // Injetar CSS do autocomplete se n├úo existir
            ...
            <div class="card p-3 mb-4 bg-light" style="overflow: visible;">
                <div style="display:flex; gap:0.75rem; align-items:flex-end; flex-wrap:nowrap; width:100%;">
                    ...
                    <div id="titulo-field-wrap" style="flex:2; min-width:150px; display:none;">
                        <label style="font-size:0.75rem; font-weight:600; color:#2c5282; margin-bottom:3px; display:block;"><i class="ph ph-text-t"></i> T├¡tulo do Atestado</label>
                        <input type="text" id="atestado-titulo-horas" class="form-control" placeholder="Ex: Consulta m├®dica, Acompanhamento..." style="padding:.4rem;">
                    </div>
                    ...
                    <div id="atestado-dias-fields" style="display:flex; gap:1rem; flex-shrink:0; align-items:flex-end;">
                        <div>
                            <label style="font-size:0.75rem; font-weight:600; color:#2c5282; margin-bottom:3px; display:block;">Data In├¡cio</label>
                        ...
                        <div>
                            <label style="font-size:0.75rem; font-weight:600; color:#94a3b8; margin-bottom:3px; display:block;">T├®rmino (calc.)</label>
                    ...
                    <div id="atestado-horas-fields" style="display:none; gap:1rem; flex-shrink:0; align-items:flex-end;">
                        ...
                        <div>
                            <label style="font-size:0.75rem; font-weight:600; color:#2c5282; margin-bottom:3px; display:block;">Hor├írio In├¡cio</label>
                            <input type="time" id="atestado_inicio_hora" class="form-control" style="padding:0.4rem; width:110px;">
                        </div>
                        <div>
                            <label style="font-size:0.75rem; font-weight:600; color:#2c5282; margin-bottom:3px; display:block;">Hor├írio Fim</label>
    ...
    try {
        ...
        dd.innerHTML = data.map((c, i) =>
            `<div class="cid-option" data-code="${c.code}" data-desc="${c.desc.replace(/"/g, '&quot;')}" onclick="window.selectCID('${c.code}', this.dataset.desc)">
                <strong>${c.code}</strong> ÔÇö ${c.desc}
             </div>`
        ).join('');
        dd.style.display = 'block';
    } catch (e) { 
...
window.selectCID = function (code, desc) {
    ...
    document.getElementById('cid-search').value = `${code} ÔÇö ${desc}`;
...
window.triggerAtestadoUpload = function () {
    ...
    if (tipo === 'horas') {
        ...
        if (!titulo) {
            alert('Digite o t├¡tulo do atestado de horas!');
        ...
        // CID ├® opcional: aceita c├│digo v├ílido OU t├¡tulo livre digitado no campo
        const cidText = document.getElementById('cid-search')?.value?.trim();
        if (!selectedCID && !cidText) {
            alert('Digite um CID ou um t├¡tulo para o atestado antes de anexar o arquivo.');
...
window.uploadAtestadoWithCID = async function (inputEl) {
    ...
    // Gerar nome no padr├úo Z01_DD-MM-AA_NomeColab
    const today = new Date();
    const dd = String(today.getDate()).padStart(2, '0');
    const mm = String(today.getMonth() + 1).padStart(2, '0');
    const aa = String(today.getFullYear()).slice(2);
    ...
    if (tipo === 'horas') {
        ...
        // CID v├ílido selecionado OU texto livre digitado no campo
    ...
    if (tipo === 'dias') {
        ...
        if (!inicioVal) { alert('Informe a Data de In├¡cio do atestado.'); return; }
        formData.append('atestado_inicio', inicioVal);
        formData.append('atestado_fim', fimVal || inicioVal);
    } else {
        // Horas: data do atestado + hor├írios
...
window.saveVencimento = async function (docId, inputId) {
    ...
    try {
        ...
        if (res.ok) {
            ...
            await loadDocumentosList(); // Para atualizar a exibi├º├úo do Venc: dd/mm/aaaa no texto
...
window.enviarAtestadoContabilidade = async function (docId, emailInputId, btn) {
    ...
    // Suporte a m├║ltiplos e-mails separados por ';'
    const emails = email.split(';').map(e => e.trim()).filter(e => e.length > 0);
    if (emails.length === 0) { alert('Informe ao menos um e-mail v├ílido.'); return; }
...
window.renderPagamentosCompetencia = function () {
    ...
    // ÔöÇÔöÇ Se├º├úo sazonal: F├®rias (por ano, sem v├¡nculo de m├¬s) ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
    const feriasDoAno = currentDocs.filter(d => d.tab_name === 'Pagamentos' && d.document_type === 'F├®rias' && d.year == y);
    ...
    secFerias.innerHTML = `
        <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:1rem; flex-wrap:wrap; gap:0.75rem;">
            <h5 style="margin:0; color:#1e40af; display:flex; align-items:center; gap:0.5rem;">
                <i class="ph ph-sun-horizon" style="font-size:1.2rem;"></i> F├®rias <span style="font-size:0.8rem; font-weight:400; color:#64748b; margin-left:4px;">(${y} ÔÇö sazonal)</span>
            </h5>
            <div style="display:flex; align-items:center; gap:1rem; flex-wrap:wrap;">
                <div style="display:flex; align-items:center; gap:8px; font-size:0.82rem;">
                    ...
                    <label style="display:flex;align-items:center;gap:3px;cursor:pointer;margin:0;font-weight:500;">
                        <input type="radio" name="ferias-assin-${y}" value="NAO_EXIGE"> N├úo
                    </label>
                </div>
                <label style="display:inline-flex; align-items:center; gap:6px; cursor:pointer; background:#1e40af; color:#fff; border-radius:8px; padding:6px 14px; font-size:0.85rem; font-weight:600; transition:background 0.2s;"
                    ...
                    <i class="ph ph-upload-simple"></i> Adicionar F├®rias
                    <input type="file" accept=".pdf" style="display:none;"
                        onchange="const r=this.closest('div').querySelector('input[name^=ferias-assin-]:checked'); window.uploadDocument(this, 'Pagamentos', 'F├®rias', '${y}', null, null, r ? r.value : 'PENDENTE')">
                </label>
            </div>
        </div>
        <div id="ferias-slots-${y}" style="display:flex; flex-wrap:wrap; gap:0.75rem;">
            ${feriasDoAno.length === 0
                ? `<p style="color:#94a3b8; font-size:0.85rem; margin:0;">Nenhum documento de f├®rias cadastrado para ${y}. Clique em "Adicionar F├®rias" para inserir.</p>`
                : feriasDoAno.map(d => `
                    <div style="background:#eff6ff; border:1px solid #bfdbfe; border-radius:10px; padding:0.75rem 1rem; display:flex; align-items:center; gap:0.75rem; min-width:220px;">
                        ...
                        <div style="flex:1; min-width:0;">
                            <div style="font-weight:600; font-size:0.85rem; color:#1e3a8a; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;" title="${d.file_name || 'F├®rias'}">F├®rias ${y}</div>
    ...
    // ÔöÇÔöÇ Se├º├úo Outros (documentos avulsos do m├¬s) ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
    ...
    secOutros.innerHTML = `
        <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:1rem; flex-wrap:wrap; gap:0.75rem;">
            <h5 style="margin:0; color:#475569; display:flex; align-items:center; gap:0.5rem;">
                <i class="ph ph-folder-open" style="font-size:1.2rem;"></i> Outros <span style="font-size:0.8rem; font-weight:400; color:#94a3b8; margin-left:4px;">(documentos avulsos do m├¬s)</span>
            </h5>
            <div style="display:flex; align-items:center; gap:1rem; flex-wrap:wrap;">
                <div style="display:flex; align-items:center; gap:8px; font-size:0.82rem;">
                    <span style="font-weight:600;color:#64748b;">Exige Assinatura?</span>
                    ...
                    <label style="display:flex;align-items:center;gap:3px;cursor:pointer;margin:0;font-weight:500;">
                        <input type="radio" name="outros-assin-${y}-${m}" value="NAO_EXIGE"> N├úo
                    </label>
                </div>
        ...
        <div style="display:flex; flex-wrap:wrap; gap:0.75rem;">
            ${outrosDocs.length === 0
                ? `<p style="color:#94a3b8; font-size:0.85rem; margin:0;">Nenhum documento avulso para este m├¬s.</p>`
                : outrosDocs.map(d => `
...
window.deleteDoc = async function (docId, btnEl) {
    ...
    // Remo├º├úo otimista: esconde o card imediatamente para feedback visual instant├óneo
    ...
    try {
        ...
        if (res.ok) {
            ...
            // Atualizar lista em mem├│ria e re-renderizar a aba em background
            await loadDocumentosList();
            ...
            // Reverter a remo├º├úo otimista em caso de erro
            if (docCard) {
...
window.abrirArquivoOneDrive = function (path) {
    if (!path) return alert('Caminho do arquivo n├úo fornecido.');
...
window.viewDoc = async function (docId) {
    try {
        ...
        // Verificar se popup de doc-modal existe, caso contr├írio abrir numa nova janela
        const modal = document.getElementById('doc-modal');
        const modalBody = document.getElementById('modal-doc-body');
...
window.viewAssinado = async function (docId) {
    ...
    try {
        ...
        if (!res.ok) {
            const err = await res.json().catch(() => ({}));
            alert(err.error || 'PDF assinado ainda n├úo est├í dispon├¡vel.');
            return;
        }
...
window.downloadAssinado = async function (docId) {
    ...
    try {
        let handle = null;
        // Pede a pasta ao usu├írio antes do fetch para garantir que n├úo perde o foco/evento de clique do navegador (exig├¬ncia de seguran├ºa do Chrome)
        if (window.showSaveFilePicker) {
            try {
                ...
                if (e.name === 'AbortError') return; // Usu├írio cancelou a janela Salvar Como
            }
        }
        ...
        if (!res.ok) {
            document.body.style.cursor = 'default';
            const err = await res.json().catch(() => ({}));
            alert(err.error || 'PDF assinado ainda n├úo est├í pronto para download. Tente via Atualizar.');
            return;
        }
        ...
        if (handle) {
            ...
            return; // Sucesso, arquivo salvo onde o usu├írio quis
        }

        // Fallback: se o navegador n├úo suportar a janela Salvar Como (ex: Safari antigo, Firefox padr├úo)
        let fileName = 'documento_assinado_' + docId + '.pdf';
...
function getEffectiveStatus(c) {
    ...
    // Se est├í "Ativo" ou "F├®rias", verificamos as datas para saber se deve mostrar F├®rias
    if (status === 'Ativo' || status === 'F├®rias') {
        const today = new Date().toISOString().split('T')[0];
        ...
        // 2┬║ per├¡odo (apenas quando fracionada = Sim + tirada)
        ...
        if (em1 || em2) return 'F├®rias';

        // Voltou de f├®rias: apenas retorna Ativo se hoje ├® posterior ao fim de TODOS os per├¡odos v├ílidos
        if (status === 'F├®rias') {
...
function updateStatusChip(val) {
    document.querySelectorAll('.status-chip').forEach(c => {
        ...
        // c.style.display = 'none'; // Mantido vis├¡vel para permitir a sele├º├úo
    });
    ...
    if (input.files && input.files[0]) {
        ...
        if (colabId) {
            ...
            fetch(`${API_URL}/upload-foto/${colabId}`, {
                ...
                .then(data => {
                    if (data.sucesso) {
                        // A pr├®-visualiza├º├úo base64 j├í est├í correta no c├¡rculo.
                        // N├úo substitu├¡mos src por URL do servidor (ef├¬mero no Render).
                        // Apenas garantimos que a foto seja vis├¡vel ap├│s o upload.
                        const preview = document.getElementById('colab-foto-preview');
                        const stateSaved = document.getElementById('photo-state-saved');
...
window.mascaraRG = function (el) {
    ...
    if (raw.length <= 9) {
        ...
        // Para RGs maiores que 9 d├¡gitos, permite formato livre mantendo a pontua├º├úo original digitada
...
// Validar campo gen├®rico no frontend
...
window.toggleConjuge = function () {
    ...
    if (estado && (estado.value === 'Casado' || estado.value === 'Uni├úo Est├ível')) {
...
window.toggleMotorista = function () {
    ...
    if (cargoSelect && cargoSelect.value.toUpperCase().includes('MOTORISTA')) {
        ...
        section.style.display = 'none';
        // NEVER auto-clear CNH fields ÔÇö user must edit them manually
    }
};
...
window.mascaraTelefone = function (i) {
    ...
    let v = i.value;
    v = v.replace(/\D/g, ""); // Remove n├úo-d├¡gitos
    if (v.length > 10) {
        v = v.replace(/^(\d\d)(\d{5})(\d{4}).*/, "($1) $2-$3"); // 11 d├¡gitos
    } else if (v.length > 5) {
        v = v.replace(/^(\d\d)(\d{4})(\d{0,4}).*/, "($1) $2-$3"); // 10 d├¡gitos (fixo)
    } else if (v.length > 2) {
...
function updateProbationBadge(admissaoDate) {
    ...
    try {
        ...
        containers.forEach(container => {
            if (diffDays <= 45) {
                container.innerHTML = '<span class="probation-badge">1┬║ 45</span>';
            } else if (diffDays <= 90) {
                container.innerHTML = '<span class="probation-badge second">2┬║ 45</span>';
            }
        });
    } catch (e) { console.error('Erro ao calcular per├¡odo de experi├¬ncia:', e); }
}
...
// --- GEST├âO DE GERADORES DE DOCUMENTOS ---
...
// ÔöÇÔöÇÔöÇ Se├º├úo de Recibos de Benef├¡cios nos Geradores ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
window.renderRecibosGeradoresSection = function () {
    ...
    <div style="margin-bottom:1.5rem;">
      <div style="display:flex;align-items:center;gap:10px;margin-bottom:.85rem;">
        ...
        <div>
          <div style="font-size:.95rem;font-weight:700;color:#1e293b;">Recibos de Benef├¡cios</div>
          <div style="font-size:.78rem;color:#64748b;">Gera├º├úo individual de recibo por colaborador</div>
        </div>
      </div>
      <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(220px,1fr));gap:.85rem;">
        ...
        <div style="border:1.5px solid #bbf7d0;border-radius:12px;padding:1rem 1.1rem;background:#f0fdf4;display:flex;flex-direction:column;gap:.6rem;">
          <div style="display:flex;align-items:center;gap:8px;">
            ...
            <div>
              <div style="font-weight:700;color:#065f46;font-size:.92rem;">Vale Refei├º├úo</div>
        ...
        <div style="border:1.5px solid #bfdbfe;border-radius:12px;padding:1rem 1.1rem;background:#eff6ff;display:flex;flex-direction:column;gap:.6rem;">
          <div style="display:flex;align-items:center;gap:8px;">
            ...
            <div>
              ...
              <div style="font-size:.74rem;color:#2563eb;">Meio: VT (bilhete ├║nico)</div>
            </div>
          </div>
        ...
        <div style="border:1.5px solid #fed7aa;border-radius:12px;padding:1rem 1.1rem;background:#fff7ed;display:flex;flex-direction:column;gap:.6rem;">
          <div style="display:flex;align-items:center;gap:8px;">
            ...
            <div>
              <div style="font-weight:700;color:#92400e;font-size:.92rem;">Vale Combust├¡vel</div>
              <div style="font-size:.74rem;color:#d97706;">Meio: VC (combust├¡vel)</div>
            </div>
          </div>
    ...
    // Modal de gera├º├úo individual (cria uma vez)
        ...
        <div style="background:#fff;border-radius:16px;padding:1.75rem 2rem;width:420px;max-width:96vw;box-shadow:0 20px 60px rgba(0,0,0,.2);">
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:1.25rem;">
            ...
            <button onclick="document.getElementById('modal-recibo-individual').style.display='none'"
              style="background:none;border:none;font-size:1.4rem;color:#94a3b8;cursor:pointer;line-height:1;">├ù</button>
          </div>
          ...
          <div style="display:flex;flex-direction:column;gap:1rem;">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:.75rem;">
              <div>
                <label style="font-size:.8rem;font-weight:600;color:#475569;display:block;margin-bottom:.3rem;">M├¬s</label>
            ...
            <div>
              <label style="font-size:.8rem;font-weight:600;color:#475569;display:block;margin-bottom:.3rem;">
                Valor VR por dia (R$) <span style="font-weight:400;color:#94a3b8;">ÔÇö apenas para VR</span>
              </label>
...
// ÔöÇÔöÇÔöÇ Abrir modal de recibo individual ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
window.abrirModalReciboIndividual = function (tipo) {
    const modal = document.getElementById('modal-recibo-individual');
    if (!modal) return;
    const nomes = { VR: 'Vale Refei├º├úo', VT: 'Vale Transporte', VC: 'Vale Combust├¡vel' };
    const cores = { VR: '#059669', VT: '#2563eb', VC: '#d97706' };
    document.getElementById('mri-titulo').textContent = `Gerar Recibo de ${nomes[tipo]}`;
    document.getElementById('mri-subtitulo').textContent = `Selecione o colaborador e o per├¡odo de refer├¬ncia.`;
...
// ÔöÇÔöÇÔöÇ Busca inline de colaborador no modal ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
window._mriSearch = function (q) {
    ...
    lista.innerHTML = res.map(c => `
        <div onclick="window._mriSelectColab(${c.id},'${(c.nome||'').replace(/'/g,'\\\'')}')"
          ...
          <span style="font-size:.75rem;color:#94a3b8;margin-left:8px;">${c.cargo || ''} ┬À ${c.departamento || ''}</span>
        </div>`).join('');
};
...
// ÔöÇÔöÇÔöÇ Gerar recibo individual pelo modal ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
window._mriGerar = function () {
    ...
    if (!colabId) {
        if (typeof Swal !== 'undefined') Swal.fire('Aten├º├úo', 'Selecione um colaborador.', 'warning');
...
window.renderGeradoresList = function (items) {
    ...
    const TEMPLATE_LABELS = {
        admissao: { label: 'Admiss├úo', bg: '#eff6ff', color: '#1d4ed8', border: '#bfdbfe' },
        contratos: { label: 'Contratos', bg: '#f0fdf4', color: '#15803d', border: '#bbf7d0' },
        ambos: { label: 'Ambos', bg: '#fdf4ff', color: '#c026d3', border: '#f0abfc' },
        nenhum: { label: 'ÔÇö', bg: '#f8fafc', color: '#94a3b8', border: '#e2e8f0' },
    };

    const PROTECTED_NAMES = [
        'autoriza├º├úo de desconto em folha',
        'ordem de servi├ºo nr01',
        'termo de n├úo interesse terapia',
    ...
    const isProtected = (nome) => {
        ...
        // Expor essas c├│pias precisas para que o usu├írio consiga exclu├¡-las
        const BAD_EXACT_NAMES = [
            'AUTORIZA├ç├âO DE DESCONTO EM FOLHA DE PAGAMENTO',
            'ORDEM DE SERVI├çO NR01'
        ];
        if (BAD_EXACT_NAMES.includes(originalName)) return false;

        // Forcefully allow these to be deleted
        if (u.includes('equipamento') || u.includes('ve├¡culo') || u.includes('veiculo')) return false;

        return PROTECTED_NAMES.some(pn => u.includes(pn));
    };

    // Sort: protegidos primeiro, depois alfab├®tico
...
window.renderGeradoresTemplates = function (departamentos, geradores, templates) {
    ...
    // Contratos de uso exclusivo de outros fluxos (ex: Multas) n├úo devem aparecer nos templates de admiss├úo
    const GERADORES_EXCLUSIVOS_AVULSO = ['AUTORIZA├ç├âO DE DESCONTO EM FOLHA DE PAGAMENTO', 'AUTORIZA├ç├âO DE DESCONTO EM FOLHA'];
    ...
    const templates = [
        {
            nome: "NR1",
            conteudo: `
<p style="text-align: center; font-weight: bold; font-size: 1.2rem; margin-bottom: 2rem;">ORDEM DE SERVI├çO - NR1</p>

<p style="font-weight: bold; text-decoration: underline;">DESCRI├ç├âO DA ATIVIDADE</p>
<p style="text-transform: uppercase;">FAZER SUC├ç├âO COM EQUIPAMENTOS APROPRIADOS DOS DEJETOS DOS BANHEIROS, REPOR OS DESODORANTES, EFETUAR LAVAGEM E SECAGEM DOS MESMOS E EFETUAR A CARGA E DESCARGA DOS BANHEIROS QU├ìMICOS NOS CAMINH├òES E NOS LOCAIS DEFINIDOS PELO SEU SUPERIOR IMEDIATO, NORMAS E PROCEDIMENTOS INTERNOS.</p>

<p style="font-weight: bold; text-decoration: underline; margin-top: 1.5rem;">IDENTIFICA├ç├âO DOS RISCOS AMBIENTAIS</p>
...
<ul style="list-style-type: none; padding-left: 0; margin-top: 0.5rem; line-height: 1.6;">
    <li><b>F├¡sicos:</b> Ru├¡do peculiar a ambientes externos e umidade da lavagem dos sanit├írios.</li>
    <li><b>Qu├¡micos:</b> Produtos saneantes: desinfetantes, bactericida e desodoriza├º├úo sanit├íria.</li>
    <li><b>Biol├│gicos:</b> Suc├º├úo de dejetos e limpeza de sanit├írios qu├¡micos.</li>
    <li><b>Ergon├┤micos:</b> intensidade pequena (poss├¡vel postura inadequada, poss├¡vel stress).</li>
    <li><b>Acidentes:</b> intensidade pequena (poss├¡veis acidentes de quedas, cortes e perfura├º├Áes e outros).</li>
</ul>
...
<table style="width: 100%; border-collapse: collapse; margin-top: 0.5rem;" border="1">
    <thead>
        <tr style="background-color: #f1f5f9;">
            <th style="padding: 8px; text-align: left;">EPIÔÇÖs (Equipamentos de Prote├º├úo Individual)</th>
            <th style="padding: 8px; text-align: left;">OBSERVA├ç├òES</th>
        </tr>
    </thead>
    <tbody>
        <tr>
            <td style="padding: 8px;">├ôCULOS DE PROTE├ç├âO, LUVA DE NEOLATEX, CAPACETE COM JUGULAR, BOTA TIPO B COM BICO DE A├çO, UNIFORME COMPLETO, PROTETOR SOLAR, PROTETOR AUDITIVO, CAPA DE CHUVA.</td>
...
<ul style="margin-top: 0.5rem; line-height: 1.6;">
    ...
    <li>ORIENTA├ç├òES DE SEGURAN├çA DOS LOCAIS DE PRESTA├ç├âO DE SERVI├çOS.</li>
</ul>

<p style="margin-top: 2rem;">Declaro ter recebido as instru├º├Áes de Seguran├ºa e Sa├║de no Trabalho de acordo com a NR-1, bem como os EPIs necess├írios e comprometo-me a cumprir todas as normas estabelecidas.</p>
`
        },
        {
            nome: "Acordo Individual Benef├¡cios",
            conteudo: `
<p style="margin-top: 1.5rem;">CARO COLABORADOR,</p>
<p>├ü EMPRESA:</p>

<p style="margin-top: 1.5rem;">
    <b>AMERICA RENTAL EQUIPAMENTOS LTDA</b>, Situada na Rua Salto da Divisa, n┬║ 97, CEP 07252-300, Pq Alvorada - Guarulhos SP, inscrita no CNPJ sob o n┬║ 03.434.448/0001-01, neste ato representado pela s├│cia propriet├íria Sra. Nicole Mezuraro Maio, brasileira, solteira, empres├íria, portadora da c├®dula de identidade R.G. n┬║ 43.690.066 SSP/SP e CPF/MF n┬║ 355.026.968-47, doravante denominada EMPRESA.
</p>

<p style="margin-top: 1.5rem;">
    Decidem as partes, na melhor forma de direito, celebrar o presente <b>ACORDO INDIVIDUAL</b>, para fins de alterar algumas condi├º├Áes do atual contrato de trabalho vigente, que reger-se-├í mediante as cl├íusulas e condi├º├Áes adiante estipuladas.
</p>

<p style="margin-top: 1.5rem;"><b>CL├üUSULA PRIMEIRA - DOS MOTIVOS</b></p>
<p>Com o foco de acrescentar melhorias e qualidade de vida aos colaboradores a empresa <b>AMERICA RENTAL EQUIPAMENTOS LTDA</b>, por mera liberalidade, disponibiliza conv├¬nios com os estabelecimentos:</p>

<ol style="margin-top: 1rem; line-height: 2;">
    <li>REDE DROGA LESTE FARM├üCIA</li>
    <li>SUPERMERCADO PARAISO - MERCADINHO BERLIM LTDA - ME</li>
    <li>ACADEMIA - ATITUDE FITNESS</li>
</ol>

<p style="margin-top: 1.5rem;"><b>CL├üUSULA SEGUNDA ÔÇô DOS DESCONTOS E DOS ESTABELICIMENTOS</b></p>
<p>O colaborador autoriza os descontos de seu sal├írio caso venha utilizar os conv├¬nios colocados a sua disposi├º├úo, conforme numerados na cl├íusula anterior. Ademais, o colaborador fica ciente que n├úo ├® obrigado a utilizar o conv├¬nio, logo, sem a utiliza├º├úo n├úo haver├í qualquer desconto de sua folha de pagamento.</p>

<p style="margin-top: 1.5rem;"><b>CL├üUSULA TERCEIRA - DA VIG├èNCIA</b></p>
<p>O presente acordo vigorar├í a partir da presente data pelo per├¡odo da vig├¬ncia do contrato de trabalho do Colaborador.</p>
            `,
            variaveis: ""
        },
        {
            nome: "Autoriza├º├úo de Uso de Imagem",
            conteudo: `
<p style="margin-top: 2rem;">
    <b>AUTORIZO</b> o uso de minha imagem e voz, em todo e qualquer material entre fotos, documentos e outros meios de comunica├º├úo, para campanhas promocionais e institucionais e etc. desta empresa, <b>AMERICA RENTAL EQUIPAMENTOS LTDA</b>, Situada na Rua Salto da Divisa, n┬║ 97, CEP 07252-300, Pq Alvorada - Guarulhos SP, inscrita no CNPJ sob o n┬║ 03.434.448/0001-01, sejam essas destinadas ├á divulga├º├úo ao p├║blico em geral e/ou apenas para uso interno, e desde que n├úo haja desvirtuamento da sua finalidade.
</p>

<p style="margin-top: 1.5rem;">
    A presente autoriza├º├úo ├® concedida a t├¡tulo gratuito, abrangendo o uso da imagem acima mencionada em todo territ├│rio nacional e no exterior, sob qualquer forma e meios, ou sejam, em destaque: (I) out-door; (II) bus-door; folhetos em geral (encartes, mala direta, cat├ílogo, etc.); (III) folder de apresenta├º├úo; (IV) an├║ncios em revistas e jornais em geral; (V) home page; (VI) cartazes; (VII) back-light; (VIII) m├¡dia eletr├┤nica (pain├®is, v├¡deo-tapes, televis├úo, cinema, programa para r├ídio, rede social entre outros).
</p>

<p style="margin-top: 1.5rem;">
    Por esta ser a express├úo da minha vontade declaro que autorizo o uso acima descrito sem que nada haja a ser reclamado a t├¡tulo de direitos conexos ├á minha imagem ou a qualquer outro.
</p>
            `,
            variaveis: ""
        },
        {
            nome: "Acordo de Aux├¡lio-Combust├¡vel",
            conteudo: `
<p style="margin-top: 1.5rem;">
    <b>AMERICA RENTAL EQUIPAMENTOS LTDA</b>, Situada na Rua Salto da Divisa, n┬║ 97, CEP 07252-300, Parque Alvorada - Guarulhos SP, Inscrita no CNPJ sob o n┬║ 03.434.448/0001-01, denominada empregador, e Colaborador, de comum acordo e na melhor forma do direito, as partes celebram o presente Acordo Individual Escrito, com apoio nos art. 444, 457, 458 e art. 468 da CLT para tratar exclusivamente das condi├º├Áes para fornecimento de aux├¡lio-combust├¡vel, mantendo-se inalteradas as demais cl├íusulas contratuais firmadas.
</p>

<p style="margin-top: 1.5rem;">
    <b>Cl├íusula Primeira:</b> O empregador fornecer├í mensalmente o valor fixo de R$220,00 (duzentos e vinte reais) a t├¡tulo de aux├¡lio-combust├¡vel ao trabalhador que comprovar a necessidade de utiliza├º├úo de ve├¡culo pr├│prio para o deslocamento casa ÔÇô trabalho, de forma escrita.
</p>

<p style="margin-top: 1rem;">
    <b>Par├ígrafo primeiro:</b> A comprova├º├úo de que trata essa cl├íusula, dever├í ser feita mediante apresenta├º├úo de comprovante de resid├¬ncia em nome pr├│prio e identifica├º├úo de ve├¡culo utilizado no ato da contrata├º├úo.
</p>

<p style="margin-top: 1rem;">
    <b>Par├ígrafo Segundo:</b> O valor a t├¡tulo de aux├¡lio-combust├¡vel ser├í reajustado anualmente a crit├®rio do empregador.
</p>

<p style="margin-top: 1.5rem;">
    <b>Cl├íusula Segunda:</b> As partes esclarecem que referido aux├¡lio-combust├¡vel possui natureza indenizat├│ria, n├úo se integrando ├á remunera├º├úo para quaisquer fins.
</p>

<p style="margin-top: 1.5rem;">
    <b>Cl├íusula Terceira:</b> S├úo condi├º├Áes para o fornecimento do aux├¡lio-combust├¡vel mensalmente ao trabalhador, de forma cumulativa, a utiliza├º├úo de ve├¡culo pr├│prio para deslocamento e a inexist├¬ncia de qualquer falta ao trabalho no m├¬s correspondente.
</p>

<p style="margin-top: 1.5rem;">
    <b>Cl├íusula Quarta:</b> Em caso de aus├¬ncia injustificada por parte do empregado ao trabalho o aux├¡lio-combust├¡vel n├úo ser├í fornecido.
</p>

<p style="margin-top: 1rem;">
    <b>Par├ígrafo ├║nico:</b> Considera-se como aus├¬ncia injustificada, qualquer hip├│tese distinta da prevista no art. 473 da CLT.
</p>

<p style="margin-top: 1.5rem;">
    <b>Cl├íusula Quinta:</b> Ao trabalhador que n├úo utilizar ve├¡culo pr├│prio para deslocamento casa trabalho, n├úo ser├í pago o aux├¡lio-combust├¡vel.
</p>

<p style="margin-top: 1.5rem;">
    <b>Cl├íusula Sexta:</b> Fica desde j├í autorizada a revis├úo das condi├º├Áes para o fornecimento e/ou supress├úo do referido aux├¡lio-combust├¡vel pelo empregador a qualquer tempo, e sem aviso pr├®vio, n├úo importando em direito adquirido do trabalhador, n├úo se aderindo ao contrato de trabalho.
</p>

<p style="margin-top: 1.5rem;">
    <b>Cl├íusula S├®tima:</b> As partes firmam o presente de comum acordo, assinando em duas vias de igual teor.
</p>
            `,
            variaveis: ""
        }
...
window.openModalGerador = function () {
    ...
    // Padr├úo: marcar ambos os templates
...
window.editGerador = async function (id) {
    try {
        ...
        // Detectar se ├® texto puro leg├¡vel (legado) ou HTML
...
function setupGeradores() {
    ...
    if (!form) {
        console.warn('Formul├írio de gerador n├úo encontrado na inicializa├º├úo. Tentando novamente em breve.');
        setTimeout(setupGeradores, 500); // Tentar novamente se o HTML n├úo carregou
        return;
    }
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (result && !result.error) {
                ...
                try {
                    ...
                    if (geradorId && deptIds.length > 0) {
                        // Buscar templates atuais para n├úo sobrescrever os de outros geradores
            ...
            console.error(e);
            alert('Falha cr├¡tica ao salvar gerador. Verifique o console.');
        }
    });
}

// Fun├º├Áes do Editor de Texto
...
window.calcParcelaDesconto = function () {
    ...
    if (!valStr) valStr = '0';
    // Substituir v├¡rgula por ponto para c├ílculo
...
window.processarGeracao = async function () {
    ...
    if (gerador && (gerador.nome || '').toUpperCase().includes('DESCONTO EM FOLHA')) {
        requestBody.desconto_descricao = document.getElementById('desconto-descricao').value || 'N├úo informado';
...
window.abrirPreviewDocumento = function (data) {
    ...
    if (!container) {
        // Fallback robusto se o modal foi removido erroneamente por cache/c├│digos antigos
        console.warn("Modal preview doc n├úo encontrado. Recriando...");
        const htmlFallback = `
    ...
    if (previewBtnSalvar) {
        ...
        previewBtnSalvar.innerHTML = '<i class="ph ph-paperclip"></i> Anexar ao Prontu├írio';
        previewBtnSalvar.onclick = async function () {
            ...
            try {
                ...
                if (!colaboradorId) {
                    throw new Error('N├úo foi poss├¡vel identificar o colaborador. Abra o prontu├írio e tente novamente.');
                }
                ...
                if (typeof showToast !== 'undefined') {
                    showToast('Documento anexado ao prontu├írio com sucesso!', 'success');
                } else {
    ...
    // Verificar op├º├úo de assinatura manual
    const comAssinatura = document.querySelector('input[name="assinatura-tipo"]:checked')?.value === 'sim';


    // 1. Cabe├ºalho com Logotipo ÔÇö sem margem para colar no topo da p├ígina
    ...
    const colabInfoBase = `
        <div style="border: 1px solid #000; padding: 0.4rem 0.75rem; margin-top: 0.5rem; line-height: 1.3; font-size: 0.75rem;">
            ...
            <p style="margin: 0px;">ENDERE├çO: ${data.colaborador.ENDERECO || '---'}</p>
    ...
    // 3. Conte├║do ÔÇö compactar espa├ºamento de par├ígrafos
    ...
    // CSS de quebra de p├ígina correta ÔÇö evita cortar par├ígrafos e cl├íusulas no meio
    ...
    if (comAssinatura) {
        ...
        <div style="margin-top: 1.5rem;">
            ...
            <div style="margin-top: 2.5rem; display: flex; justify-content: space-between; align-items: flex-end;">
                ...
                <div style="text-align: center; width: 45%;">
                    ...
                    <div style="border-top: 1.5px solid #000; padding-top: 0.35rem;">
                        <span style="font-weight: 700; font-size: 0.85rem;">Am├®rica Rental Equipamentos Ltda</span><br>
        ...
        // Sem campos de assinatura ÔÇö apenas a data (assinatura ser├í digital)
        footerHtml = `
        <div style="margin-top: 1.5rem;">
            <p style="font-weight: 700; font-size: 0.9rem;">${dataFormatada}</p>
        </div>`;
    ...
    // Logo cola no topo (sem padding), o resto do conte├║do tem padding lateral uniforme
...
// Salvar como PDF ÔÇö usa o di├ílogo de impress├úo do navegador com destino "Salvar em PDF"
...
// --- GEST├âO DE ADMISS├âO ---
const ADMISSAO_STATUS_STYLES = {
    'Aguardando in├¡cio': { bg: '#f1f3f5', color: '#495057', border: '#adb5bd', icon: 'ph-hourglass-high', label: 'Aguardando' },
    ...
    'F├®rias': { bg: '#fdf7e3', color: '#c2aa72', border: '#c2aa72', icon: 'ph-airplane-tilt', label: 'F├®rias' },
...
window.loadAdmissaoSelect = async function () {
    try {
        ...
        // Apenas colaboradores pendentes de admiss├úo
        // Apenas colaboradores com status de admiss├úo pendente
        const ADMISSAO_PENDENTES = [
            'aguardando inicio', 'aguardando in├¡cio',
            'processo iniciado', 'em admissao', 'em admiss├úo',
            'aguardando', 'pendente'
        ];
        const pendentes = rows.filter(r => {
            ...
            // Inclui se o status for de admiss├úo pendente OU se admissao_status indicar processo em aberto
            if (ADMISSAO_PENDENTES.includes(s)) return true;
            if (admStatus && admStatus !== 'concluida' && admStatus !== 'conclu├¡do') return true;
            return false;
        });
...
window.sendAssinafyWhatsApp = async function (tipo, suffix) {
    if (!viewedColaborador || !viewedColaborador.telefone) {
        alert('Telefone do colaborador n├úo encontrado para enviar WhatsApp.');
        return;
    }
    ...
    const msg = `Ol├í, ${viewedColaborador.nome_completo}.\n\nSeu Exame Admissional est├í dispon├¡vel para assinatura digital.\n\nClique no link abaixo para assinar:\n${linkAssinafy}\n\nAm├®rica Rental Equipamentos Ltda.`;
...
// --- Refatora├º├úo: Fun├º├úo gen├®rica para construir HTML da lista de contratos (Admiss├úo / Prontu├írio) ---
window.buildAdmissaoSignatureRows = function (availableGeradores, assinaturas, docs, colab) {
    return availableGeradores.map(g => {
        ...
        if (!isSigned && !isPending && !naoExige && !isAnexado) {
            dynamicControls = `
                <div id="admissao-inline-box-${g.id}" style="display:flex; align-items:center; gap:12px; background:#f8fafc; padding:8px 12px; border-radius:6px; border:1px solid #e2e8f0; font-size:0.85rem;">
                    ...
                    <label style="display:flex; align-items:center; gap:4px; margin:0; cursor:pointer;">
                        <input type="radio" name="req-ass-adm-${g.id}" value="nao" onchange="window.renderInlineAdmissaoAction('${g.id}', 'nao', '${colabId}')"> N├úo
                    </label>
                </div>
        ...
        // Bot├úo excluir: usa id do doc (admissao_assinaturas) ou docEquivalente (documentos)
...
window.sendSingleAdmissaoSignature = async function (geradorId, colabId, btn) {
    ...
    try {
        ...
        if (!gerador) throw new Error("Gerador n├úo encontrado.");
...
window.uploadAdmissaoAvulso = async function (geradorId, colabId, fileInput) {
    ...
    const gerador = window._admissaoGeradores?.find(g => String(g.id) === String(geradorId));
    if (!gerador) return alert('Gerador n├úo encontrado.');
...
// ===== ABA CONTRATOS (PRONTU├üRIO DIGITAL) ÔÇö apenas Outros Contratos =====
window.renderContratosTab = async function (container, searchTerm = '') {
    ...
    // Vai direto para Outros Contratos ÔÇö sem sub-aba de Admiss├úo
    await window.renderContratosAvulso(container, searchTerm);
};

// Helper: recarrega a aba Contratos no container correto
// S├│ roda se ca-list-container estiver no DOM (indica que a aba Contratos est├í ativa)
window._reloadContratosContainer = async function () {
    window._contratosAvulsoLoaded = false;
    // O elemento ca-list-container ├® renderizado por renderContratosAvulso
    // Se ele existe, o usu├írio EST├ü na aba Contratos
...
window._avaliarRegraGerador = function (g, colab, deptNome) {
    ...
    // Verificar restri├º├úo de departamento
    ...
    // Verificar condi├º├úo de campo do colaborador
    if (regra.condicao) {
        ...
        if (operador === 'contains') {
            ...
            // Aceitar varia├º├Áes "Sim"/"sim", "Nao"/"N├úo"/"n├úo", "Intermitente" etc.
...
window.renderContratosAvulso = async function (container, searchTerm = '') {
    ...
    try {
        ...
        const EXCLUIDOS_FIXOS = [
            'autoriza├º├úo de desconto em folha de pagamento',
        ...
        const isExcluido = (g) => {
            ...
            // Excluir geradores cujo nome come├ºa com "Sinistro"
            if (nLower.startsWith('sinistro')) return true;
            if (nLower.startsWith('sinistro -')) return true;
            return false;
        };

        // Geradores eleg├¡veis (sem exclu├¡dos e sem sinistro)
        ...
        if (!algumTemRegra) {
            ...
            const LEGACY_MAP = [
                { nome: 'Termo de N├âO Interesse Terapia', cond: deNorm(c.terapia_participa) === 'nao' || deNorm(c.terapia_participa) === 'nao' },
                { nome: 'Termo de Interesse Terapia', cond: deNorm(c.terapia_participa) === 'sim' },
                { nome: 'Responsabilidade Bilhete ├Ünico', cond: (c.meio_transporte || '').toLowerCase().includes('vt') },
                ...
                { nome: 'Acordo Individual Benef├¡cios', cond: true },
                { nome: 'Autoriza├º├úo de Uso de Imagem', cond: true },
                ...
                { nome: 'Solicita├º├úo de VT', cond: true },
            ];
        ...
        // Regra: mostrar TODOS os eleg├¡veis na lista suspensa de qualquer colaborador
        // (excetos sinistro e exclu├¡dos, j├í tratados acima)
        ...
        const dropdownGeradores = geradoresElegiveis.filter(g => {
            ...
            // Se dropdown_todos = false e n├úo ├® auto-vis├¡vel para este colab, n├úo aparece no dropdown
        ...
        // Normaliza├º├úo para matching de nomes
        const _norm = s => (s || '').normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase().trim();

        // Fun├º├úo para encontrar doc correspondente a um gerador
        ...
        // Renderiza├º├úo INTERCALADA: para cada gerador de perfil, mostra o doc
        // correspondente (se j├í existir) ou a linha pendente (se ainda n├úo).
        // Docs avulsos sem gerador correspondente ficam no final.
        // ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
        const docsUsados = new Set();
        let combinedHtml = '';

        // --- MUDAN├çA: Ordenar TODOS os documentos j├í gerados do mais novo para o mais antigo ---
        ...
        for (const g of autoGeradores) {
            ...
            if (!docMatch) {
                // Gerador pendente: renderiza a linha de perfil aguardando gera├º├úo
        ...
        // ÔöÇÔöÇ Slot especial: Ficha de Registro (upload-only, n├úo gerado por template) ÔöÇÔöÇ
            ...
            <div style="display:flex; align-items:center; justify-content:space-between; padding:0.65rem 0.75rem; border:1.5px dashed #64748b; border-radius:8px; background:#f8fafc; gap:0.75rem;">
                <div style="display:flex; align-items:center; gap:0.6rem; flex:1;">
                    ...
                    <div>
                        ...
                        <div style="font-size:0.75rem; color:#64748b; margin-top:1px;">Anexe o PDF da Ficha de Registro para envio ├á assinatura</div>
                    </div>
                </div>
                <label class="btn btn-secondary" style="display:flex;align-items:center;gap:0.4rem;cursor:pointer;font-size:0.82rem;padding:0.35rem 0.8rem;margin:0;">
                    <i class="ph ph-upload-simple"></i> Anexar PDF
...
// Guard: impede execu├º├Áes concorrentes do sync
...
window.sincronizarStatusAssinaturas = async function (showFeedback) {
    if (!viewedColaborador) return;
    // Evita execu├º├Áes concorrentes (loop de reload)
    if (window._syncContratosRunning) return;
    window._syncContratosRunning = true;

    const btn = null; // bot├úo manual removido, mantido por compatibilidade
    ...
    try {
        ...
        // Docs que o banco J├ü atualizou para Assinado (backend polling j├í rodou)
        ...
        // 'Pendente' = salvo mas ainda n├úo enviado / 'Aguardando' = j├í enviado ao Assinafy
        const pendentes = contratosDocs.filter(d =>
            (d.assinafy_status === 'Pendente' || d.assinafy_status === 'Aguardando') && d.assinafy_id
        );

        // Se n├úo h├í pendentes mas h├í assinados recentes: se ├® o bot├úo manual, apenas recarregar a lista
            ...
            if (showFeedback) {
                ...
                if (typeof showToast !== 'undefined') {
                    const msg = jaAssinados.length > 0
                        ? `Lista atualizada ÔÇö ${jaAssinados.length} documento(s) assinado(s).`
        ...
        // S├│ recarrega a lista se algum status mudou para Assinado
        // Evita loop infinito: reload ÔåÆ auto-sync ÔåÆ reload
        if (atualizado > 0) {
        ...
        if (showFeedback && typeof showToast !== 'undefined') {
            const msg = atualizado > 0
                ...
                : `Verificado ÔÇö ${pendentes.length} documento(s) ainda aguardando assinatura.`;
...
// POPUP ESPECIAL: Solicita├º├úo de Vale Transporte (VT)
// ÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉ
window._abrirPopupVT = function (geradorId, geradorNome) {
    const c = viewedColaborador || {};

    // ÔöÇÔöÇ Parser de endere├ºo do colaborador ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
    ...
    // Caso contr├írio, tenta fazer o parse da string completa
    ...
    if (!vtRua && endFull) {
        // Padr├úo comum: "Rua das Flores, 123, Bairro, Guarulhos - SP, 00000-000"
        // Tenta extrair por v├¡rgulas
        const partes = endFull.split(',').map(p => p.trim());
        vtRua    = partes[0] || '';
        // N├║mero pode ser 2┬¬ parte num├®rica ou embutido na rua ("Rua X 123")
        if (partes[1] && /^\d/.test(partes[1])) {
            ...
            // CEP costuma ser a ├║ltima parte num├®rica
            const cepParte = partes[partes.length - 1] || '';
            if (/\d{5}-?\d{3}/.test(cepParte) && !vtCEP) vtCEP = cepParte.match(/\d{5}-?\d{3}/)[0];
        } else {
            // Tenta extrair n├║mero embutido na string da rua: "Rua das Flores 123"
    ...
    <div style="background:#fff;border-radius:14px;width:100%;max-width:700px;box-shadow:0 25px 80px rgba(0,0,0,0.35);max-height:92vh;display:flex;flex-direction:column;">
      ...
      <div style="background:#0f172a;padding:1.1rem 1.5rem;border-radius:14px 14px 0 0;display:flex;align-items:center;justify-content:space-between;flex-shrink:0;">
        <div style="display:flex;align-items:center;gap:10px;">
          ...
          <div>
            <h3 style="margin:0;color:#f1f5f9;font-size:1rem;font-weight:700;">Solicita├º├úo de Vale Transporte</h3>
      ...
      <div style="overflow-y:auto;flex:1;padding:1.5rem;">
        ...
        <div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;padding:1.2rem;margin-bottom:1.2rem;">
          <p style="margin:0 0 0.8rem;font-weight:700;color:#334155;font-size:0.95rem;">O colaborador opta pela utiliza├º├úo do Vale Transporte?</p>
          <div style="display:flex;gap:2rem;">
            <label style="display:flex;align-items:center;gap:0.5rem;cursor:pointer;font-size:1rem;font-weight:600;color:#16a34a;">
              ...
              Ô£à Sim ÔÇö Opto pelo Vale Transporte
            </label>
            <label style="display:flex;align-items:center;gap:0.5rem;cursor:pointer;font-size:1rem;font-weight:600;color:#dc2626;">
              ...
              ÔØî N├úo ÔÇö N├úo opto pelo Vale Transporte
            </label>
          </div>
        </div>

        <!-- Se├º├úo SIM: endere├ºo + linhas -->
        <div id="vt-secao-sim" style="display:none;">
          <!-- Endere├ºo Residencial ÔÇö oculto, preenchido automaticamente do cadastro -->
          ...
          <!-- Linhas de Transporte Resid├¬nciaÔåÆTrabalho -->
          <div style="background:#eff6ff;border:1px solid #bfdbfe;border-radius:10px;padding:1rem;margin-bottom:0.8rem;">
            <p style="margin:0 0 0.6rem;font-weight:700;color:#1d4ed8;font-size:0.9rem;">­ƒÜî Resid├¬ncia ÔåÆ Trabalho</p>
          ...
          <!-- Linhas de Transporte TrabalhoÔåÆResid├¬ncia -->
          <div style="background:#fdf4ff;border:1px solid #f0abfc;border-radius:10px;padding:1rem;">
            <p style="margin:0 0 0.6rem;font-weight:700;color:#c026d3;font-size:0.9rem;">­ƒÜî Trabalho ÔåÆ Resid├¬ncia</p>
            <table style="width:100%;border-collapse:collapse;font-size:0.85rem;">
        ...
        <div id="vt-secao-nao" style="display:none;">
          <div style="background:#fef2f2;border:1px solid #fecaca;border-radius:10px;padding:1.2rem;text-align:center;">
            ...
            <p style="margin:0.5rem 0 0;color:#b91c1c;font-weight:600;">O colaborador n├úo optou pelo Vale Transporte.</p>
            <p style="margin:0.3rem 0 0;color:#64748b;font-size:0.85rem;">O documento ser├í gerado com a op├º├úo "N├úo opto" marcada e enviado para assinatura.</p>
          </div>
        </div>
...
window._gerarVTConfirmado = async function (geradorId, geradorNome) {
    ...
    if (!opcao) { alert('Selecione se o colaborador opta ou n├úo pelo Vale Transporte.'); return; }
...
window.previewContratoPerfilAssinado = async function (geradorId, geradorNome) {
    // ÔöÇÔöÇ Intercep├º├úo especial para Solicita├º├úo de VT ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
...
// Anexar ao Prontu├írio direto do preview (com assinatura pendente)
// Salva o PDF como 'Pendente', fecha o preview e mostra o bot├úo "Enviar para Assinatura"
window.anexarAoProntuarioPerfil = async function (btn) {
    ...
    try {
        ...
        if (!previewContent) throw new Error('Conte├║do do preview n├úo encontrado. Feche e gere novamente.');
...
// Enviar para assinatura um doc j├í salvo no banco (Pendente, sem assinafy_id)
...
window.enviarAssinaturaDocSalvo = async function (docId, geradorNome) {
    ...
    try {
        ...
        // Recarrega para refletir o novo status do banco (assinafy_id preenchido ÔåÆ estado "Aguardando")
        await window._reloadContratosContainer();
    } catch (e) {
            ...
            <div style="text-align:left;display:flex;flex-direction:column;gap:1.2rem;padding:0.25rem 0;">
                ...
                <div>
                    ...
                    <div style="display:flex; gap:1.5rem;">
                        ...
                        <label style="display:flex; align-items:center; gap:0.4rem; cursor:pointer; font-size:0.9rem;">
                            <input type="radio" name="swal-assinatura" value="nao" checked style="width:16px;height:16px;accent-color:#2563eb;"> N├úo
                        </label>
                    </div>
                </div>
            </div>`,
        ...
        preConfirm: async function () {
            ...
            try {
                ...
                if (reqAssinatura === 'sim') {
                    ...
                    const dt = new Date();
                    const horaEnvio = dt.toLocaleDateString() + ' ├ás ' + dt.toLocaleTimeString();
...
// Upload com tipo de documento pr├®-definido (ex: Ficha de Registro, NR1 - n├úo gerados por template)
window.uploadContratoExternoComTipo = async function (input, docType, tabName) {
    ...
    try {
        ...
        // Recarregar a aba correta ap├│s upload
        await loadDocumentosList();
...
window.openContratoViewerPopup = function (pdfUrl, nomeDoc) {
    if (!pdfUrl || pdfUrl.endsWith('undefined')) { alert('URL do documento n├úo encontrada.'); return; }
...
window.buildContratosSignatureRows = function (assinaturas, docs, colab) {
    ...
    docs.forEach(doc => {
        ...
        // 'Pendente' SEM assinafy_id = doc salvo localmente, nunca enviado ao Assinafy (1┬¬ vez)
        ...
        if (isSigned) {
            ...
            // Documento salvo localmente (Pendente sem assinafy_id) ÔÇö aguardando envio ao Assinafy
            ...
            statusBadge = `<span data-role="status-badge" style="color:#7c3aed;font-size:0.75rem;font-weight:600;">Documento salvo ÔÇö clique em Enviar para Assinatura${_uploadStr ? ': ' + _uploadStr : ''}</span>`;
...
// Excluir contrato com prote├º├úo por senha (funciona para assinados e n├úo-assinados)
window.excluirContratoComSenha = async function (docId, tipo) {
    const senha = prompt('ÔÜá´©Å Esta a├º├úo ├® irrevers├¡vel.\n\nDigite a senha para excluir o contrato:');
    if (senha === null) return; // cancelado
    if (senha !== 'EXc2499!') {
        alert('Senha incorreta. Exclus├úo cancelada.');
        return;
    }
    if (!confirm('Confirmar exclus├úo do contrato? Esta a├º├úo n├úo pode ser desfeita.')) return;

    try {
        let res;
        if (tipo === 'admissao') {
            // Exclui da tabela admissao_assinaturas via endpoint pr├│prio
        ...
        if (res.ok) {
            if (typeof showToast !== 'undefined') showToast('Contrato exclu├¡do com sucesso.', 'success');
            await window._reloadContratosContainer();
        } else {
...
window.enviarDocumentoAvulsoAssinatura = async function (docId, btn) {
    ...
    try {
        ...
        if (res.ok) {
            ...
            // Atualiza o card diretamente no DOM usando data-role (imune ├á normaliza├º├úo de estilos do browser)
            ...
            if (card) {
                ...
                // ├ìcone: paperclip ÔåÆ avi├úo (usa data-role para sele├º├úo confi├ível)
                ...
                // Status badge (usa data-role para sele├º├úo confi├ível)
            ...
            // Nota: N├âO recarrega aqui ÔÇö novo_processo demora 3s+ e reload antecipado reverte UI.
            // O interval de 30s sincroniza com o DB ap├│s o processo completar.
            window._syncContratosRunning = false;
        } else {
            ...
            <div style="padding:1.5rem;display:flex;flex-direction:column;gap:1rem;">
                ...
                <div id="ca-extra-fields-desconto" style="display:none; padding:1rem; background:#f8fafc; border:1px dashed #cbd5e1; border-radius:8px;">
                    ...
                    <div style="display:flex; flex-direction:column; gap:0.6rem;">
                        <div>
                            <label style="font-size:0.75rem;font-weight:600;color:#475569;">Descri├º├úo do Desconto</label>
                            <input type="text" id="ca-desconto-descricao" placeholder="Ex: Multa de Tr├ónsito..." style="width:100%;padding:0.5rem;border:1px solid #cbd5e1;border-radius:6px;font-size:0.85rem;">
                        </div>
...
window.gerarContratoAvulso = async function () {
    ...
    try {
        ...
        if (gNome.toUpperCase().includes('DESCONTO EM FOLHA')) {
            requestBody.desconto_descricao = document.getElementById('ca-desconto-descricao')?.value || 'N├úo informado';
        ...
        // Sobrescreve o bot├úo Salvar para fazer upload ao prontu├írio
        setTimeout(() => {
...
window.previewAdmissaoDoc = async function (geradorId, colabId, evt) {
    ...
    if (!colabId) { alert('Colaborador n├úo identificado.'); return; }
    ...
    try {
        ...
        if (!data.html) {
            alert('N├úo foi poss├¡vel carregar o documento.');
            return;
        }

        // Reutiliza a fun├º├úo de preview do gerador (com layout completo)
        // Precisamos temporariamente salvar a sele├º├úo de assinatura para n├úo alterar o modal principal
        window.abrirPreviewDocumento(data);

        // Subscreve o evento de salvar na Admiss├úo para enviar ao backend
        setTimeout(() => {
            ...
            btnSalvar.onclick = async function () {
                ...
                try {
                    ...
                    if (!previewContent) throw new Error('Conte├║do do preview n├úo encontrado');

                    const pdfBlob = await window.gerarPDFBlob(previewContent);
                    const safeName = (data.gerador_nome || 'documento_admissao').replace(/[^a-zA-Z0-9├Ç-´┐¢ _-]/g, '');
                    ...
                    if (sendPrompt.isConfirmed) {
                        ...
                        showToast('Documento de admiss├úo salvo na pasta do colaborador.', 'success');
                    }
        ...
        alert('Erro ao carregar pr├®-visualiza├º├úo: ' + e.message);
    }
};
...
window.rodarDiagnosticoAssinafy = async function () {
    ...
    try {
        ...
        alert("Diagn├│stico completo! A tela seguinte mostrar├í o status exato.");
        alert(msg);
    } catch (e) {
...
window.sendAdmissaoSignatures = async function (listId = 'admissao-signature-list', btnId = 'btn-enviar-assinaturas') {
    ...
    if (!container) { alert('Lista de documentos n├úo encontrada.'); return; }
    ...
    // Dedup: garantir que n├úo h├í IDs duplicados
    ...
    try {
        ...
        let msg = `Ô£à ${ok.length} documento(s) enviado(s) para assinatura no e-mail do colaborador.`;
        if (erros.length > 0) msg += `\n\nÔÜá´©Å ${erros.length} erro(s):\n` + erros.map(e => `ÔÇó ${e.nome || e.id}: ${e.erro}`).join('\n');
        alert(msg);

        // Recarregar a aba em que o usu├írio est├í (Passo 2 Admiss├úo ou Contratos no Prontu├írio)
...
// Fun├º├úo para visualizar documentos assinados via tabela 'documentos' (ASO, EPI, Contratos)
...
// ===== INIT ADMISS├âO WORKFLOW =====
// Exibe o painel correto (bot├úo "Iniciar" ou stepper) dependendo do status do colaborador
window.initAdmissaoWorkflow = async function (colabId, step, silent) {
    ...
    try {
        ...
        if (!colab) { if (!silent) console.warn('[Admissao] Colaborador n├úo encontrado:', colabId); return; }

        // Atualiza refer├¬ncia global
        viewedColaborador = colab;

        // Pr├®-carrega documentos, assinaturas e geradores para o dashboard de admiss├úo funcionar sem depend├¬ncia da aba prontu├írio
        try {
            ...
            // Resolve Auto Geradores exactly like Prontu├írio Digital unified Contratos Tab
            ...
            const EXCLUIDOS_FIXOS = ['autoriza├º├úo de desconto em folha de pagamento', 'autorizacao de desconto em folha de pagamento', 'autorizar desconto', 'termo de responsabilidade de chaves'];
            const isExcluido = (g) => {
                const nLower = (g.nome || '').toLowerCase().trim();
            ...
            if (geradoresElegiveis.length > 0 && !geradoresElegiveis.some(g => g.visibilidade_regra)) {
                ...
                const c = colab;
                const LEGACY_MAP = [
                    { nome: 'Termo de N├âO Interesse Terapia', cond: deNorm(c.terapia_participa) === 'nao' },
                    { nome: 'Termo de Interesse Terapia', cond: deNorm(c.terapia_participa) === 'sim' },
                    { nome: 'Responsabilidade Bilhete ├Ünico', cond: (c.meio_transporte || '').toLowerCase().includes('vt') },
                    { nome: 'Responsabilidade Celular', cond: deNorm(c.celular_participa) === 'sim' },
                    { nome: 'Responsabilidade Chaves', cond: deNorm(c.chaves_participa) === 'sim' },
        ...
        } catch (e) { console.error('Erro ao pr├®-carregar depend├¬ncias da admiss├úo:', e); }
        ...
        // Se o processo j├í foi iniciado, mostrar o stepper
...
window.startFinalAdmission = async function () {
    ...
    console.log("[Admissao] Bot├úo 'Iniciar' clicado. viewedColaborador:", viewedColaborador);
    if (!viewedColaborador) {
...
window.handleAdmissaoFotoUpload = async function (event) {
    ...
    if (file.size > 5 * 1024 * 1024) {
        showToast('Foto muito grande! M├íximo 5MB.', 'error');
        return;
    }
    ...
    try {
        ...
        viewedColaborador.foto = result.file_path; // Simular que t├í com a foto preenchida (se o back atualiza o collab tamb├®m era ├│timo). O back j├í lida com os documentos tab_name FOTO_PERFIL ou a gente pode mandar o path e fazer um PUT no colab pra gravar "foto"
...
window.renderAdmissaoDataSummary = async function (colab) {
    ...
    try {
        ...
        let fieldsHtml = '';
        if (missing.length === 0) {
            fieldsHtml = `<div style="color:#059669; font-weight:700; margin-top:0.5rem;"><i class="ph ph-check-circle"></i> Todos os dados essenciais est├úo preenchidos!</div>`;
        } else {
        ...
        if (alertEl && listEl) {
            if (missing.length > 0) {
                ...
                listEl.innerHTML = missing.map(f => `<div>ÔÇó ${f}</div>`).join('');
...
window.saveAdmissaoResponsavel = async function (colabId, nomeResponsavel) {
    try {
        ...
        if (!res.ok) throw new Error('Falha ao salvar respons├ível');

        if (typeof Toastify !== 'undefined') {
            Toastify({ text: 'Respons├ível atribu├¡do com sucesso!', backgroundColor: '#059669' }).showToast();
        }
        ...
        console.error(e);
        alert('Erro ao salvar respons├ível.');
    }
};
...
function calculateAdmissaoStep1Completion(c) {
    // Checklist base ÔÇö todos os campos obrigat├│rios
    const baseChecklist = [
        ...
        { key: 'sexo', label: 'Sexo' },
        { key: 'cor_raca', label: 'Cor/Ra├ºa' },
        { key: 'nacionalidade', label: 'Nacionalidade' },
        { key: 'grau_instrucao', label: 'Grau Instru├º├úo' },
        { key: 'nome_mae', label: 'Nome M├úe' },
        { key: 'nome_pai', label: 'Nome Pai' },
        { key: 'rg_tipo', label: 'Tipo Doc' },
        { key: 'rg', label: 'N├║mero Doc' },
        { key: 'pis', label: 'PIS/PASEP' },
        { key: 'titulo_eleitoral', label: 'T├¡tulo Eleitoral' },
        { key: 'titulo_zona', label: 'Zona Eletr.' },
        { key: 'titulo_secao', label: 'Se├º├úo Eletr.' },
        { key: 'ctps_numero', label: 'CTPS N├║m.' },
        { key: 'ctps_serie', label: 'CTPS S├®rie' },
        ...
        { key: 'endereco', label: 'Endere├ºo' },
        { key: 'matricula_esocial', label: 'Matr├¡cula eSocial' },
        { key: 'cargo', label: 'Cargo' },
        { key: 'departamento', label: 'Departamento' },
        { key: 'cbo', label: 'CBO' },
        { key: 'data_admissao', label: 'Admiss├úo' },
        { key: 'tipo_contrato', label: 'Tipo Contrato' },
        { key: 'salario', label: 'Sal├írio Base' },
        { key: 'meio_transporte', label: 'Meio Transp.' },
        { key: 'adiantamento_salarial', label: 'Adiantamento' },
        { key: 'insalubridade', label: 'Insalubridade' },
        { key: 'escala_tipo', label: 'Escala Padr├úo' },
        { key: 'horario_entrada', label: 'Entrada' },
        { key: 'horario_saida', label: 'Sa├¡da' },
        { key: 'intervalo_entrada', label: 'Intervalo Ini' },
        { key: 'intervalo_saida', label: 'Intervalo Fim' },
        { key: 'banco_nome', label: 'Banco' },
        { key: 'banco_agencia', label: 'Ag├¬ncia' },
        { key: 'banco_conta', label: 'Conta' }
    ];

    let activeChecklist = [...baseChecklist];

    // Campos condicionais por tipo de documento (RG = exige ├│rg├úo e data; CIN/CNH = n├úo exige)
    const rgTipo = c.rg_tipo || 'RG';
    if (rgTipo === 'RG') {
        activeChecklist.push({ key: 'rg_orgao', label: '├ôrg├úo Emissor' });
        activeChecklist.push({ key: 'rg_data_emissao', label: 'Expedi├º├úo Doc' });
    }
    ...
    const filhos = depArr.filter(d => d.grau_parentesco !== 'C├┤njuge');

    if (isCasado) {
        activeChecklist.push({ key: 'conjuge_nome', label: 'Nome C├┤njuge' });
        activeChecklist.push({ key: 'conjuge_cpf', label: 'CPF C├┤njuge' });
    }
    ...
    // Adicionar blocos informativos (c├┤njuge/dependentes/valores) ÔÇö sem contar na porcentagem
    const extraFields = [];
    if (conjuge_nome) {
        extraFields.push({ label: 'C├┤njuge - Nome', value: conjuge_nome, filled: true, isExtra: true });
        extraFields.push({ label: 'C├┤njuge - CPF', value: conjuge_cpf, filled: !!conjuge_cpf, isExtra: true });
    }
    filhos.forEach((f, i) => {
    ...
    if (c.meio_transporte && c.meio_transporte !== 'Pr├│prio / A p├®') {
...
function updateAdmissaoStepPercentages(colab) {
    ...
    // ÔöÇÔöÇ Passo 1: Dados cadastrais ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
    const step1 = calculateAdmissaoStep1Completion(targetColab);
    const pc1 = step1.percent;

    // ÔöÇÔöÇ Passo 2: Santander ÔÇö 100% se ficha foi gerada ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
    const pc2 = targetColab.santander_ficha_data ? 100 : 0;

    // ÔöÇÔöÇ Passo 3: Assinaturas ÔÇö usa geradores/assinaturas carregados via Admiss├úo ÔöÇÔöÇÔöÇ
    let pc3 = 0;
    ...
    if (containerSignature) {
        ...
        if (combinedList.length === 0) {
            containerSignature.innerHTML = `<div style="padding:1rem; text-align:center; color:#64748b; font-size:0.85rem; font-style:italic;">Nenhum contrato configurado no sistema ou anexado no prontu├írio.</div>`;
        } else {
            containerSignature.innerHTML = combinedList.map(item => {
                ...
                if (hasFile && item.doc.created_at) {
                    ...
                    dataText = `<div style="font-size:0.75rem; color:#94a3b8; margin-top:2px;"><i>Upload obrigat├│rio via Prontu├írio Digital</i></div>`;
                }
    ...
    // ÔöÇÔöÇ Passo 4: Ficha Cadastral ÔÇö documentos do colaborador (01_FICHA_CADASTRAL) ÔöÇÔöÇ
    const _ec = (targetColab.estado_civil || '').toLowerCase();
    const _isCasado = _ec.includes('casad') || _ec.includes('vi├║v') || _ec.includes('viuv') || _ec.includes('divorc');
    const _certidao = _isCasado ? 'Certid├úo de Casamento' : 'Certid├úo de Nascimento';
    const _isMotorista2 = (targetColab.cargo || '').toUpperCase().includes('MOTORISTA');
    const fixed = [
        'T├¡tulo Eleitoral',
        _certidao,
        'Comprovante de endere├ºo',
        'Hist├│rico escolar'
    ];
    ...
    fixed.push('Carteira de vacina├º├úo', 'Curr├¡culo', 'Carteira de Trabalho');
    if (_isCasado) {
        fixed.push('CPF do C├┤njuge');
    }
    ...
    // Pens├úo (Din├ómico)
    const temPensaoPront = (targetColab.tem_pensao_alimenticia === 'Sim') || !!fichaDocs.find(d => d.document_type === 'Pens├úo Aliment├¡cia');
    if (temPensaoPront) fixed.push('Pens├úo Aliment├¡cia');
    ...
    const deps = depList.filter(d => d.grau_parentesco !== 'C├┤njuge');
    const hoje = new Date();
    ...
    // ÔöÇÔöÇ Passo 6: Contabilidade ÔÇö 100% se ficha enviada ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
    const pc6 = targetColab.admissao_contabil_enviada_em ? 100 : 0;

    // ÔöÇÔöÇ Passo 7: Efetiva├º├úo ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
...
window.toggleCheck = function (el) {
    // Desativado: sele├º├úo agora ├® apenas via upload
    console.log('Toggle desativado. Use o bot├úo de Upload.');
};
...
window.sendASOEmail = async function () {
    if (!viewedColaborador) {
        alert('Carregue um colaborador primeiro abrindo a edi├º├úo ou admiss├úo.');
        return;
    }
    ...
    const exames = examesCompl2 ? `Exame Padr├úo\nExames Complementares: ${examesCompl2}` : 'Exame Padr├úo';

    const mailBody = `T├¡tulo: Exame Admissional\n\nSegue abaixo as informa├º├Áes para a realiza├º├úo do exame Admissional do colaborador que deve comparecer.\n\nData: ${dt}\n\nNome: ${viewedColaborador.nome_completo}\nCPF: ${viewedColaborador.cpf}\nFun├º├úo: ${viewedColaborador.cargo || '-'}\nDepartamento: ${viewedColaborador.departamento || '-'}\n\nExames:\n${exames}\n\nÔÜá´©Å IMPORTANTE:\nAp├│s o exame ficar pronto, favor enviar o documento por e-mail diretamente para: rh@americarental.com.br`;
    ...
    try {
        ...
        if (confirm(`N├úo foi poss├¡vel enviar automaticamente pelo servidor. Erro do Servidor:\n\n${e.message}\n\nDeseja abrir o seu programa de e-mail (Outlook/Gmail) com o texto j├í preenchido?`)) {
...
window.finalizarAdmissao = async function () {
    ...
    if (!confirm(`Confirmar a admiss├úo definitiva de ${viewedColaborador.nome_completo}?\n\nO colaborador passar├í para o status "Em Integra├º├úo".`)) return;

    try {
        await apiPut(`/colaboradores/${viewedColaborador.id}`, {
            status: 'Em Integra├º├úo',
            admissao_status: 'Conclu├¡da'
        });

        // Atualizar o objeto local
        viewedColaborador.status = 'Em Integra├º├úo';
        if (viewedColaborador) viewedColaborador.status = 'Em Integra├º├úo';

        // Toast de sucesso
        if (typeof admissaoToast === 'function') {
            admissaoToast(`Ô£à ${viewedColaborador.nome_completo} admitido com sucesso! Agora em Integra├º├úo.`, 'success');
        } else {
            alert('Admiss├úo realizada com sucesso! O colaborador agora est├í Em Integra├º├úo.');
        }

        // Navegar para m├│dulo de integra├º├úo
        setTimeout(() => {
        ...
        alert('Erro ao finalizar admiss├úo: ' + e.message);
    }
};


/**
 * Copia o link de assinatura para a ├írea de transfer├¬ncia
 */
...
window.iniciarAssinafy = async function (docType, tabName, btn) {
    ...
    try {
        ...
        if (res.sucesso) {
            ...
            if (docInfoDiv) {
                // Atualizar ou criar o par├ígrafo de data de envio
                ...
                // Link de assinatura em par├ígrafo separado abaixo
        ...
        if (e.name === 'AbortError') {
            alert('ÔÅ│ O processo est├í demorando mais que o esperado.\n\nO Assinafy pode j├í ter processado o documento. Aguarde 1 minuto e recarregue a p├ígina para verificar o status.');
        } else {
...
window.syncAssinafyStatus = async function (docId, btn) {
    ...
    try {
        ...
        console.error('Erro requisi├º├úo sync Assinafy:', err);
    } finally {
        // Redesenho da aba reescrever├í o icone, mas por cautela revertermos a animacao se falhar
        if (icon) {
...
window.forceOnedriveSync = async function (docId, btn) {
    ...
    try {
        ...
        if (data.sucesso) {
            alert(`Ô£à Sincronizado com sucesso!\n\nArquivo: ${data.cloudName}\nPasta: ${data.targetDir}\n\n--- LOG ---\n${logText}`);
        } else {
            alert(`ÔØî Falha na sincroniza├º├úo OneDrive:\n${data.error}\n\n--- LOG ---\n${logText}`);
        }
    } catch (e) {
...
window.syncAllAtestados = async function (ids, btn) {
    ...
    btn.disabled = false;
    btn.innerHTML = original;
    alert(`Ô£à Sincroniza├º├úo conclu├¡da!\nÔ£ô Sucesso: ${ok}\nÔ£ù Falha: ${fail}\n\nUse o bot├úo Ôÿü individual para ver o log de cada falha.`);
};
    ...
    try {
        ...
        if (data.sucesso) {
            ...
            if (data.rhLocation) {
                ...
                gpsRH = `\n\nÔÜá´©Å PASTA 'RH' ENCONTRADA EM OUTRO LUGAR:\nEndere├ºo: ${data.rhLocation.webUrl}\nID Drive: ${dId}`;
            }

            let msg = `Ô£à O OneDrive est├í CONECTADO corretamente!\n\n` +
            ...
            let errorMsg = `ÔØî ${data.error}\n`;
            if (data.code) errorMsg += `C├│digo: ${data.code}\n`;
            if (data.details) errorMsg += `Detalhes: ${JSON.stringify(data.details)}`;
            alert(errorMsg);
        }
    } catch (e) {
        alert("Erro na requisi├º├úo: " + e.message);
    } finally {
        btn.disabled = false;
        btn.innerHTML = originalHtml;
    }
...
window.syncOneDriveManual = async function (id, btnElement = null) {
    // Se n├úo passou o elemento, tenta achar pelos IDs conhecidos
    ...
    try {
        ...
        if (data.sucesso) {
            alert(`Ô£à SUCESSO TOTAL!\nCaminho: ${data.path}`);
        } else {
            alert(`ÔØî Erro na Sincroniza├º├úo:\n${data.message || data.error}`);
        }
    } catch (e) {
        alert("Erro na requisi├º├úo: " + e.message);
    } finally {
...
window.resetSystem = async function () {
    const confirmation1 = confirm("­ƒÜ¿ ATEN├ç├âO: Voc├¬ tem certeza que deseja LIMPAR TODOS os colaboradores do sistema?\n\nIsso apagar├í todos os dados do banco de dados (dependentes, fotos, documentos registrados). Os arquivos f├¡sicos no OneDrive n├úo ser├úo apagados por seguran├ºa.");
    if (!confirmation1) return;

    const confirmation2 = confirm("CONFIRMA├ç├âO FINAL: Deseja realmente excluir permanentemente todos os registros de colaboradores?");
    ...
    try {
        ...
        if (data.sucesso) {
            alert("Sistema limpo com sucesso! A p├ígina ser├í recarregada.");
            location.reload();
        } else {
                    ...
                    <div style="display:flex;align-items:flex-start;gap:1rem;">
                        ...
                        <div style="flex:1;">
                            <div style="font-weight:700;font-size:0.9rem;color:#0f172a;margin-bottom:4px;">
                                <i class="ph ph-bell-ringing" style="color:#dc2626;"></i> Notifica├º├úo
                            </div>
...
// --- POLLING: Notifica├º├Áes de Formul├írio de Experi├¬ncia (para usu├írios RH) ---
            ...
            try {
                ...
                popup.innerHTML = `
                    <div style="display:flex;align-items:flex-start;gap:1rem;">
                        ...
                        <div style="flex:1;">
                            <div style="font-weight:700;font-size:0.9rem;color:#0f172a;margin-bottom:4px;">
                                <i class="ph ph-user-check" style="color:#1d4ed8;"></i> Formul├írio de Experi├¬ncia Preenchido
                            </div>
                            <div style="color:#1d4ed8;font-weight:600;font-size:0.95rem;margin-bottom:4px;">${dados.colaborador_nome || 'Colaborador'}</div>
                            <div style="color:#64748b;font-size:0.8rem;">
                                ${dados.departamento ? dados.departamento + ' ┬À ' : ''}
    ...
    try {
        ...
        for (const notif of notificacoes) {
            if (!_comNotifSeen.has(notif.id)) {
                ...
                const remetente = dados.remetente || 'Log├¡stica';
                const clienteNome = dados.cliente_nome || 'Cliente n├úo informado';
                ...
                popup.innerHTML = `
                    <div style="display:flex;align-items:flex-start;gap:1rem;">
                        ...
                        <div style="flex:1;">
                            ...
                            <div style="color:#7048e8;font-weight:600;font-size:0.95rem;margin-bottom:4px;">${dados.cliente_nome || 'Cliente n├úo informado'}</div>
                            <div style="color:#64748b;font-size:0.8rem;">
        ...
        for (const notif of notifs) {
            ...
            try {
                ...
                if (notif.tipo === 'aviso_faltas') {
                    ...
                    bg = '#fef2f2'; color = '#ef4444'; icon = 'ph-warning-circle'; titulo = 'Licen├ºa Vencida'; navTarget = 'admin'; // Navigates to admin panel or wherever licencas are managed
                } else if (notif.tipo === 'formulario_experiencia') {
                    bg = '#fdf2f8'; color = '#ec4899'; icon = 'ph-clipboard-text'; titulo = 'Experi├¬ncia'; navTarget = 'experiencia';
                } else if (notif.tipo === 'novo_sinistro') {
                    bg = '#dcfce7'; color = '#059669'; icon = 'ph-warning'; titulo = 'Novo Sinistro (Log├¡stica)'; navTarget = 'colaboradores';
                } else if (notif.tipo === 'estoque_minimo') {
                    bg = '#fff5e6'; color = '#e67700'; icon = 'ph-package'; titulo = 'Estoque M├¡nimo'; navTarget = 'estoque';
                } else if (notif.tipo === 'novo_colaborador_equipe') {
                    bg = '#fdf2f8'; color = '#ec4899'; icon = 'ph-user-plus'; titulo = 'Novo Colaborador para Distribui├º├úo'; navTarget = 'logistica-equipes';
                } else if (notif.tipo === 'nova_ocorrencia') {
                    bg = '#fdf2f8'; color = '#ec4899'; icon = 'ph-warning-octagon'; titulo = 'Ocorr├¬ncia Registrada'; navTarget = 'dashboard';
                } else {
                    bg = '#f1f5f9'; color = '#475569'; icon = 'ph-bell-ringing'; titulo = 'Notifica├º├úo'; navTarget = 'dashboard';
                }
                ...
                let contentHTML = '';
                if (notif.tipo === 'aviso_faltas') {
                    const nomeColab = dados.nome_colab || 'Colaborador n├úo identificado';
                    ...
                    contentHTML = `
                        ...
                        <div style="color:#64748b;font-size:0.85rem;">O colaborador <b>${colabNome}</b> entrou de f├®rias e foi retirado da equipe, deixando a equipe desfalcada com apenas 1 membro.</div>
                    `;
                } else if (notif.tipo === 'licenca_vencida') {
                    const nomeLicenca = dados.nome_licenca || 'Licen├ºa desconhecida';
                    ...
                    contentHTML = `
                        ...
                        <div style="color:#64748b;font-size:0.85rem;">Um novo boletim de ocorr├¬ncia foi anexado.</div>
                    `;
                } else if (notif.tipo === 'estoque_minimo') {
                    const nomeProduto = dados.nome || (notif.mensagem || '').replace(/^ESTOQUE BAIXO:\s*/i, '').split('(')[0].trim();
                    const qtdAtual = dados.quantidade_atual !== undefined ? dados.quantidade_atual : 'ÔÇö';
                    const qtdMin   = dados.quantidade_minima !== undefined ? dados.quantidade_minima : 'ÔÇö';
                    contentHTML = `
                        ...
                        <div style="display:flex;gap:12px;font-size:0.82rem;">
                            ...
                            <span style="color:#64748b;">M├¡nimo: ${qtdMin}</span>
                        </div>
                    `;
                } else if (notif.tipo === 'formulario_experiencia') {
                    ...
                    contentHTML = `
                        ...
                        <div style="color:#64748b;font-size:0.85rem;">O colaborador <b style="color:${color}">${colabNome}</b> ├® um novo colaborador para distribui├º├úo de equipe.</div>
                    `;
                } else {
                    contentHTML = `
                        <div style="font-weight:700;font-size:0.9rem;color:#0f172a;margin-bottom:4px;">
...
async function renderPdfToContainer(pdfUrl, containerId, onScrollEnd) {
    ...
    try {
        if (!window.pdfjsLib) throw new Error('Biblioteca pdf.js n├úo carregada');
        ...
        container.innerHTML = '<div style="color:red; padding:2rem;">Erro ao carregar renderiza├º├úo do PDF: ' + e.message + '</div>';
        onScrollEnd();
    }
}
...
// --- L├ôGICA ASSINATURA TESTEMUNHAS ---
let ctxTestemunhas = {};
...
window.abrirModalAssinaturaTestemunhas = async function (docId) {
    ...
    // Mostrar todos os colaboradores cadastrados como poss├¡veis testemunhas
...
window.salvarAssinaturasTestemunhas = async function () {
    ...
    if (!doc || !doc.file_path) { alert('Documento original n├úo encontrado.'); return; }
    if (typeof PDFLib === 'undefined') { alert('A biblioteca de processamento de PDF n├úo est├í carregada.'); return; }
    ...
    try {
        ...
        if (!pdfResp.ok) throw new Error('N├úo foi poss├¡vel baixar o PDF original.');
        const existingPdfBytes = await pdfResp.arrayBuffer();

        const pdfDoc = await PDFLib.PDFDocument.load(existingPdfBytes);

        const pages = pdfDoc.getPages();
        // Usa SEMPRE a ├║ltima p├ígina para as assinaturas das testemunhas
        ...
        // --- Captura canvas em alta resolu├º├úo (3x DPI) ---
        ...
        // ÔöÇÔöÇ Posicionamento ancorado no RODAP├ë da ├║ltima p├ígina ÔöÇÔöÇ
        // TESTEMUNHAS ficam na faixa de ~150 a ~270pt do rodap├®
        // (abaixo delas ficar├í o bloco do Colaborador, em ~30 a ~130pt)
        const tImgH    = 55;  // altura da imagem de assinatura
        const bottomMargin = 160; // margem do rodap├® para o bloco das testemunhas
        ...
        // ÔòÉÔòÉ TESTEMUNHA 1 (Esquerda) ÔòÉÔòÉ
        const t1X = 56;
        ...
        // ÔòÉÔòÉ TESTEMUNHA 2 (Direita) ÔòÉÔòÉ
        ...
        const formData = new FormData();
        formData.append('document_id', doc.id); // For├ºa UPDATE em vez de INSERT
        formData.append('file', file);
        formData.append('colaborador_id', viewedColaborador.id);
        formData.append('colaborador_nome', viewedColaborador.nome_completo || 'Desconhecido');
...
// --- L├ôGICA ASSINATURA COLABORADOR (Advert├¬ncia) ---
let ctxColaborador = {};
let currentDocIdForColab = null;

// ÔöÇÔöÇÔöÇ SELFIE EPI ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
...
async function _epiIniciarCamera() {
    ...
    try {
        // Preferir c├ómera frontal (user-facing)
        ...
        if (statusEl) statusEl.textContent = 'C├ómera pronta. Posicione seu rosto.';
    } catch (err) {
        console.error('[EPI Selfie] C├ómera:', err);
        if (statusEl) statusEl.innerHTML = '<span style="color:#dc2626;"><i class="ph ph-warning"></i> N├úo foi poss├¡vel acessar a c├ómera. Verifique as permiss├Áes do navegador.</span>';
...
window._epiTirarFoto = function () {
    ...
    // Captura o frame do v├¡deo no canvas ÔÇö espelhado para ficar natural
    canvas.width = video.videoWidth || 640;
    canvas.height = video.videoHeight || 480;
    const ctx = canvas.getContext('2d');

    // Espelha horizontalmente (pois o v├¡deo est├í flipado via CSS; queremos salvar sem flip)
    ctx.save();
    ...
    // Mostrar canvas com foto, esconder v├¡deo
...
window.abrirModalAssinaturaColaborador = async function (docId) {
    ...
    renderPdfToContainer(pdfUrl, 'pdf-viewer-colaborador', () => {
        // Ap├│s PDF carregado, iniciar c├ómera e mostrar passo 1
...
window.salvarAssinaturaColaborador = async function () {
    if (!_epiSelfieBase64) {
        alert('├ë necess├írio tirar a selfie antes de assinar.'); return;
    }
    if (isCanvasBlank('canvas-colaborador')) {
        alert('A assinatura do colaborador ├® obrigat├│ria.'); return;
    }

    const doc = currentDocs.find(d => d.id === currentDocIdForColab);
    if (!doc || !doc.file_path) { alert('Documento n├úo encontrado.'); return; }
    ...
    try {
        ...
        const pdfDoc = await PDFLib.PDFDocument.load(existingPdfBytes);
        const pages = pdfDoc.getPages();
        // Usa SEMPRE a ├║ltima p├ígina, igual ├ás testemunhas
        ...
        // Testemunhas ocupam a faixa 160-260pt do rodap├®.
        // Colaborador fica na faixa 30-130pt do rodap├®, sem sobreposi├º├úo.
        const cImgH = 55;
        const cWidth = 280;
        ...
        if (doc.created_at || doc.data_inclusao) {
            ...
            if (!isNaN(dtCriado)) {
                ...
                lastPage.drawText(`Inclu├¡do no sistema em: ${dateStrCriado}`, { x: cX, y: cCpfY - 24, size: 8, color: PDFLib.rgb(0.35, 0.35, 0.35) });
            }
        }
...
window._excluirEpiEntrega = async function(id, nome, btnEl, ficha_id) {
    ...
    const senha = prompt('Digite a senha para autorizar a exclus├úo:');
...
// ============================================================
// ABA FICHA DE EPI NO PRONTU├üRIO
// ============================================================
async function renderFichaEpiTab(container) {
    container.innerHTML = '<p class="text-muted">Carregando fichas de EPI...</p>';
    const colabId = viewedColaborador?.id;
    if (!colabId) { container.innerHTML = '<div class="alert alert-info">Colaborador n├úo identificado.</div>'; return; }
    ...
    if (!fichas || !templates) {
        container.innerHTML = `
            <div style="text-align:center; padding: 40px 20px; color: #64748b;">
                ...
                <p style="font-size:1rem; font-weight:600; color:#1e3a5f; margin-bottom:8px;">N├úo foi poss├¡vel carregar os dados de EPI</p>
    ...
    const SETORES_ADMIN = ['Comercial', 'Financeiro', 'Log├¡stica', 'Logistica', 'Administrativo', 'RH'];
    const isSetorAdmin = SETORES_ADMIN.includes(dept) || SETORES_ADMIN.includes(cargo);

    // Procura template por pontua├º├úo: cargo tem prioridade sobre departamento para evitar matches incorretos
    ...
    // Monta tabela de hist├│rico de entregas
    const hoje = new Date(); hoje.setHours(0, 0, 0, 0);
    let tabelaHtml = '';
    if (todasEntregas && todasEntregas.length > 0) {
        ...
        const linhas = todasEntregas.map(e => {
            ...
            return `
                <tr style="border-bottom:1px solid #f1f5f9;${recente ? 'background:#fffbeb;' : ''}">
                    ...
                    <td style="padding:0.55rem 0.85rem;font-size:0.85rem;color:#0f172a;font-weight:500;">${e.epi_nome || 'ÔÇö'}</td>
                    ...
                    <td style="padding:0.55rem 0.85rem;font-size:0.8rem;color:#64748b;">${e.grupo || 'ÔÇö'}</td>
                    <td style="padding:0.55rem 0.85rem;font-size:0.8rem;color:#64748b;">${e.registrado_por || 'ÔÇö'}</td>
        ...
        // ÔöÇÔöÇ Fun├º├úo: ver selfie de entrega de EPI ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
        window._verSelfieEpiEntrega = async function(colaboradorId, dataEntrega) {
            ...
            lb.innerHTML = `<div style="background:#1e293b;border-radius:16px;padding:2rem;min-width:280px;text-align:center;color:#e2e8f0;">
                <div style="font-size:2rem;display:block;margin-bottom:0.5rem;">ÔÅ│</div>
            ...
            try {
                ...
                if (!selfies || selfies.length === 0) {
                    lb.innerHTML = `<div style="background:#1e293b;border-radius:16px;padding:2rem;min-width:280px;max-width:480px;text-align:center;color:#e2e8f0;position:relative;">
                        ...
                        <div style="font-size:3rem;margin-bottom:0.75rem;">­ƒôÀ</div>
                        <p style="font-weight:700;margin:0 0 4px;">Nenhuma selfie encontrada</p>
                        <p style="font-size:0.8rem;color:#94a3b8;margin:0;">Esta entrega n├úo possui selfie registrada.<br>As selfies s├úo salvas nas entregas a partir de agora.</p>
                    </div>`;
                    return;
                }

                // Selfie mais pr├│xima da data de entrega (DD/MM/YYYY)
                ...
                <div style="background:#0f172a;border-radius:16px;overflow:hidden;max-width:560px;width:100%;position:relative;box-shadow:0 25px 60px rgba(0,0,0,0.7);">
                    <div style="background:#1e3a5f;padding:0.85rem 1.25rem;display:flex;align-items:center;justify-content:space-between;">
                        <div style="display:flex;align-items:center;gap:8px;">
                            ...
                            <div>
                                ...
                                <div style="color:#93c5fd;font-size:0.75rem;">Entrega em ${dataEntrega || 'ÔÇö'}</div>
                            </div>
                        </div>
                    ...
                    <div style="padding:0.75rem 1.25rem;background:#1e293b;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:0.5rem;">
                        <div style="font-size:0.78rem;color:#94a3b8;">
                            <span style="color:#fbbf24;font-weight:600;">Registrado por:</span> ${selfieAlvo.registrado_por || 'ÔÇö'}
                        </div>
                ...
                lb.innerHTML = `<div style="background:#1e293b;border-radius:16px;padding:2rem;min-width:280px;max-width:480px;text-align:center;color:#e2e8f0;position:relative;">
                    <button onclick="document.getElementById('epi-selfie-lightbox').remove()" style="position:absolute;top:12px;right:14px;background:rgba(255,255,255,0.15);border:none;color:#fff;width:30px;height:30px;border-radius:50%;cursor:pointer;font-size:1.1rem;">&times;</button>
                    <div style="font-size:3rem;margin-bottom:0.75rem;">ÔÜá´©Å</div>
        ...
        <div style="margin-top:2rem;">
            <div style="display:flex;align-items:center;gap:0.6rem;margin-bottom:0.85rem;padding-bottom:0.5rem;border-bottom:2px solid #e2e8f0;">
                ...
                <h4 style="margin:0;font-size:0.97rem;font-weight:700;color:#0f172a;">Hist├│rico de EPIs Entregues</h4>
                ...
                <table style="width:100%;border-collapse:collapse;">
                    <thead>
                        <tr style="background:#f8fafc;">
                            ...
                            <th style="padding:0.6rem 0.85rem;font-size:0.78rem;font-weight:700;color:#475569;text-align:center;">A├º├Áes</th>
                        </tr>
                    </thead>
                    <tbody>${linhas}</tbody>
                </table>
    ...
    container.innerHTML = `
        ...
        <div style="background:#fff7ed;border:1.5px solid #fed7aa;border-radius:10px;padding:1rem 1.25rem;margin-bottom:1.25rem;display:flex;justify-content:space-between;align-items:center;gap:1rem;flex-wrap:wrap;">
            <div style="display:flex;align-items:center;gap:0.75rem;">
                ...
                <p style="margin:0;font-size:0.88rem;color:#92400e;">Nenhuma ficha ativa dispon├¡vel para ${cargo || dept}.</p>
            </div>
            ${templateDoColab ? `
...
window.excluirFichaEpi = async function (fichaId, grupo) {
    if (!confirm(`Tem certeza que deseja excluir a Ficha: ${grupo}?\n\nEsta a├º├úo tamb├®m remover├í todos os registros de entrega vinculados a esta ficha e n├úo pode ser desfeita.`)) return;
...
window.gerarFichaEpiManualProntuario = async function (templateId) {
    if (!confirm('Deseja gerar a Ficha de EPI para esse colaborador usando o template padr├úo vinculado a este cargo?')) return;
    ...
    if (!template) return alert('Template inv├ílido.');
    ...
    try {
        ...
        // Recarrega a aba para exibir a ficha ativa rec├®m-criada
...
// Navega para step (num├®ricos: 1 e 3; selfie tem controle pr├│prio)
window._assinStep = function (n) {
    ...
    if (n === 1) {
        ...
        const btnNext = document.getElementById('btn-assin-next');
        if (btnNext) { btnNext.style.display = 'flex'; btnNext.innerHTML = 'Pr├│ximo <i class="ph ph-arrow-right"></i>'; }
    } else if (n === 3) {
    ...
    if (n === 2) {
        ...
        if (ul) {
            ...
            // Agrupar itens iguais: ["CAMISETA - G", "CAMISETA - G"] ÔåÆ "2x CAMISETA - G"
            const grouped = {};
...
// Bot├úo "Pr├│ximo / Confirmar / Fechar"
window._assinNextStep = async function () {
    ...
    if (step === 1) {
        ...
        try {
            ...
            if (respEnd.ok) {
                ...
                if (perguntas.length > 0) {
                    ...
                    html += '<p style="margin-top:0;">Onde voc├¬ est├í retirando os seguintes itens?</p>';
            ...
            console.error('Erro ao buscar endere├ºos:', e);
        }
        ...
        if (nextBtn) nextBtn.style.display = 'none'; // selfie usa bot├Áes pr├│prios
        window._epiAssinIniciarCamera();
        return;
    }

    if (step === 2) {
        // Valida selfie obrigat├│ria
        ...
        // Verifica EPIs entregues h├í menos de 15 dias (aviso j├í exibido no step 1 ao clicar +)
        // Captura assinatura
        ...
        try {
            ...
            // Timeout de 30s para evitar que o bot├úo fique preso em "Salvando..."
            ...
            let securityData = { gps_lat: '', gps_lon: '', dispositivo: navigator.userAgent };
            if (window.getDeviceSecurityData) {
                try {
                    securityData = await window.getDeviceSecurityData();
                } catch(e) { console.error('Erro ao obter GPS', e); }
            }
            ...
            const resp = await fetch(`${API_URL}/epi-fichas/${window._assinFichaId}/entregas`, {
                ...
                body: JSON.stringify({
                    ...
                    gps_lat: securityData.gps_lat,
                    gps_lon: securityData.gps_lon,
                    dispositivo: securityData.dispositivo
                })
            });
            ...
            (async () => {
                try {
                    ...
                    if (fich && cid) {
                        ...
                        // Buscar TODAS as entregas j├í registradas para esta ficha (hist├│rico completo)
            ...
            // Recarrega a aba ap├│s 2s e abre o PDF na tela
            setTimeout(() => {
                const old = document.getElementById('epi-assinatura-overlay');
                if (old) old.remove();
            ...
            const msg = err.name === 'AbortError'
                ? 'O servidor demorou muito para responder (30s). Verifique se o servidor est├í online e tente novamente.'
...
window.gerarNovaFichaEpi = async function () {
    ...
    if (fichaAtiva) {
        const ok = confirm(`J├í existe uma ficha ativa (${fichaAtiva.grupo}). Deseja fechar a atual e criar nova?`);
        if (!ok) return;
    }
    ...
    // Abre a visualiza├º├úo automaticamente ap├│s criar a nova ficha (assinada)
    if (created.id) {
...
// ==========================================
// PRONTU├üRIO TABS SEARCH FILTER
// ==========================================
