...
if 'verificarENotificarEstoqueMinimoPorEndereco' not in code:
    code = code.replace('// POST: registrar entrega assinada de EPIs', helper_code + '\n// POST: registrar entrega assinada de EPIs')


old_baixa = re.search(r'function processarBaixaEstoque\(item, count, enderecoId, enderecoNome\) \{.*?\/\/ Carregar todo o estoque para a memória', code, re.DOTALL)
new_baixa = '''function processarBaixaEstoque(item, count, enderecoId, enderecoNome) {
...
if old_baixa:
    code = code.replace(old_baixa.group(0), new_baixa)


old_manual = re.search(r'db\.run\(\'UPDATE estoque SET quantidade_atual = quantidade_atual - \?, atualizado_em = CURRENT_TIMESTAMP WHERE id = \?\', \[qtd, id\], \(errU\) => \{.*?res\.json\(\{ success: true \}\);\s*\}\);', code, re.DOTALL)
new_manual = '''db.run('UPDATE estoque SET quantidade_atual = quantidade_atual - ?, atualizado_em = CURRENT_TIMESTAMP WHERE id = ?', [qtd, id], (errU) => {
...
if old_manual:
    code = code.replace(old_manual.group(0), new_manual)


old_mov = re.search(r'db\.run\(\'UPDATE estoque SET quantidade_atual = MAX\(0, quantidade_atual \+ \?\), atualizado_em = CURRENT_TIMESTAMP WHERE id = \?\', \[qtd, id\], \(errU\) => \{.*?res\.json\(\{ success: true \}\);\s*\}\);', code, re.DOTALL)
new_mov = '''db.run('UPDATE estoque SET quantidade_atual = MAX(0, quantidade_atual + ?), atualizado_em = CURRENT_TIMESTAMP WHERE id = ?', [qtd, id], (errU) => {
            if (errU) return res.status(500).json({ error: errU.message });
            if (endereco_id) {
                db.get('SELECT s.quantidade, s.quantidade_minima, ee.tipo_notificacao, ee.nome as e_nome FROM estoque_saldo_por_endereco s JOIN estoque_enderecos ee ON ee.id = s.endereco_id WHERE s.estoque_id = ? AND s.endereco_id = ?', [id, endereco_id], (errS, rowS) => {
                    const oldSaldo = rowS ? rowS.quantidade : 0;
...
if old_mov:
    code = code.replace(old_mov.group(0), new_mov)

with open('backend/server.js', 'w', encoding='utf-8') as f:
    f.write(code)

print("Patch applied using python")
