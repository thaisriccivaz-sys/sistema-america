...
if (formColab) {
    formColab.addEventListener('submit', async (e) => {
        ...
        const cnhDocInput = document.getElementById('colab-cnh-documento');
        const cnhFile = cnhDocInput ? cnhDocInput.files[0] : null;
        const cnhDocId = document.getElementById('colab-cnh-doc-id') ? document.getElementById('colab-cnh-doc-id').value : '';

        // Validação de Motorista
        if (data.cargo.toUpperCase().includes('MOTORISTA')) {
            if (!isPartial) {
                if (!data.cnh_numero || !data.cnh_vencimento || !data.cnh_categoria) {
                    alert('Preenchimento Obrigatório: Dados da CNH para Motorista não podem ficar vazios.');
                    btnRestorer();
                    return;
                }
                ...
                if (!cnhDocId && !cnhFile) {
                    alert('O envio do Documento da CNH em formato PDF é obrigatório para Motoristas no ato do cadastro/alteração.');
                    btnRestorer();
                    return;
                }
            }
        ...
        try {
            ...
            if ((data.estado_civil === 'Casado' || data.estado_civil === 'União Estável') && colabId) {
                ...
                // SUBIR PDF DO CONJUGE (CASO PREENCHIDO)
                if (conjDoc) {
                    const safeConjNome = formatStringGlobal(conjNome);
                    const docData = new FormData();
                    docData.append('colaborador_id', colabId);
                    docData.append('colaborador_nome', data.nome_completo);
                    docData.append('tab_name', '03_Dependentes');
                    docData.append('sub_folder', 'CONJUGE_' + safeConjNome); 
                    docData.append('document_type', 'Documento_Identidade');
                    docData.append('custom_name', `DOCUMENTO_${safeConjNome}`);
                    docData.append('file', conjDoc);
                    ...
                    const conjInput = document.getElementById('conjuge-documento');
            ...
            // SUBIR PDF DA CNH (MOTORISTA)
            if (cnhFile && data.cargo.toUpperCase().includes('MOTORISTA')) {
                const safeColabNomeTratado = formatStringGlobal(data.nome_completo);
                const docDataCnh = new FormData();
                docDataCnh.append('colaborador_id', colabId);
                docDataCnh.append('colaborador_nome', data.nome_completo);
                docDataCnh.append('tab_name', '01_Documentos_Pessoais');
                docDataCnh.append('document_type', 'CNH');
                docDataCnh.append('custom_name', `CNH_${safeColabNomeTratado}`);
                docDataCnh.append('file', cnhFile);
                
                await fetch(`${API_URL}/documentos`, {
                    method: 'POST', body: docDataCnh, headers: { 'Authorization': `Bearer ${currentToken}` }
                });
                
                const cnhInput = document.getElementById('colab-cnh-documento');
...
window.renderTabContent = function(tabId, tabTitle) {
    ...
    } else if (tabId === 'Dependentes' || tabId === 'Treinamento' || tabId === 'Multas') {
        const btnLabelMap = { 'Dependentes': 'Documento de Dependente', 'Treinamento': 'Certificado/Curso', 'Multas': 'Multa / Doc Adicional' };
        ...
        vencimentoInputHtml = `
            <div style="display: flex; flex-direction: column; gap: 0.2rem; margin-right: 0.5rem;">
                ...
                <div style="display:flex; gap:0.25rem; align-items: center;">
                    ...
                    ${isSaved ? `
                        <button type="button" class="btn btn-secondary btn-sm" onclick="saveVencimento(${existingDoc.id}, 'venc-${tabId}-${docType}')" title="Salvar alteração de validade" style="padding:0; width: 42px; height: 42px; justify-content: center;">
...
window.renderASOTab = function(container, filteredDocs) {
    ...
    if (viewedColaborador && viewedColaborador.admissao) {
        const adm = new Date(viewedColaborador.admissao + 'T12:00:00');
