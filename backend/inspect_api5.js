...
async function t(){ 
  let fd = new FormData(); fd.append('file', fs.readFileSync('test_2pages.pdf'), {filename:'t.pdf'}); fd.append('require_initials', 'false'); fd.append('has_initials', 'false'); 
  let dr = await uploadForm('/v1/accounts/'+ACCOUNT_ID+'/documents', fd); 
  let dId = JSON.parse(dr).data.id; 
  ...
  let sId = JSON.parse(s_res).data[0].id; 
  let a = await req('POST','/v1/documents/'+dId+'/assignments', { signers: [{id: sId, role: 'signer', notification_methods: ['Email']}], method: 'virtual'}); 
  let r = JSON.parse(a).data.items; 
  console.log('Result Length:', r?.length, r?.map(x=>x.field.type)); 
} t();
