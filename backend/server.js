...
// -- GET /api/rh/escala -- Escala para o RH (Operacional + Administrativo)
app.get('/api/rh/escala', authenticateToken, (req, res) => {
    ...
    db.all(SELECT id, nome_completo, cargo, departamento, foto_base64, foto_path, aso_exame_data,
        ...
        const p1 = new Promise(resolve => {
            db.all(SELECT id, ferias_programadas_inicio, ferias_programadas_fim FROM colaboradores WHERE id IN ( + '' + ) AND ferias_programadas_inicio IS NOT NULL AND ferias_programadas_inicio != '', ids, (e, rows) => {
                const s = {};
                (rows||[]).forEach(r => { if(!r.ferias_programadas_inicio||!r.ferias_programadas_fim)return; let cur=new Date(r.ferias_programadas_inicio+'T12:00:00'); const end=new Date(r.ferias_programadas_fim+'T12:00:00'); while(cur<=end){const d=cur.toISOString().split('T')[0];if(!s[r.id])s[r.id]={};s[r.id][d]='ferias';cur.setDate(cur.getDate()+1);} });
                resolve(s);
            });
        });
        const p2 = new Promise(resolve => {
            db.all(SELECT colaborador_id, atestado_inicio, atestado_fim FROM documentos WHERE colaborador_id IN ( + '' + ) AND (tab_name LIKE '%ATESTADO%' OR document_type LIKE '%Atestado%' OR tab_name='Atestados') AND atestado_inicio IS NOT NULL AND atestado_fim IS NOT NULL, ids, (e, rows) => {
                const s = {};
                (rows||[]).forEach(r => { let cur=new Date(r.atestado_inicio+'T12:00:00'); const end=new Date(r.atestado_fim+'T12:00:00'); while(cur<=end){const d=cur.toISOString().split('T')[0];if(!s[r.colaborador_id])s[r.colaborador_id]={};s[r.colaborador_id][d]='afastado';cur.setDate(cur.getDate()+1);} });
                resolve(s);
            });
        });
        const p3 = new Promise(resolve => {
            db.all(SELECT colaborador_id, data_falta FROM faltas WHERE colaborador_id IN ( + '' + ) AND data_falta >= ? AND data_falta <= ?, [...ids, inicio, fim], (e, rows) => {
                const s = {}; (rows||[]).forEach(r=>{if(!s[r.colaborador_id])s[r.colaborador_id]={};s[r.colaborador_id][r.data_falta]='falta';}); resolve(s);
            });
        });
        Promise.all([p1,p2,p3]).then(([ferSet,atestSet,faltSet]) => {
            const datas = [];
            let cur = new Date(inicio+'T12:00:00'); const endD = new Date(fim+'T12:00:00');
            ...
            const result = (colabs||[]).map(c => {
                const asoD={}; if(c.aso_exame_data){const [d,m,y]=c.aso_exame_data.split('/');if(d&&m&&y)asoD[y+'-'+m.padStart(2,'0')+'-'+d.padStart(2,'0')]='aso';}
                const aus={...asoD,...((faltSet||{})[c.id]||{}),...((atestSet||{})[c.id]||{}),...((ferSet||{})[c.id]||{})};
                const escStr=(c.escala_tipo||'').toLowerCase();
                let fE=[]; try{const p=JSON.parse(c.escala_folgas||'[]');fE=Array.isArray(p)?p.map(f=>String(f).trim().toLowerCase()):[String(p).trim().toLowerCase()];}catch(e){fE=(c.escala_folgas||'').split(/[,;]+/).map(f=>f.trim().toLowerCase()).filter(Boolean);}
                ...
                const isDDL=(ds)=>{if(!temC||!cicB)return false;const dd=new Date(ds+'T12:00:00');if(dd.getDay()!==0)return false;const diff=Math.round((dd-cicB)/(86400000));if(diff<0)return false;return Math.round(diff/7)%3===2;};
                const getF=(ds)=>{const dd=new Date(ds+'T12:00:00');const dow=dd.getDay();if(escStr.includes('12x36')){if(c.escala_ciclo_inicio){const c12=new Date(c.escala_ciclo_inicio+'T12:00:00');const dif=Math.round((dd-c12)/86400000);if(dif<0)return false;return dif%2===1;}return false;}if(dow===0&&isDDL(ds))return true;if(ult!==null&&dow===ult){const sun=new Date(ds+'T12:00:00');sun.setDate(sun.getDate()-dow);if(isDDL(sun.toISOString().split('T')[0]))return false;}if(escStr.includes('escala_uma_folga')||escStr.includes('escala_duas_folgas')){if(fDow.length>0)return fDow.includes(dow);}if(escStr.includes('seg_sexta')||escStr.includes('5x2'))return dow===0||dow===6;if(escStr.includes('6x1'))return dow===0;return dow===0;};
