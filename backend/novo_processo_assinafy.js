...
module.exports = { enviarDocumentoParaAssinafy, gerarLinkSemEmail };

/**
 * Igual ao enviarDocumentoParaAssinafy, mas SEM notificação por e-mail.
 * Apenas gera o link de assinatura para ser copiado manualmente.
 */
async function gerarLinkSemEmail(documentId, colaboradorId) {
    console.log(`\n${'='.repeat(50)}`);
    console.log(`[ASSINAFY LINK] INICIO | Doc=${documentId} | Colab=${colaboradorId}`);
    ...
    // Upload do arquivo
    const filePath = require('path').resolve(doc.file_path);
    const fs = require('fs');
    if (!fs.existsSync(filePath)) throw new Error(`Arquivo não encontrado: ${filePath}`);
    ...
    const form = new FormData();
    form.append('file', fs.createReadStream(filePath), {
        filename: doc.file_name || require('path').basename(filePath),
        contentType: 'application/pdf'
    });
    ...
    if (!assinafyDocId) throw new Error('Upload OK mas ID não retornado.');
    ...
    for (let i = 1; i <= 60; i++) {
        ...
        const statusRes = await req('GET', `/v1/documents/${assinafyDocId}`, null);
        const docStatus = (statusRes.json?.data?.status || statusRes.json?.status || '').toLowerCase();
        if (!docStatus.includes('processing')) break;
        if (i === 60) throw new Error('Timeout no processamento pelo Assinafy.');
    }

    // Signatário (e-mail não é obrigatório para link, mas CPF sim)
    let signerId = null;
    const searchRes = await req('GET', `/v1/accounts/${ACCOUNT_ID}/signers?tax_id=${cpf}`, null);
    const lista = searchRes.json?.data || [];
    ...
    if (Array.isArray(lista) && lista.length > 0) {
        ...
        if (createRes.status < 200 || createRes.status >= 300)
            throw new Error(`Erro ao criar signatário: ${createRes.json?.message || createRes.raw}`);
        signerId = createRes.json?.data?.id || createRes.json?.id;
    }
    ...
    if (assignRes.status < 200 || assignRes.status >= 300)
        throw new Error(`Erro ao criar assignment: ${assignRes.json?.message || assignRes.raw}`);

    const assignList = (() => {
        const d = assignRes.json?.data;
    ...
    // Salvar no banco (mesmo fluxo, mas sem marcar como "enviado por e-mail")
    await new Promise((res, rej) =>
        db.run(
            `UPDATE documentos SET assinafy_id = ?, assinafy_status = 'Pendente', assinafy_url = ?, assinafy_sent_at = CURRENT_TIMESTAMP WHERE id = ?`,
            [assinafyDocId, urlAssinatura, documentId],
            (err) => err ? rej(err) : res()
        )
    );

    console.log(`[LINK GERADO] ${urlAssinatura}`);
    return { assinafyDocId, urlAssinatura, nomeColab: nome };
}