...
if (!html.includes('id="view-comercial-credenciamento"')) {
    const attachPointIdx = html.indexOf('id="view-licencas"');
    if (attachPointIdx !== -1) {
        const realAttach = html.lastIndexOf('<!--', attachPointIdx);
        html = html.substring(0, realAttach) + newView + '\n                ' + html.substring(realAttach);
