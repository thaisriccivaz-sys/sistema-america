...
const startStr = '// Atualizar saldo por endereço se informado';
const endStr = 'res.json({ success: true, quantidade_atual: novaQtd, tipo });';

const startIndex = code.indexOf(startStr);
const endIndex = code.indexOf(endStr, startIndex);

if (startIndex > 0 && endIndex > 0) {
    const chunkToReplace = code.substring(startIndex, endIndex + endStr.length);
    
    const newChunk = \`// Atualizar saldo por endereço se informado
                if (endereco_id) {
    ...
    console.log('patched movimentar with simple replace');
} else {
