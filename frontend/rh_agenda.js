        ...
        return `<div class="ag-wrap">
            <div class="ag-header">
                ...
                <div class="ag-header-right">
                    ...
                    <button class="ag-nav-btn" onclick="rhAgendaAbrirHistorico()" style="color:#7c3aed;border-color:#c4b5fd;" title="Histórico de alterações"><i class="ph ph-clock-counter-clockwise"></i> Histórico</button>
...
window.rhAgendaAbrirHistorico = async function() {
    ...
    overlay.innerHTML = `<div style="background:#fff;border-radius:16px;width:100%;max-width:1000px;box-shadow:0 25px 60px rgba(0,0,0,.35);animation:agHistSlide .25s ease;">
        <div style="display:flex;justify-content:space-between;align-items:center;padding:1.2rem 1.5rem;background:linear-gradient(135deg,#7c3aed,#5b21b6);border-radius:16px 16px 0 0;position:sticky;top:0;z-index:1;">
            ...
            <button onclick="document.getElementById('ag-rh-hist-overlay').remove()" style="background:rgba(255,255,255,.2);border:none;border-radius:8px;color:#fff;width:32px;height:32px;cursor:pointer;font-size:1.1rem;display:flex;align-items:center;justify-content:center;">✕</button>
        </div>
        <div style="padding:1.5rem;" id="ag-rh-hist-body"><div style="text-align:center;padding:2rem;color:#94a3b8;">Carregando...</div></div>
    </div>
    ...
    try {
        ...
        const body = document.getElementById('ag-rh-hist-body');
        if (!rows.length) {
        ...
        document.getElementById('ag-rh-hist-body').innerHTML = '<div style="color:#dc2626;padding:2rem;">Erro: ' + e.message + '</div>';
    }
};
