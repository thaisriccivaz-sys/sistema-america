  ...
  return _equipes.filter(eq => eq.nome !== 'Equipe Reserva').map(eq => {
    ...
    return `<div class="eq-col" data-equipe-id="${eq.id}" style="${colWidth}">
      ...
      <div class="eq-col-body" id="eq-body-${eq.id}"
        ...
        style="${isEquipePadrao ? 'flex-direction:row; flex-wrap:wrap; align-content:flex-start; column-gap:1rem; row-gap:0; background: linear-gradient(to right, transparent calc(50% - 0px), #e2e8f0 calc(50% - 0px), #e2e8f0 calc(50% + 1px), transparent calc(50% + 1px));' : ''}">
...
function _renderParesHtml(membros, b, isEquipePadrao = false) {
  ...
  for (let i = 0; i < rows; i++) {
    html += `<div style="display:flex; gap:4px; margin-bottom:2px;${isEquipePadrao ? ' width: calc(50% - 0.5rem);' : ''}">`;
