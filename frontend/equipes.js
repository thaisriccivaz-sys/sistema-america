...
window._eqAbrirHabilidades = function(e, colabId) {
    ...
    if (isLider) {
        skillsDisponiveis = ['Caminhão Carroceria', 'Caminhão Tanque', 'Carretinha Caminhonete', 'Carretinha Utilitário', 'VAC', 'Reparos', 'Montagem', 'Desmontagem', 'Visita técnica'];
    } else if (isMotorista) {
        skillsDisponiveis = ['Carretinha Caminhonete', 'Carretinha Utilitário', 'VAC', 'Reparos', 'Montagem', 'Desmontagem', 'Visita técnica'];
        if (cnhCaminhao) {
        ...
        // Ajudantes
        skillsDisponiveis = ['VAC', 'Reparos', 'Montagem', 'Desmontagem', 'Visita técnica'];
    }
...
function _renderBoard(busca) {
  ...
  return vEq.filter(eq => eq.nome !== 'Equipe Reserva' && eq.nome !== 'Equipe Intermitente').map(eq => {
    ...
    const colWidth = isEquipePadrao ? 'width: 680px; min-width: 680px;' : (emPares ? 'width: 340px; min-width: 340px;' : '');
...
function _renderCard(m) {
  ...
  const expTag = emExp ? `<span class="eq-card-func" style="background:#ffedd5;color:#c2410c;flex-shrink:1;min-width:0;">EXPERIÊNCIA</span>` : '';
