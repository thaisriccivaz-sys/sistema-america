...
window._setEpiQty = async function (epi, qty) {
    ...
    if (qty>prevQty && prevQty===0 && window._requiresSize(epi)) {
        ...
        if(tamanho){
            ...
            const idx = window._assinEpisDisponiveis.indexOf(epi);
            if(idx !== -1) {
                window._assinEpisDisponiveis[idx] = nomeComTamanho;
                if (epi !== nomeComTamanho) {
                    delete window._assinQtds[epi];
                }
            } else if (!window._assinEpisDisponiveis.includes(nomeComTamanho)) {
                window._assinEpisDisponiveis.push(nomeComTamanho);
            }
            window._assinQtds[nomeComTamanho] = qty;
