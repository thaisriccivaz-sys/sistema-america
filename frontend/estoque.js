    ...
    try {
        ...
        table.innerHTML = data.map(item => {
            ...
            const badges = saldos.length > 0
                ...
                : '<span style="color:#94a3b8;font-size:0.78rem;font-style:italic;">Sem endereco</span>';
            ...
            const warnIcon = isLow ? '<i class="ph ph-warning-circle" style="color:#ef4444;margin-right:4px;" title="Estoque Minimo"></i>' : "";
            return '<tr style="' + (isLow ? "background:#fff5f5;" : "") + '">' +
                ...
                '<td style="text-align:right;white-space:nowrap;">' +
                    ...
                    '<button class="btn btn-sm" onclick="window.ajustarEstoqueRapido(' + item.id + ',' + item.quantidade_atual + ',1)" title="Entrada Rapida" style="background:#f0fdf4;border:1px solid #bbf7d0;color:#16a34a;padding:4px 8px;border-radius:4px;"><i class="ph ph-plus"></i></button>' +
                    '<button class="btn btn-sm" onclick="window.abrirModalTransferirEstoque(' + item.id + ')" title="Transferir entre enderecos" style="background:#eff6ff;color:#1d4ed8;border:1px solid #bfdbfe;padding:4px 8px;border-radius:4px;margin-left:4px;"><i class="ph ph-arrows-left-right"></i></button>' +
...
window.abrirModalBaixaEstoque = async function(item) {
    if (!item) { console.error("[abrirModalBaixaEstoque] item nao encontrado no cache"); return; }
    ...
    const { value: vals, isConfirmed } = await Swal.fire({
        ...
        html: '<div style="text-align:left;">' +
            ...
            '<div style="margin-bottom:12px;">' +
                '<label style="font-weight:600;font-size:0.85rem;color:#475569;display:block;margin-bottom:4px;">Endereco para baixa *</label>' +
                '<select id="swal-baixa-end" style="width:100%;padding:8px 12px;border:1.5px solid #e2e8f0;border-radius:8px;font-size:0.9rem;"><option value="">— Selecione o local —</option>' + opts + '</select>' +
            '</div>' +
            '<div style="margin-bottom:12px;">' +
        ...
        preConfirm: () => {
            ...
            if (!endId) { Swal.showValidationMessage("Selecione o endereco"); return false; }
...
window.abrirModalNovoEndereco = async function() {
    const { value: nome, isConfirmed } = await Swal.fire({
        title: "Novo Endereco de Estoque",
        input: "text",
        inputLabel: "Nome do endereco (ex: Deposito A, Prateleira 2...)",
        inputPlaceholder: "Nome do endereco",
        showCancelButton: true,
        confirmButtonText: "Criar",
        confirmButtonColor: "#1d4ed8",
        cancelButtonText: "Cancelar",
        inputValidator: v => { if (!v || !v.trim()) return "Informe o nome do endereco"; }
    });
    ...
    try {
        ...
        Swal.fire({ icon: "success", title: "Endereco criado!", text: novo.nome + " adicionado.", timer: 1400, showConfirmButton: false });
    } catch(e) { Swal.fire("Erro", e.message, "error"); }
};
...
window.editarEstoque = async function(item) {
    if (!item) { console.error("[editarEstoque] item indefinido no cache"); return; }
    await _carregarEnderecos();
...
window.ajustarEstoqueRapido = async function(id, qtdAtual, variacao) {
    ...
    if (novaQtd < 0) return Swal.fire("Atencao", "A quantidade nao pode ser menor que zero.", "warning");
    const token = window.currentToken || localStorage.getItem("erp_token") || localStorage.getItem("token");
    try {
        ...
        window.renderEstoqueTable();
    } catch(e) { Swal.fire("Erro", "Nao foi possivel atualizar a quantidade.", "error"); }
};

// Excluir
window.excluirEstoque = async function(id) {
    const ok = await Swal.fire({ title: "Excluir item?", text: "Esta acao nao pode ser desfeita.", icon: "warning", showCancelButton: true, confirmButtonColor: "#ef4444", cancelButtonColor: "#64748b", confirmButtonText: "Sim, excluir" });
...
// Transferir estoque entre enderecos
...
window.renderEstoqueHistorico = async function() {
    ...
    if (!table) return;
    try {
        table.innerHTML = '<tr><td colspan="7" style="text-align:center;color:#64748b;">Carregando historico...</td></tr>';
        const token = window.currentToken || localStorage.getItem("erp_token") || localStorage.getItem("token");
        const r = await fetch(API_URL + "/estoque/historico", { headers: { "Authorization": "Bearer " + token } });
        if (!r.ok) throw new Error("Erro ao buscar historico");
        const data = await r.json();
        if (data.length === 0) {
            table.innerHTML = '<tr><td colspan="7" style="text-align:center;color:#64748b;">Nenhuma movimentacao registrada.</td></tr>';
            return;
        }
        table.innerHTML = "";
        data.forEach(h => {
        ...
        table.innerHTML = '<tr><td colspan="7" style="text-align:center;color:#ef4444;">Erro ao carregar historico: ' + err.message + '</td></tr>';
    }
};