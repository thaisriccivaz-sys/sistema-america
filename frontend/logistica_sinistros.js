...
window._logSinRenderCardGeral = function(s, container) {
    ...
    if (s.status === 'assinado') {
        // Após assinatura: sempre mostra Assinado, independente da situação
        statusText = 'Assinado';
        statusColor = '#10b981';
        statusBg = '#d1fae5';
    } else if (sit === 'Finalizado - Passar para RH' || sit === 'Finalizado') {
        statusText = 'Aguardando Assinaturas (RH)';
        statusColor = '#f59e0b';
        statusBg = '#fef3c7';
    } else if (sit === 'Finalizado (Interno)') {
        statusText = 'Finalizado';
        statusColor = '#6d28d9';
        statusBg = '#f3e8ff';
    } else {
        // Novo ou Em averiguação
...
window._logSinPreencherDadosVeiculo = function() {
    ...
    if (v.ano_modelo)         dados.push({ label: 'Ano',            value: v.ano_modelo });
