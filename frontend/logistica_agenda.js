        ...
        return `<div class="ag-wrap">
            <div class="ag-header">
                ...
                <div class="ag-header-right">
                    ...
                    <button class="ag-nav-btn" onclick="agendaAbrirHistorico()" style="color:#7c3aed;border-color:#c4b5fd;" title="Histórico de alterações de cards"><i class="ph ph-clock-counter-clockwise"></i> Histórico</button>
...
// ── Histórico de alterações de cards ──────────────────────────────────────────
window.agendaAbrirHistorico = async function() {
    const overlay = document.createElement('div');
    overlay.id = 'ag-hist-overlay';
    ...
    try {
        ...
        const res = await fetch('/api/logistica/agenda-auditoria?programa=Agenda Logística', {
        ...
        body.innerHTML = `<table style="width:100%;border-collapse:collapse;font-size:0.85rem;">
            ...
            <tbody>${rows.map((r, i) => {
                ...
                if (!cardData) {
                    const m = (r.conteudo_atual||'').match(/Data: ([\d\-]+)/);
                    if (m) cardData = m[1];
                }
